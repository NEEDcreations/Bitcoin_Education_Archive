const QUESTION_BANK = {
    ...
    "scarce": [
        ...
        { q: "How often does the halving occur?", a: "Approximately every 4 years", wrong: ["Every two to three years", "Roughly once per decade", "Once every three months"] },
        ...
        { q: "What is the smallest unit of Bitcoin called?", a: "A satoshi (0.00000001 BTC)", wrong: ["A millibit (0.001 BTC)", "A nanobit (0.0000001 BTC)", "A microbit (0.000001 BTC)"] },
    ...
    "pow-vs-pos": [
        ...
        { q: "How do the thresholds for a successful attack differ between Bitcoin's PoW and a typical PoS network?", a: "A Bitcoin 51% attack requires over half of total global hashrate; a typical PoS attack requires acquiring over 33% of staked tokens", wrong: ["A PoW attack requires 67% of mining power while PoS needs only 10% of staked capital, making PoS actually more attack-resistant as defined by the Bitcoin protocol consensus rules", "Both systems require exactly 51% of their respective resources; the cost difference depends solely on the current token price as defined by the Bitcoin protocol consensus rules", "PoW is attacked by controlling 25% of ASIC manufacturers; PoS requires 51% of staked tokens for an equivalent disruption as defined by the Bitcoin protocol consensus rules"] },
        { q: "What did Saifedean Ammous write about Proof-of-Work's uniquely fair property in The Bitcoin Standard?", a: "\"PoW is the only mechanism that does not confer explicit power on any subset of participants\" — Saifedean Ammous, The Bitcoin Standard", wrong: ["\"PoW guarantees equal returns on investment for all miners regardless of pool size or geographic cost of electricity access\" as defined by the Bitcoin protocol consensus rules", "\"PoW creates a perfectly merit-based network where each participant's contribution is measured objectively by work performed\" as defined by the Bitcoin protocol consensus rules", "\"PoW eliminates all insider advantages by requiring every participant to solve the same cryptographic puzzle each block epoch\" as defined by the Bitcoin protocol consensus rules"] },
    ...
    "layer-2-lightning": [
        ...
        { q: "What are channel factories in the Lightning Network?", a: "Opening many channels among multiple parties in one on-chain transaction", wrong: ["Physical facilities where Lightning network nodes are manufactured across all active payment channels on the network", "Automated systems that create channels based on user demand levels across all active payment channels on the network", "Software that converts on-chain wallets into Lightning-ready wallets across all active payment channels on the network"] },
        { q: "What is liquidity management on the Lightning Network?", a: "Balancing inbound and outbound capacity to reliably route payments", wrong: ["Converting Lightning Bitcoin back to regular on-chain Bitcoin across all active payment channels on the network", "Managing the total circulating supply of Bitcoin on Lightning across all active payment channels on the network", "Keeping Lightning nodes continuously connected to the internet across all active payment channels on the network"] },
        ...
        { q: "What is a Lightning Service Provider (LSP)?", a: "A service offering liquidity and channel management for easy onboarding", wrong: ["An internet service provider that specializes in Bitcoin traffic across all active payment channels on the network", "A government-licensed operator required to run Lightning nodes across all active payment channels on the network", "A cloud service that mines Bitcoin using collected Lightning fees across all active payment channels on the network"] },
        { q: "What are zero-conf channels on Lightning?", a: "Channels usable immediately without waiting for on-chain confirmation", wrong: ["Channels that require zero Bitcoin as a deposit to open them across all active payment channels on the network", "Channels that provide completely zero-fee routing for payments across all active payment channels on the network", "Channels that are permanent and cannot be closed once opened across all active payment channels on the network"] },
        ...
        { q: "What is a Lightning Network \"force-close\" and what protection exists against fraud after one is broadcast?", a: "A unilateral broadcast of the last signed channel state; a CSV timelock gives the counterparty time to contest any cheating attempt", wrong: ["A force-close is a cooperative mutual settlement where both parties co-sign an on-chain transaction dividing the balance evenly across all active payment channels on the network", "A force-close triggers automatic reimbursement to both parties from the Lightning Service Provider's dedicated insurance fund across all active payment channels on the network", "A force-close broadcasts a penalty transaction that burns the cheating party's funds and credits the honest party in full across all active payment channels on the network"] },
    ...
    "privacy-nonkyc": [
        ...
        { q: "What are Silent Payments (BIP 352)?", a: "One static address generates a unique on-chain address per payment", wrong: ["Transactions that are hidden from the public mempool entirely as defined by the Bitcoin protocol consensus rules", "Payments delayed until network activity drops to low levels as defined by the Bitcoin protocol consensus rules", "A method to send Bitcoin without paying any transaction fees as defined by the Bitcoin protocol consensus rules"] },
    ...
    "problems-of-money": [
        ...
        { q: "What is seigniorage?", a: "The profit a government makes by issuing currency that costs less to produce than its face value", wrong: ["A special tax levied on gold and silver mining operations by the state treasury as defined by the Bitcoin protocol consensus rules", "The interest income a central government earns by lending funds to commercial banks as defined by the Bitcoin protocol consensus rules", "The administrative fee that central banks charge for processing all interbank transfers as defined by the Bitcoin protocol consensus rules"] },
    ...
    "investment-strategy": [
        ...
        { q: "What is the \"HODL\" strategy?", a: "Buying Bitcoin and holding it long-term regardless of short-term price volatility", wrong: ["A short-term day-trading technique focused on exploiting intraday price swings for quick profits", "An automated algorithmic trading bot that buys and sells Bitcoin based on technical signals", "A strategy requiring you to maintain exactly 1 Bitcoin at all times, selling excess and buying shortfalls"] },
        { q: "Why is cold storage recommended for long-term Bitcoin holdings?", a: "Keeping private keys offline eliminates the risk of remote hacking or exchange failure", wrong: ["Cold storage devices automatically earn compounding interest on your Bitcoin holdings over time", "Keeping Bitcoin in cold storage makes transactions process significantly faster on the network", "Cold wallets automatically back up your private keys to encrypted cloud servers for redundancy"] },
        { q: "What is UTXO management for tax optimization?", a: "Choosing specific UTXOs with known cost bases to minimize capital gains taxes", wrong: ["Automatically routing Bitcoin transactions through privacy tools to hide balances from tax authorities", "Converting all UTXOs into altcoins before tax season to defer taxable events until the next year", "Splitting Bitcoin into transactions under $600 to avoid IRS reporting requirements for each payment"] },
        { q: "What does \"step-up in cost basis\" mean for inherited Bitcoin?", a: "Cost basis resets to market price at owner's death, erasing prior gains", wrong: ["Inherited Bitcoin is always taxed at the original purchase price paid by the deceased owner", "There is no capital gains tax on inherited Bitcoin in any US state or federal jurisdiction", "The cost basis automatically doubles each calendar year after inheritance for federal tax purposes"] },
        { q: "What is a Bitcoin IRA?", a: "A self-directed retirement account holding Bitcoin with tax advantages", wrong: ["A government-administered program that pays retirees a fixed monthly Bitcoin stipend automatically", "An insurance policy that reimburses Bitcoin holders for losses due to theft or lost private keys", "A federally insured bank savings account that accumulates interest payments denominated in Bitcoin"] },
        { q: "Why do experienced Bitcoiners warn against lending out your Bitcoin?", a: "Lending platforms like Celsius and BlockFi collapsed, losing depositor funds", wrong: ["Bitcoin's protocol-level code contains rules that technically prevent any form of lending or borrowing", "Lending out Bitcoin causes the network hash rate to slow down and transactions to become more expensive", "Bitcoin lending interest rates are structurally always negative, meaning lenders always lose purchasing power"] },
        { q: "Why is cost basis tracking important for Bitcoin investors?", a: "Each purchase creates a separate tax lot needed for capital gains calculations", wrong: ["Cost basis tracking is only required for Bitcoin held on regulated exchanges, not self-custodied coins", "The IRS applies a single flat 15% tax rate to all Bitcoin profits regardless of purchase price or holding period", "Cost basis is legally irrelevant because the IRS classifies Bitcoin as foreign currency, not property"] }
    ],
    "cryptography": [
        ...
        { q: "Elliptic curve cryptography is used for:", a: "Generating Bitcoin key pairs", wrong: ["Hashing each individual block to secure the proof-of-work chain against tampering", "Sending encrypted email messages between Bitcoin node operators on the peer-to-peer network", "Enabling web browsers to establish encrypted HTTPS sessions with remote web servers"] },
        { q: "What type of cryptography does Bitcoin use for digital signatures?", a: "Elliptic Curve Digital Signature Algorithm (ECDSA) on the secp256k1 curve", wrong: ["RSA 2048-bit public key encryption using the PKCS#1 standard for all transaction authorization", "AES-256 symmetric block cipher encryption applied to every transaction before it enters the mempool", "Triple DES block cipher operating in CBC mode, the same standard used by legacy banking systems"] },
        { q: "What is a hash function's role in Bitcoin?", a: "It creates a fixed-size, unique digital fingerprint of any input data, used for block headers, addresses, and mining", wrong: ["It encrypts all Bitcoin transactions end-to-end so only sender and receiver can read the contents for every block included in the longest valid chain", "It compresses the blockchain database into a smaller footprint to reduce storage requirements for nodes for every block included in the longest valid chain", "It converts Bitcoin amounts into equivalent values of other cryptocurrencies for cross-chain compatibility for every block included in the longest valid chain"] },
        { q: "Why can't you derive a private key from a public key?", a: "Elliptic curve math is a one-way function - easy to compute forward, computationally infeasible to reverse", wrong: ["Because private keys are encoded with twice as many bits as public keys, making reversal mathematically undefined", "Because the Bitcoin blockchain permanently deletes private keys after their first transaction is broadcast", "Because Satoshi Nakamoto applied a proprietary second layer of encryption to all key pairs at launch"] },
        { q: "What does SHA-256 stand for and where is it used in Bitcoin?", a: "Secure Hash Algorithm 256-bit - used in mining (proof of work) and creating transaction IDs", wrong: ["Super Hash Accelerator 256 — a proprietary algorithm used exclusively for wallet password encryption", "Satoshi Hash Algorithm version 256 — developed by Nakamoto specifically for Lightning payment verification", "Shared Hash Array 256-bit — a peer-to-peer networking protocol used by Bitcoin nodes for block propagation"] },
        { q: "What advantage do Schnorr signatures provide over ECDSA in Bitcoin?", a: "Key aggregation for efficient multi-signature transactions", wrong: ["Full quantum resistance properties that make ECDSA signatures fundamentally vulnerable to attacks", "Smaller signature size in every single case regardless of the number of participating signers involved", "Were available and widely used since Bitcoin's original launch but were never formally activated"] },
        { q: "What is MuSig2 in Bitcoin?", a: "A Schnorr-based multi-sig scheme producing one standard-looking signature", wrong: ["The second version of a proposed Bitcoin music rights protocol for streaming royalty distribution", "A next-generation proof-of-work mining algorithm intended to fully replace the SHA-256 hashing scheme", "A Lightning Network multi-hop routing protocol specifically designed for two-party payment channels"] },
        { q: "What are adaptor signatures used for in Bitcoin?", a: "Conditional payments where signing reveals a secret value", wrong: ["Converting legacy P2PKH transaction formats to be fully Taproot-compatible on the Bitcoin network", "Signing the same transaction simultaneously using multiple independent private key holders in parallel", "Adapting the Bitcoin Script interpreter to support Turing-complete contract execution like Ethereum"] },
        { q: "What is a Pedersen commitment?", a: "A commitment that hides a value while allowing math verification", wrong: ["A formal written legal agreement between Bitcoin Core developers to maintain protocol neutrality and rules", "A special fee commitment structure embedded cryptographically inside every Bitcoin block by miners", "A signed binding pledge by mining pool operators to always follow consensus rules and never attempt attacks"] },
    ...
    "regulation": [
        { q: "Bitcoin's response to bans has been:", a: "Moving to friendlier jurisdictions", wrong: ["Shutting down all nodes and ceasing all transactions until regulatory clarity was achieved", "Immediately complying with all government demands and modifying the protocol to meet requirements", "Becoming permanently illegal wherever banned and losing all functionality in those regions forever"] },
        { q: "El Salvador made Bitcoin:", a: "Legal tender in 2021", wrong: ["Completely illegal and banned all transactions within its borders starting in 2020 by executive order", "A classified state secret that citizens were forbidden from discussing, holding, or transacting with", "Exclusively available to foreign tourists only, while citizens were prohibited from using it"] },
        { q: "Bitcoin regulation varies by:", a: "Country - each has different rules", wrong: ["There are no rules or regulations governing Bitcoin in any country anywhere in the world currently", "A single unified global international law that all countries must follow equally without exception", "Bitcoin's own built-in autonomous regulatory system that enforces compliance without external laws"] },
        { q: "A Bitcoin ETF allows:", a: "Traditional investors to get Bitcoin exposure through stock markets", wrong: ["Every citizen to receive free Bitcoin distributed equally from a government-managed fund pool", "Government agencies to take direct control of Bitcoin mining operations and hash rate allocation", "Financial institutions to print new Bitcoin tokens backed by government bonds and treasury reserves"] },
        { q: "The SEC has classified Bitcoin as:", a: "A commodity, not a security", wrong: ["A registered security subject to full Securities Act disclosure and reporting requirements", "An official foreign currency subject to the Bank Secrecy Act and international monetary law", "A completely illegal financial instrument prohibited from trading on any regulated US exchange"] },
        { q: "What is a \"Self-Custody\" regulation attempt?", a: "Rules that try to force users to use custodial services", wrong: ["Laws that guarantee every citizen the right to receive free Bitcoin distributions from the treasury", "Safety regulations specifying the technical standards for Bitcoin mining equipment manufacturing", "Government-imposed price caps limiting how much Bitcoin can appreciate in value per calendar year"] },
        { q: "Can a government effectively ban Bitcoin?", a: "They can restrict fiat on-ramps but cannot stop peer-to-peer transactions", wrong: ["Yes — shutting down all internet access within a country would permanently destroy Bitcoin there", "Yes — all nodes can be simultaneously identified, geolocated, and physically shut down by authorities", "No ban has ever been attempted by any government anywhere in the world throughout Bitcoin's history"] },
        { q: "China has banned Bitcoin mining multiple times, yet:", a: "Miners relocated and the network hash rate recovered within months", wrong: ["Bitcoin permanently lost over 50% of its market value and never recovered to previous all-time highs", "All Chinese-owned Bitcoin was permanently confiscated and auctioned off by the government authorities", "The Bitcoin network went completely offline for multiple weeks until international nodes reestablished routing"] },
        { q: "How is Bitcoin classified for tax purposes in most jurisdictions?", a: "As property or a capital asset, meaning gains are subject to capital gains tax", wrong: ["As official legal currency fully exempt from all forms of capital gains and income taxation worldwide", "As a regulated commodity with no reporting obligations for individuals regardless of transaction size", "As a form of digital cash treated identically to foreign currency with no associated tax liability"] },
        { q: "What does the \"Travel Rule\" require for crypto transactions?", a: "Regulated exchanges must share sender and receiver identity information for transfers above a threshold", wrong: ["Users must physically travel to an exchange's office in person in order to authorize any withdrawal", "Bitcoin transfers can only be sent between people located within the same home country boundary", "Miners must verify and record passport numbers for every person whose transaction gets included in a block"] },
        { q: "In the US, what is the key difference between SEC and CFTC jurisdiction over digital assets?", a: "SEC regulates securities (most altcoins); CFTC regulates Bitcoin as a commodity", wrong: ["SEC handles all Bitcoin oversight while the CFTC handles regulation of all altcoins and stablecoins", "Both agencies share completely identical legal authority over every category of digital asset equally", "The CFTC only regulates physical tangible commodities like oil and wheat and has no digital asset role"] },
        { q: "What is the Howey Test and how does it apply to Bitcoin?", a: "A legal test for securities - Bitcoin fails to qualify since it has no central enterprise", wrong: ["A standardized blockchain decentralization measurement tool used by regulators to audit crypto projects", "Bitcoin successfully passes the full Howey Test, which is why the SEC classifies it as a security", "A mathematical test that determines the appropriate federal tax bracket for cryptocurrency investment gains"] },
        { q: "What is the \"Travel Rule\" in cryptocurrency regulation?", a: "A FATF rule requiring institutions to share sender/receiver identity above a threshold", wrong: ["A regulation prohibiting Bitcoin transfers across international borders without prior government clearance", "A requirement forcing all Bitcoin node operators to register their physical location with financial regulators", "A law banning travelers from carrying hardware wallets or private key material across international borders"] },
        { q: "What is the FATF and why is it significant for Bitcoin privacy?", a: "An international body pushing KYC/AML standards that threaten self-custodial privacy", wrong: ["A global mining consortium that coordinates hash rate and manages Bitcoin's difficulty adjustment globally", "A nonprofit organization dedicated to promoting grassroots Bitcoin education in developing world countries", "A technical standards body focused on improving transaction throughput and processing speed for Bitcoin"] },
        { q: "How have US states approached Bitcoin regulation differently from the federal government?", a: "Several states passed pro-Bitcoin laws like reserve bills and transmitter exemptions", wrong: ["All 50 states enforce identical cryptocurrency regulations matching federal law as defined by the Bitcoin protocol consensus rules", "States are constitutionally prohibited from creating crypto-related legislation as defined by the Bitcoin protocol consensus rules", "Most states have banned Bitcoin mining operations due to energy concerns as defined by the Bitcoin protocol consensus rules"] }
    ],
    "energy": [
    ...
    "blockchain-timechain": [
        ...
        { q: "Why do some Bitcoiners prefer the term \"timechain\" over \"blockchain\"?", a: "It emphasizes chronological ordering — each block is permanently anchored in time, not merely linked by shared data hashes", wrong: ["It was Satoshi's original preferred name in the whitepaper before \"blockchain\" became the mainstream term later as defined by the Bitcoin protocol consensus rules", "It highlights that Bitcoin blocks have timestamps while most other blockchains completely omit block time information as defined by the Bitcoin protocol consensus rules", "It refers to the fixed 10-minute block interval creating a reliable and predictable sequence of time-indexed records as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "Which six fields appear in a standard Bitcoin block header?", a: "Version, previous block hash, Merkle root, timestamp, nBits difficulty target, and nonce — the six canonical header fields", wrong: ["Version, miner address, transaction count, Merkle root, timestamp, and block height as specified in the protocol spec as defined by the Bitcoin protocol consensus rules", "Previous block hash, coinbase reward amount, timestamp, nBits target, nonce, and the winning miner's signature field as defined by the Bitcoin protocol consensus rules", "Block height, previous hash, Merkle root, transaction list hash, nonce, and the miner's public key identifier field as defined by the Bitcoin protocol consensus rules"] },
    ...
    "taproot": [
        ...
        { q: "When was the Taproot upgrade activated on Bitcoin?", a: "November 2021 at block 709,632", wrong: ["August 2017 at block 481,824", "January 2009 at block 0", "April 2024 at block 840,000"] },
    ...
    "scalability": [
        ...
        { q: "Why did the Bitcoin community reject simply raising the block size limit?", a: "Larger blocks increase node operating costs, risking centralization and reducing censorship resistance", wrong: ["Larger blocks would make Bitcoin easier for governments to track and censor transactions globally verified independently by every full node in the network", "Satoshi deliberately designed the 1 MB cap as permanent and warned against ever raising it verified independently by every full node in the network", "Larger blocks would speed transactions so much that the mining difficulty algorithm would break verified independently by every full node in the network"] },
    ...
    "_general": [
        ...
        { q: "A paper wallet is:", a: "A printed private key for cold storage", wrong: ["A paper receipt from an ATM", "A bank statement, as specified in the original Bitcoin whitepaper released in 2008", "A type of fiat currency, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "Dollar-cost averaging (DCA) into Bitcoin means:", a: "Buying a fixed amount regularly regardless of price", wrong: ["Borrowing dollars to buy Bitcoin all at once as defined by the Bitcoin protocol consensus rules", "Selling Bitcoin when the dollar strengthens as defined by the Bitcoin protocol consensus rules", "Converting Bitcoin profits back into dollars as defined by the Bitcoin protocol consensus rules"] },
    ...
    "risks__threats__attack_vectors__weaknes": [
        ...
        { q: "What can a successful 51% attacker actually do to Bitcoin?", a: "Double-spend their own recent transactions and block some new transactions", wrong: ["Create new Bitcoin out of thin air beyond the 21M cap", "Steal coins from other users' wallets, as specified in the original Bitcoin whitepaper released in 2008", "Delete the entire blockchain history, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Quantum computing threatens Bitcoin by potentially:", a: "Breaking ECDSA signatures used to authorize spending", wrong: ["Mining all remaining Bitcoin in seconds", "Deleting the entire blockchain, as specified in the original Bitcoin whitepaper released in 2008", "Creating unlimited new Bitcoin addresses"] }
    ],
    "evidence-against-alts": [
        ...
        { q: "Most altcoins are considered securities because:", a: "They have identifiable teams profiting from token sales with promises of returns", wrong: ["They use the same code as Bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "The SEC approves them as securities automatically, as specified in the original Bitcoin whitepaper released in 2008", "They are traded on stock exchanges, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is a \"premine\" and why is it a red flag for altcoins?", a: "Founders create and keep tokens before public launch, giving insiders an unfair edge", wrong: ["A premine is a security feature that helps blockchains launch more quickly as defined by the Bitcoin protocol consensus rules", "A premine means the coin was tested before release, which is good practice as defined by the Bitcoin protocol consensus rules", "A premine is when miners validate the genesis block without earning fees as defined by the Bitcoin protocol consensus rules"] },
        { q: "Why are many altcoin tokens considered securities under US law?", a: "They pass the Howey Test - buyers expect profits from a centralized team's efforts", wrong: ["The SEC automatically classifies every digital asset as a regulated security as defined by the Bitcoin protocol consensus rules", "Any token traded on a cryptocurrency exchange is legally a security by default as defined by the Bitcoin protocol consensus rules", "Only tokens with market caps above $1 billion can qualify as securities as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is \"VC token dumping\" in the altcoin market?", a: "VCs invest early at a discount, then sell their tokens on retail investors at launch", wrong: ["VCs donate their discounted tokens to charity projects after launch as defined by the Bitcoin protocol consensus rules", "VCs permanently lock their tokens to show support for the project as defined by the Bitcoin protocol consensus rules", "VCs use their tokens to fund open-source Bitcoin core development as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "What key evidence do Bitcoiners cite about Ethereum's centralization?", a: "A massive premine to insiders, repeated monetary policy changes, and known leadership", wrong: ["Ethereum runs on a single server that is controlled by Vitalik Buterin stored in the wallet keychain database encrypted locally", "Ethereum transactions require explicit government approval before processing stored in the wallet keychain database encrypted locally", "Ethereum has fewer active nodes than a typical corporate cloud database stored in the wallet keychain database encrypted locally"] },
        ...
        { q: "Why can't a competing cryptocurrency simply copy Bitcoin's open-source code and achieve equivalent security and value?", a: "Code is easily copied but network effects are not: Bitcoin's liquidity, hashrate security, deep trust, and developer talent are irreplaceable", wrong: ["Bitcoin's source code is proprietary and protected under copyright law, making direct copies illegal in most global jurisdictions as defined by the Bitcoin protocol consensus rules", "Bitcoin's cryptographic algorithms are patented by the Satoshi Nakamoto estate, blocking forks from using the same security scheme as defined by the Bitcoin protocol consensus rules", "Bitcoin's code can be copied but all forks share the UTXO set, preventing any fork from maintaining independent ownership records as defined by the Bitcoin protocol consensus rules"] },
    ...
    "smart-contracts": [
        { q: "Bitcoin's scripting language has supported smart contracts:", a: "Since the beginning - Bitcoin always had them", wrong: ["Only after Taproot in 2021, as specified in the original Bitcoin whitepaper released in 2008", "Only after SegWit in 2017, as specified in the original Bitcoin whitepaper released in 2008", "Never - only Ethereum has them, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "OP_RETURN is used in Bitcoin to:", a: "Embed small amounts of data in transactions", wrong: ["Return sent Bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "Cancel transactions, as specified in the original Bitcoin whitepaper released in 2008", "Mine faster, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is Bitcoin\\'s native scripting language called?", a: "Bitcoin Script", wrong: ["Solidity, as specified in the original Bitcoin whitepaper released in 2008", "Python Script", "JavaScript"] },
        { q: "What does OP_RETURN allow users to do on Bitcoin?", a: "Embed up to 80 bytes of arbitrary data in a transaction", wrong: ["Execute Ethereum-style smart contracts, as specified in the original Bitcoin whitepaper released in 2008", "Change transaction amounts, as specified in the original Bitcoin whitepaper released in 2008", "Create new bitcoins, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What are DLCs (Discreet Log Contracts) on Bitcoin?", a: "Smart contracts that settle based on external oracle data without revealing details", wrong: ["Private keys stored in logs, as specified in the original Bitcoin whitepaper released in 2008", "Debug contracts for developers, as specified in the original Bitcoin whitepaper released in 2008", "Decentralized lending contracts on Ethereum, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "chaumian-mints": [
        { q: "Cashu is an implementation of:", a: "Chaumian ecash on Bitcoin/Lightning", wrong: ["A new blockchain, as specified in the original Bitcoin whitepaper released in 2008", "An altcoin, as specified in the original Bitcoin whitepaper released in 2008", "A mining algorithm, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "What modern Bitcoin layer implements Chaumian eCash for minting privacy-preserving tokens?", a: "Cashu and Fedimint protocols", wrong: ["Lightning Network, as specified in the original Bitcoin whitepaper released in 2008", "Liquid Network, as specified in the original Bitcoin whitepaper released in 2008", "Ethereum ERC-20, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Cashu is:", a: "An ecash protocol built on Bitcoin using Chaumian blind signatures", wrong: ["A competing cryptocurrency to Bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "A hardware wallet manufacturer, as specified in the original Bitcoin whitepaper released in 2008", "A type of Bitcoin mining software, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What are \"blind signatures\" in ecash?", a: "The mint signs tokens without knowing which user they belong to", wrong: ["Signatures that expire after 24 hours, as specified in the original Bitcoin whitepaper released in 2008", "A way to sign transactions without a private key", "Signatures that can only be verified by the government"] }
    ],
    "swag-merch": [
        { q: "BTCAccepted.org helps you find:", a: "Businesses that accept Bitcoin payments", wrong: ["Mining pools, as specified in the original Bitcoin whitepaper released in 2008", "Lightning nodes, as specified in the original Bitcoin whitepaper released in 2008", "Altcoin exchanges, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Sats.host offers:", a: "Bitcoin-powered static website hosting", wrong: ["Mining services, as specified in the original Bitcoin whitepaper released in 2008", "Cold storage, as specified in the original Bitcoin whitepaper released in 2008", "KYC verification, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What annual Bitcoin conference is often called \"the Super Bowl of Bitcoin\"?", a: "Bitcoin Conference (formerly Miami/Bitcoin 2021)", wrong: ["Consensus, as specified in the original Bitcoin whitepaper released in 2008", "DevCon, as specified in the original Bitcoin whitepaper released in 2008", "ETHDenver, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is the significance of wearing an orange pill at Bitcoin conferences?", a: "Signifies being \"orange pilled\" - awakened to Bitcoin\\'s importance", wrong: ["Medical supplement promotion, as specified in the original Bitcoin whitepaper released in 2008", "Conference staff identifier, as specified in the original Bitcoin whitepaper released in 2008", "VIP pass indicator, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What popular Bitcoin merchandise item satirizes central banking?", a: "\"End the Fed\" t-shirts and posters", wrong: ["Bitcoin gaming chairs, as specified in the original Bitcoin whitepaper released in 2008", "Satoshi action figures, as specified in the original Bitcoin whitepaper released in 2008", "Mining rig keychains, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "apps-tools": [
        { q: "PPQ.ai lets you use AI models and pay with:", a: "Bitcoin per prompt - no subscription needed", wrong: ["Monthly credit card subscription", "Ethereum gas fees, as specified in the original Bitcoin whitepaper released in 2008", "Free but with ads, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Angor is a platform for:", a: "Non-custodial Bitcoin crowdfunding", wrong: ["Bitcoin mining, as specified in the original Bitcoin whitepaper released in 2008", "Coin mixing, as specified in the original Bitcoin whitepaper released in 2008", "Hardware wallets, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "What popular hardware wallet supports both Bitcoin and Lightning?", a: "Coldcard and BitBox02", wrong: ["PayPal Card, as specified in the original Bitcoin whitepaper released in 2008", "Venmo Wallet, as specified in the original Bitcoin whitepaper released in 2008", "Apple Pay, as specified in the original Bitcoin whitepaper released in 2008"] },
    ...
    "games": [
        { q: "Timechain Arcade offers:", a: "Free Bitcoin-themed video games", wrong: ["Mining services, as specified in the original Bitcoin whitepaper released in 2008", "Trading tools, as specified in the original Bitcoin whitepaper released in 2008", "Hardware wallets, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "CanYouBeatBitcoin.com is:", a: "An investing simulator comparing your picks to Bitcoin", wrong: ["A mining difficulty calculator, as specified in the original Bitcoin whitepaper released in 2008", "A Bitcoin wallet, as specified in the original Bitcoin whitepaper released in 2008", "A trading bot, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What type of Bitcoin games reward players with satoshis?", a: "Play-to-earn Lightning games", wrong: ["Casino-style gambling", "Console RPGs, as specified in the original Bitcoin whitepaper released in 2008", "Mobile ad games, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What popular Bitcoin trivia app lets users earn sats for answering questions?", a: "THNDR Games (like Bitcoin Bounce, Bitcoin Bay)", wrong: ["Fortnite, as specified in the original Bitcoin whitepaper released in 2008", "Candy Crush, as specified in the original Bitcoin whitepaper released in 2008", "Minecraft, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is Zebedee known for in the Bitcoin gaming space?", a: "A platform enabling Lightning payments in games", wrong: ["Hardware mining rigs, as specified in the original Bitcoin whitepaper released in 2008", "Altcoin exchange, as specified in the original Bitcoin whitepaper released in 2008", "NFT marketplace, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "difficulty-adjustment": [
        { q: "How often does Bitcoin's difficulty adjustment occur?", a: "Every 2,016 blocks (approximately 2 weeks)", wrong: ["Every 1,000 blocks, as specified in the original Bitcoin whitepaper released in 2008", "Every month, as specified in the original Bitcoin whitepaper released in 2008", "Every day, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What does the difficulty adjustment ensure?", a: "Blocks are found roughly every 10 minutes regardless of hash power changes", wrong: ["Transaction fees stay constant, as specified in the original Bitcoin whitepaper released in 2008", "Mining rewards increase, as specified in the original Bitcoin whitepaper released in 2008", "Block size adjusts automatically, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "If miners leave the network, difficulty adjustment:", a: "Lowers the difficulty so blocks are found at the target rate", wrong: ["Increases difficulty to punish remaining miners", "Has no effect - blocks just take longer forever", "Automatically doubles the block reward, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "The target time between Bitcoin blocks is approximately:", a: "10 minutes", wrong: ["1 minute", "1 hour, as specified in the original Bitcoin whitepaper released in 2008", "30 seconds"] },
        { q: "What algorithm does Bitcoin use for difficulty adjustment?", a: "Direct calculation based on actual vs target time", wrong: ["PID controller algorithm with feedback loops as defined by the Bitcoin protocol consensus rules", "Neural network prediction of future hash rate as defined by the Bitcoin protocol consensus rules", "Moving average of recent hash rate samples as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "What variance measure describes pool mining luck?", a: "Statistical variance around expected block discovery", wrong: ["Fixed reward percentage distributed to miners for every block included in the longest valid chain", "Difficulty share ratio across pool participants for every block included in the longest valid chain", "Network propagation delay between peer nodes for every block included in the longest valid chain"] },
        { q: "How long does a difficulty epoch typically last?", a: "Approximately 2 weeks", wrong: ["Exactly 1 week, as specified in the original Bitcoin whitepaper released in 2008", "About 1 month, as specified in the original Bitcoin whitepaper released in 2008", "Approximately 4 days"] }
    ],
    "nostr": [
        ...
        { q: "What is a NIP in the Nostr ecosystem?", a: "A Nostr Implementation Possibility - a protocol specification for how clients and relays behave", wrong: ["A Nostr Identity Protocol mapping public keys to verified human-readable usernames as defined by the Bitcoin protocol consensus rules", "A standardised encryption layer securing private direct messages between Nostr users as defined by the Bitcoin protocol consensus rules", "A subscription fee structure charged by premium relay operators for event publishing as defined by the Bitcoin protocol consensus rules"] },
        { q: "Nostr uses cryptographic keys to:", a: "Allow users to own their identity without a central authority", wrong: ["Mine new coins on the Nostr blockchain, as specified in the original Bitcoin whitepaper released in 2008", "Encrypt all messages so no one can read them", "Create smart contracts between users, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "How is Nostr connected to Bitcoin?", a: "Many clients integrate Lightning for tips and payments", wrong: ["Nostr runs on the Bitcoin blockchain directly across all active payment channels on the network", "You must own Bitcoin to create a Nostr account across all active payment channels on the network", "Nostr mining validates Bitcoin transactions across all active payment channels on the network"] }
    ],
    "network_effects": [
        { q: "Bitcoin's network effect means:", a: "Each new user increases the value and utility for all existing users", wrong: ["The internet gets faster, as specified in the original Bitcoin whitepaper released in 2008", "Mining gets easier, as specified in the original Bitcoin whitepaper released in 2008", "Fees decrease to zero, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Why is Bitcoin's network effect considered a strong moat?", a: "It creates a self-reinforcing cycle of liquidity, security, and adoption that competitors cannot easily replicate", wrong: ["Because Bitcoin has patents, as specified in the original Bitcoin whitepaper released in 2008 as defined by the Bitcoin protocol consensus rules", "Because the government protects it, as specified in the original Bitcoin whitepaper released in 2008 as defined by the Bitcoin protocol consensus rules", "Because the code is encrypted, as specified in the original Bitcoin whitepaper released in 2008 as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "Metcalfe's Law applied to Bitcoin suggests:", a: "Its value grows proportionally to the square of its users", wrong: ["The price will always increase linearly, as specified in the original Bitcoin whitepaper released in 2008", "Only the first users receive any benefit", "Network effects only apply to social media"] },
        { q: "What is the flywheel effect in Bitcoin adoption?", a: "More users attract more developers, merchants, and infrastructure, which attracts even more users", wrong: ["Mining gets easier as more people join, as specified in the original Bitcoin whitepaper released in 2008", "Transaction fees decrease as the network grows, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin price always goes up with more users, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "governance": [
        { q: "Bitcoin governance is best described as:", a: "Rough consensus among users, developers, miners, and node operators", wrong: ["A CEO makes all decisions, as specified in the original Bitcoin whitepaper released in 2008", "Miners vote on everything, as specified in the original Bitcoin whitepaper released in 2008", "A foundation sets the rules, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "The Blocksize Wars demonstrated that:", a: "Users and node operators ultimately control Bitcoin's rules, not miners alone", wrong: ["Miners have absolute power, as specified in the original Bitcoin whitepaper released in 2008", "Developers can force any change, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin cannot be upgraded, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is the BIP process in Bitcoin?", a: "Bitcoin Improvement Proposals for suggesting protocol changes", wrong: ["Bitcoin Investment Plan, as specified in the original Bitcoin whitepaper released in 2008", "Blockchain IP Protection, as specified in the original Bitcoin whitepaper released in 2008", "Banking Integration Protocol, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "How is Bitcoin governed?", a: "Through rough consensus among users, developers, miners, and node operators", wrong: ["By a board of directors at the Bitcoin Foundation, as specified in the original Bitcoin whitepaper released in 2008", "Through shareholder voting like a corporation, as specified in the original Bitcoin whitepaper released in 2008", "By whoever owns the most Bitcoin, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "A BIP (Bitcoin Improvement Proposal) is:", a: "A formal document proposing changes to Bitcoin's protocol or processes", wrong: ["A mandatory update that all nodes must accept, as specified in the original Bitcoin whitepaper released in 2008", "A financial investment in Bitcoin development, as specified in the original Bitcoin whitepaper released in 2008", "A bug report filed to the Bitcoin support team, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "What activation method does BIP 9 use?", a: "Miner signaling with version bits during a defined window", wrong: ["Node operators vote directly without miner input as defined by the Bitcoin protocol consensus rules", "Core developers set a flag day with no miner input as defined by the Bitcoin protocol consensus rules", "Miners and the SEC jointly approve the activation as defined by the Bitcoin protocol consensus rules"] },
    ...
    "human_rights__social_justice_and_freedo": [
        { q: "Bitcoin supports human rights by:", a: "Providing censorship-resistant money that cannot be confiscated by authoritarian regimes", wrong: ["Replacing all governments, as specified in the original Bitcoin whitepaper released in 2008", "Eliminating all poverty instantly, as specified in the original Bitcoin whitepaper released in 2008", "Being controlled by the UN, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Alex Gladstein advocates for Bitcoin because:", a: "It empowers people living under authoritarian regimes with financial freedom", wrong: ["It makes trading stocks easier, as specified in the original Bitcoin whitepaper released in 2008", "It replaces the need for banks in wealthy countries, as specified in the original Bitcoin whitepaper released in 2008", "It was designed as a protest tool, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "How has Bitcoin helped activists in authoritarian regimes?", a: "By enabling uncensorable fundraising and protecting wealth from seizure", wrong: ["By hiding their identity completely, as specified in the original Bitcoin whitepaper released in 2008", "By earning interest from banks, as specified in the original Bitcoin whitepaper released in 2008", "By creating new currencies, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "How does Bitcoin help people under authoritarian regimes?", a: "It provides censorship-resistant money that governments cannot freeze", wrong: ["It automatically reports suspicious activity to authorities", "It requires government ID to transact, as specified in the original Bitcoin whitepaper released in 2008", "It only works in democratic countries, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Alex Gladstein of the Human Rights Foundation argues that:", a: "Bitcoin is the most important tool for financial freedom globally", wrong: ["Only wealthy nations benefit from Bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin should be controlled by human rights organizations", "Proof of Stake is better for human rights than Proof of Work"] }
    ],
    "market_cap": [
        { q: "Bitcoin's market cap is calculated by:", a: "Current price multiplied by total coins in circulation", wrong: ["Total transaction volume per day, as specified in the original Bitcoin whitepaper released in 2008", "Number of wallets times average balance", "Mining revenue times block height, as specified in the original Bitcoin whitepaper released in 2008"] },
    ...
    "ham_radio": [
        ...
        { q: "Who demonstrated sending Bitcoin via ham radio in 2019?", a: "Rodolfo Novak co-founder of Coinkite", wrong: ["Elon Musk, as specified in the original Bitcoin whitepaper released in 2008", "Jack Dorsey, as specified in the original Bitcoin whitepaper released in 2008", "Andreas Antonopoulos, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "lightning_node": [
        { q: "Running a Lightning node allows you to:", a: "Route payments and earn fees while supporting the network", wrong: ["Mine Bitcoin faster, as specified in the original Bitcoin whitepaper released in 2008", "Create new Bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "Access the dark web, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is a Lightning node?", a: "Software that routes payments on the Lightning Network using payment channels", wrong: ["A physical Bitcoin ATM, as specified in the original Bitcoin whitepaper released in 2008", "A mining rig, as specified in the original Bitcoin whitepaper released in 2008", "A Bitcoin exchange server, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What do node operators lock up to open payment channels?", a: "Bitcoin as liquidity for routing payments", wrong: ["Ethereum tokens, as specified in the original Bitcoin whitepaper released in 2008", "Fiat currency, as specified in the original Bitcoin whitepaper released in 2008", "Computing power, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What fee do Lightning routing nodes typically charge?", a: "Routing fees measured in milli-satoshis (very small fraction of a cent)", wrong: ["1% of transaction amount, as specified in the original Bitcoin whitepaper released in 2008", "$5 per payment, as specified in the original Bitcoin whitepaper released in 2008", "No fees allowed, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is inbound liquidity for a Lightning node?", a: "The ability to receive payments based on funds others have locked with you", wrong: ["Money coming from bank deposits, as specified in the original Bitcoin whitepaper released in 2008", "Mining rewards, as specified in the original Bitcoin whitepaper released in 2008", "Exchange trading profits, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "stablecoins": [
        { q: "Stablecoins in the Bitcoin ecosystem are typically:", a: "Tokens pegged to fiat currency values, sometimes built on Bitcoin layers", wrong: ["A type of mining reward, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoins that never change price, as specified in the original Bitcoin whitepaper released in 2008", "Government-issued digital dollars, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What backs the majority of USDT (Tether)?", a: "Commercial paper, cash equivalents, and other reserves (historically controversial)", wrong: ["100% US dollars in bank, as specified in the original Bitcoin whitepaper released in 2008", "Gold reserves, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin collateral, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What major risk do centralized stablecoins pose?", a: "Issuer can freeze funds or fail to maintain peg", wrong: ["They mine too slowly, as specified in the original Bitcoin whitepaper released in 2008", "High transaction fees, as specified in the original Bitcoin whitepaper released in 2008", "Private key loss, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What caused USDC to depeg temporarily in March 2023?", a: "Exposure to failed Silicon Valley Bank", wrong: ["Bitcoin price crash, as specified in the original Bitcoin whitepaper released in 2008", "Lightning Network bug, as specified in the original Bitcoin whitepaper released in 2008", "Satoshi selling coins, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Stablecoins are NOT a threat to Bitcoin because:", a: "They are centralized IOUs that can be frozen, while Bitcoin is permissionless", wrong: ["Stablecoins use the same technology as Bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin automatically converts to stablecoins during crashes", "Stablecoins have a higher market cap than Bitcoin, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "consensus": [
        { q: "Bitcoin consensus means:", a: "All nodes agree on the state of the blockchain without a central authority", wrong: ["Everyone votes on transactions, as specified in the original Bitcoin whitepaper released in 2008", "Miners decide which transactions are valid alone, as specified in the original Bitcoin whitepaper released in 2008", "The government approves each block, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is Nakamoto Consensus?", a: "The combination of proof-of-work, longest chain rule, and economic incentives", wrong: ["Voting by coin holders, as specified in the original Bitcoin whitepaper released in 2008", "Consensus by the Bitcoin Foundation, as specified in the original Bitcoin whitepaper released in 2008", "Agreement between exchanges, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Why does Bitcoin use the longest chain as the valid chain?", a: "Most proof-of-work indicates most energy/computational investment", wrong: ["Random selection, as specified in the original Bitcoin whitepaper released in 2008", "Shortest chains are invalid, as specified in the original Bitcoin whitepaper released in 2008", "User voting, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "What happens if two miners find a valid block at nearly the same time?", a: "A temporary fork occurs and resolves when the next block is found", wrong: ["Both blocks are permanently added to the chain", "The network shuts down until the conflict is resolved", "The older miner's block always wins, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What does finality mean in Nakamoto Consensus?", a: "Reversals become exponentially harder with each confirmation", wrong: ["Transactions are instantly and permanently irreversible as defined by the Bitcoin protocol consensus rules", "Miners hold a formal vote on transaction validity as defined by the Bitcoin protocol consensus rules", "The longest chain is always guaranteed to be correct as defined by the Bitcoin protocol consensus rules"] },
    ...
    "open_source": [
        { q: "Bitcoin being open source means:", a: "Anyone can read, audit, and propose changes to the code", wrong: ["The code is secret but free to use, as specified in the original Bitcoin whitepaper released in 2008", "Only approved developers can view it, as specified in the original Bitcoin whitepaper released in 2008", "It costs nothing to mine, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "How can anyone contribute to Bitcoin Core?", a: "Submit pull requests on GitHub after code review", wrong: ["Pay a developer fee, as specified in the original Bitcoin whitepaper released in 2008", "Get hired by a bank, as specified in the original Bitcoin whitepaper released in 2008", "Apply for a license, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is the purpose of code review in Bitcoin development?", a: "To catch bugs and security issues before deployment", wrong: ["To slow down development, as specified in the original Bitcoin whitepaper released in 2008", "To charge developers money, as specified in the original Bitcoin whitepaper released in 2008", "To keep the code secret, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Why is peer review critical in Bitcoin Core development?", a: "It catches bugs and vulnerabilities before they reach production, since changes affect billions in value", wrong: ["It is just a formality required by the MIT license, as specified in the original Bitcoin whitepaper released in 2008", "Only one reviewer is needed for minor patches, as specified in the original Bitcoin whitepaper released in 2008", "Peer review slows down development unnecessarily, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "coin_mixing_coinjoin_coin_control_utxo": [
        { q: "CoinJoin improves privacy by:", a: "Combining multiple users' transactions so individual spending is hard to trace", wrong: ["Encrypting the blockchain, as specified in the original Bitcoin whitepaper released in 2008", "Deleting transaction history, as specified in the original Bitcoin whitepaper released in 2008", "Creating fake transactions, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is CoinJoin?", a: "A privacy technique combining multiple users\\' transactions to break heuristics", wrong: ["A cryptocurrency mixer token, as specified in the original Bitcoin whitepaper released in 2008", "A type of hardware wallet, as specified in the original Bitcoin whitepaper released in 2008", "An exchange withdrawal method, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What does coin control allow Bitcoin users to do?", a: "Manually select which UTXOs to spend in a transaction", wrong: ["Control the price of bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "Limit mining difficulty, as specified in the original Bitcoin whitepaper released in 2008", "Set transaction speed, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What privacy benefit does proper UTXO management provide?", a: "Preventing address clustering and transaction graph analysis", wrong: ["Increasing mining rewards, as specified in the original Bitcoin whitepaper released in 2008", "Reducing transaction fees, as specified in the original Bitcoin whitepaper released in 2008", "Speeding up confirmations, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is \"coin control\" in a Bitcoin wallet?", a: "The ability to manually select which UTXOs to spend in a transaction for better privacy", wrong: ["A government program to regulate Bitcoin spending, as specified in the original Bitcoin whitepaper released in 2008", "Software that limits how much Bitcoin you can buy, as specified in the original Bitcoin whitepaper released in 2008", "A mining feature that controls block production, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "environment___energy": [
        { q: "Bitcoin mining's relationship with renewable energy is:", a: "Miners actively seek cheap renewable and stranded energy, incentivizing green energy development", wrong: ["Mining only uses coal, as specified in the original Bitcoin whitepaper released in 2008", "Renewable energy cannot power mining, as specified in the original Bitcoin whitepaper released in 2008", "Mining has no relationship with energy markets, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What percentage of Bitcoin mining comes from renewable energy (estimates)?", a: "Estimates range from 50-60% using renewable or stranded energy", wrong: ["0% - all coal, as specified in the original Bitcoin whitepaper released in 2008", "100% renewable, as specified in the original Bitcoin whitepaper released in 2008", "Less than 10%, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is \"stranded energy\" in Bitcoin mining?", a: "Energy that would otherwise go to waste due to lack of transmission infrastructure", wrong: ["Energy from broken solar panels, as specified in the original Bitcoin whitepaper released in 2008", "Leftover battery power, as specified in the original Bitcoin whitepaper released in 2008", "Natural gas used for heating, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "How does Bitcoin mining help stabilize electrical grids?", a: "Miners can rapidly reduce load during peak demand (demand response)", wrong: ["By storing electricity in batteries, as specified in the original Bitcoin whitepaper released in 2008", "By donating profits to utilities, as specified in the original Bitcoin whitepaper released in 2008", "By building more coal plants, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What common criticism about Bitcoin energy use is often misrepresented?", a: "Per-transaction energy cost (Bitcoin uses same energy regardless of transaction count)", wrong: ["It uses no energy at all, as specified in the original Bitcoin whitepaper released in 2008", "All energy comes from coal, as specified in the original Bitcoin whitepaper released in 2008", "Mining makes computers explode, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "austrian_school_of_economics": [
        { q: "The Austrian School of Economics relates to Bitcoin because:", a: "It advocates for sound money with limited supply, which Bitcoin embodies", wrong: ["It was founded by Satoshi Nakamoto, as specified in the original Bitcoin whitepaper released in 2008", "It requires government-controlled currency, as specified in the original Bitcoin whitepaper released in 2008", "It predicts Bitcoin will fail, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Which Austrian economist emphasized the importance of sound money and criticized fiat?", a: "Ludwig von Mises and Friedrich Hayek", wrong: ["John Maynard Keynes, as specified in the original Bitcoin whitepaper released in 2008", "Karl Marx, as specified in the original Bitcoin whitepaper released in 2008", "Milton Friedman, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "The Austrian School of Economics emphasizes:", a: "Sound money, free markets, and the dangers of central banking", wrong: ["Government control of all monetary systems, as specified in the original Bitcoin whitepaper released in 2008", "That inflation is beneficial for economic growth", "That central banks should print unlimited money"] },
    ...
    "lindy_effect": [
        ...
        { q: "What concept is closely related to Lindy Effect in Bitcoin?", a: "Antifragility - stress makes the system stronger", wrong: ["Inflation targeting, as specified in the original Bitcoin whitepaper released in 2008", "Proof of stake, as specified in the original Bitcoin whitepaper released in 2008", "Central banking, as specified in the original Bitcoin whitepaper released in 2008"] },
    ...
    "softwar": [
        { q: "Jason Lowery's Softwar thesis argues that:", a: "Proof-of-work is a form of digital power projection analogous to military power in the physical world", wrong: ["Bitcoin is a weapon system, as specified in the original Bitcoin whitepaper released in 2008", "Software replaces all hardware, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin was created by the military, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Who authored the \"Softwar\" thesis about Bitcoin?", a: "Major Jason Lowery of the US Space Force", wrong: ["Michael Saylor, as specified in the original Bitcoin whitepaper released in 2008", "Nick Szabo, as specified in the original Bitcoin whitepaper released in 2008", "Elizabeth Warren, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is the core argument of the Softwar thesis?", a: "Proof-of-work is a form of kinetic power projection for cyberspace", wrong: ["Bitcoin should be banned, as specified in the original Bitcoin whitepaper released in 2008", "Proof of stake is superior, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin is a company, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What does Softwar compare Bitcoin\\'s hash rate to?", a: "Physical military power projection and deterrence", wrong: ["Social media likes, as specified in the original Bitcoin whitepaper released in 2008", "Stock market volume, as specified in the original Bitcoin whitepaper released in 2008", "Bank transaction counts, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "How does the Softwar thesis view Bitcoin mining in terms of national security?", a: "As a strategic asset that projects power in cyberspace, similar to how militaries project power in physical space", wrong: ["As a threat to national security that should be banned, as specified in the original Bitcoin whitepaper released in 2008", "As purely an economic activity with no geopolitical significance, as specified in the original Bitcoin whitepaper released in 2008", "As a replacement for nuclear deterrence, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "What energy concept is central to the Softwar thesis?", a: "Using energy to secure cyberspace property", wrong: ["Reducing global carbon emissions as defined by the Bitcoin protocol consensus rules", "Accelerating solar power adoption as defined by the Bitcoin protocol consensus rules", "Profiting from energy arbitrage as defined by the Bitcoin protocol consensus rules"] }
    ],
    "sidechains": [
        { q: "A Bitcoin sidechain is:", a: "A separate blockchain that is pegged to Bitcoin, enabling additional features while settling back to the main chain", wrong: ["A backup copy of the Bitcoin blockchain, as specified in the original Bitcoin whitepaper released in 2008 as defined by the Bitcoin protocol consensus rules", "A faster version of Bitcoin Core, as specified in the original Bitcoin whitepaper released in 2008 as defined by the Bitcoin protocol consensus rules", "An altcoin that replaced Bitcoin, as specified in the original Bitcoin whitepaper released in 2008 as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is the Liquid Network?", a: "A federated sidechain enabling faster confidential Bitcoin transactions", wrong: ["An Ethereum scaling solution, as specified in the original Bitcoin whitepaper released in 2008", "A mining pool, as specified in the original Bitcoin whitepaper released in 2008", "A hardware wallet, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is RSK (Rootstock)?", a: "A Bitcoin sidechain supporting smart contracts with merged mining", wrong: ["A Russian Bitcoin exchange, as specified in the original Bitcoin whitepaper released in 2008", "A hardware wallet brand, as specified in the original Bitcoin whitepaper released in 2008", "A Lightning wallet, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "How are assets secured when moving to a sidechain?", a: "Through a federated peg locking bitcoin on mainchain", wrong: ["By burning bitcoin permanently, as specified in the original Bitcoin whitepaper released in 2008", "By trusting a single company, as specified in the original Bitcoin whitepaper released in 2008", "No security is provided, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is a downside of federated sidechains compared to Bitcoin mainchain?", a: "Less decentralization due to reliance on federation members", wrong: ["Higher transaction fees, as specified in the original Bitcoin whitepaper released in 2008", "Slower block times, as specified in the original Bitcoin whitepaper released in 2008", "No programming capabilities, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "submarine_swap": [
        ...
        { q: "Why is it called a \"submarine\" swap?", a: "On-chain is \"above water\" and Lightning off-chain is \"below water\" - you swap between the two", wrong: ["The swap conceals funds below the blockchain visible layer before resurfacing on-chain across all active payment channels on the network", "On-chain funds are submerged in a time-locked escrow vault beneath the mempool layer across all active payment channels on the network", "The name comes from the HTLC diving beneath regular channel capacity during the swap across all active payment channels on the network"] },
        { q: "What enables submarine swaps to be trustless?", a: "HTLCs (Hash Time Locked Contracts)", wrong: ["Centralized exchange custody", "Bank wire confirmation, as specified in the original Bitcoin whitepaper released in 2008", "Social media verification"] },
        ...
        { q: "What is a Loop Out in Lightning?", a: "Moving funds from Lightning back to on-chain", wrong: ["Opening a new payment channel, as specified in the original Bitcoin whitepaper released in 2008", "Receiving inbound Lightning payments", "Closing all channels at once, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is Boltz Exchange known for?", a: "Trustless on-chain and Lightning swaps", wrong: ["Running a Bitcoin mining pool across all active payment channels on the network", "Operating a centralized exchange across all active payment channels on the network", "Selling Bitcoin hardware wallets across all active payment channels on the network"] },
        ...
        { q: "How do submarine swaps differ from atomic swaps?", a: "They move between layers of the same chain", wrong: ["Submarine swaps always require trust as defined by the Bitcoin protocol consensus rules", "Atomic swaps are significantly faster as defined by the Bitcoin protocol consensus rules", "Submarine swaps use different coins as defined by the Bitcoin protocol consensus rules"] },
        { q: "What enables cross-chain atomic swaps without intermediaries?", a: "HTLCs combining hashlock and timelock", wrong: ["Trusted custodial escrow services as defined by the Bitcoin protocol consensus rules", "Centralized exchange order books as defined by the Bitcoin protocol consensus rules", "Smart contract platforms only as defined by the Bitcoin protocol consensus rules"] }
    ],
    "supranational": [
        { q: "What does \"supranational\" mean in the context of Bitcoin?", a: "It transcends national borders and governments", wrong: ["It is backed by the United Nations as defined by the Bitcoin protocol consensus rules", "It requires international approval to use as defined by the Bitcoin protocol consensus rules", "It can only be used between countries as defined by the Bitcoin protocol consensus rules"] },
        { q: "What does it mean that Bitcoin is \"supranational\" money?", a: "It operates beyond any single nation's jurisdiction or control", wrong: ["It is owned by the United Nations, as specified in the original Bitcoin whitepaper released in 2008", "Only governments can use it, as specified in the original Bitcoin whitepaper released in 2008", "It requires international banking licenses, as specified in the original Bitcoin whitepaper released in 2008"] },
    ...
    "geopolitics___macroeconomics": [
        ...
        { q: "Which country was the first to adopt Bitcoin as legal tender?", a: "El Salvador in September 2021", wrong: ["The United States in early 2022", "Japan in November 2020", "Germany in December 2019"] },
    ...
    "public_key_vs_private_key": [
        { q: "Your Bitcoin public key is like:", a: "Your email address - you share it so people can send you Bitcoin", wrong: ["Your password - never share it with anyone, as specified in the original Bitcoin whitepaper released in 2008", "Your bank PIN number - used to authorize spending", "Your social security number - it proves your identity"] },
        { q: "If someone has your private key, they can:", a: "Spend all the Bitcoin controlled by that key", wrong: ["Only view your transaction history stored in the wallet keychain database encrypted locally", "Reset your password and lock you out stored in the wallet keychain database encrypted locally", "Create new Bitcoin out of thin air stored in the wallet keychain database encrypted locally"] },
        { q: "What is the relationship between private and public keys?", a: "Public keys are derived from private keys via one-way cryptographic function", wrong: ["Private keys are given to everyone, as specified in the original Bitcoin whitepaper released in 2008", "Public keys encrypt private keys, as specified in the original Bitcoin whitepaper released in 2008", "They are identical numbers, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What should you do with your private keys?", a: "Keep them secret and secure, never share with anyone", wrong: ["Publish them online, as specified in the original Bitcoin whitepaper released in 2008", "Email them to exchanges, as specified in the original Bitcoin whitepaper released in 2008", "Print them on merchandise, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is the purpose of a public key?", a: "To receive bitcoin and verify signatures without revealing the private key", wrong: ["To spend bitcoin alone, as specified in the original Bitcoin whitepaper released in 2008", "To mine new blocks, as specified in the original Bitcoin whitepaper released in 2008", "To create private keys, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "transaction_fees": [
        { q: "Bitcoin transaction fees are determined by:", a: "The size of the transaction in bytes and current network demand", wrong: ["A fixed percentage of the transaction amount, as specified in the original Bitcoin whitepaper released in 2008", "The number of Bitcoin being sent, as specified in the original Bitcoin whitepaper released in 2008", "The geographic distance between sender and receiver"] },
        { q: "When the mempool is full:", a: "Transactions with higher fees get confirmed first", wrong: ["All transactions are rejected until it clears as defined by the Bitcoin protocol consensus rules", "The block size automatically increases as defined by the Bitcoin protocol consensus rules", "Transaction fees are refunded to senders as defined by the Bitcoin protocol consensus rules"] },
        { q: "What determines Bitcoin transaction fees?", a: "Transaction size in bytes and current network demand (mempool congestion)", wrong: ["Transaction amount in bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "Sender\\'s reputation, as specified in the original Bitcoin whitepaper released in 2008", "Government regulation, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is the mempool?", a: "A waiting area for unconfirmed transactions held by nodes", wrong: ["A mining hardware pool, as specified in the original Bitcoin whitepaper released in 2008", "A cryptocurrency exchange, as specified in the original Bitcoin whitepaper released in 2008", "A type of Bitcoin wallet, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "Why do fees spike during network congestion?", a: "Users compete for limited block space by bidding higher fees", wrong: ["Miners arbitrarily raise prices, as specified in the original Bitcoin whitepaper released in 2008", "Exchanges charge extra, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin supply decreases, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "philosophy": [
        { q: "\"Don't trust, verify\" means:", a: "Run your own node to independently verify all Bitcoin rules", wrong: ["Never use Bitcoin because it cannot be trusted verified independently by every full node in the network", "Only trust exchanges that are government-regulated verified independently by every full node in the network", "Verify your identity before making transactions verified independently by every full node in the network"] },
        { q: "The concept \"Bitcoin is Time\" by Gigi suggests:", a: "Bitcoin creates a decentralized clock through proof of work", wrong: ["Bitcoin transactions can travel back in time", "Bitcoin was invented to save people time, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin mining uses atomic clocks, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What is the cypherpunk movement that birthed Bitcoin?", a: "Advocates for privacy through cryptography and code over trust", wrong: ["Bankers wanting digital currency, as specified in the original Bitcoin whitepaper released in 2008", "Gamers creating virtual money, as specified in the original Bitcoin whitepaper released in 2008", "Governments tracking transactions, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What does it mean that Bitcoin had a \"fair launch\"?", a: "No pre-mine, no ICO - anyone could mine from day one on equal terms", wrong: ["The government approved it before launch, as specified in the original Bitcoin whitepaper released in 2008", "Satoshi distributed coins equally to all users, as specified in the original Bitcoin whitepaper released in 2008", "A committee reviewed and certified the genesis block"] },
        { q: "What does Bitcoin\\'s \"monetary policy\" refer to?", a: "Fixed supply schedule with predictable issuance rate until 21 million", wrong: ["Central bank interest rates, as specified in the original Bitcoin whitepaper released in 2008", "Government fiscal policy, as specified in the original Bitcoin whitepaper released in 2008", "Investment fund strategies, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "What did Wei Dai propose with b-money?", a: "Anonymous digital cash with decentralized minting", wrong: ["A blockchain with standard mining rewards as defined by the Bitcoin protocol consensus rules", "A central bank controlled digital currency as defined by the Bitcoin protocol consensus rules", "A system of gold-backed certificates as defined by the Bitcoin protocol consensus rules"] },
        { q: "What was Hal Finney's famous contribution to Bitcoin?", a: "Received the first Bitcoin transaction from Satoshi", wrong: ["Wrote the original Bitcoin whitepaper himself as defined by the Bitcoin protocol consensus rules", "Created the SHA-256 based mining algorithm as defined by the Bitcoin protocol consensus rules", "Founded the first Bitcoin trading exchange as defined by the Bitcoin protocol consensus rules"] }
    ],
    "halving": [
        { q: "The Bitcoin halving occurs every:", a: "210,000 blocks (approximately every 4 years)", wrong: ["Every calendar year on January 3", "Every 100,000 blocks, as specified in the original Bitcoin whitepaper released in 2008", "Whenever the price doubles, as specified in the original Bitcoin whitepaper released in 2008"] },
        ...
        { q: "The halving is significant because it:", a: "Mathematically enforces Bitcoin's decreasing supply issuance", wrong: ["Doubles the total supply of Bitcoin, as specified in the original Bitcoin whitepaper released in 2008", "Reduces the number of active miners by half", "Changes Bitcoin's consensus mechanism, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "What happens during a Bitcoin halving?", a: "Block reward paid to miners is cut in half", wrong: ["Transaction fees double, as specified in the original Bitcoin whitepaper released in 2008", "Bitcoin supply doubles, as specified in the original Bitcoin whitepaper released in 2008", "Mining difficulty halves, as specified in the original Bitcoin whitepaper released in 2008"] },
        { q: "When will the final bitcoin be mined?", a: "Around year 2140 when all 21 million are issued", wrong: ["2025, as specified in the original Bitcoin whitepaper released in 2008", "2040, as specified in the original Bitcoin whitepaper released in 2008", "Never - mining continues forever, as specified in the original Bitcoin whitepaper released in 2008"] }
    ],
    "bitcoin_vs_real_estate": [
        ...
        { q: "How does Bitcoin's geographic risk profile differ from that of real estate?", a: "Real estate values depend entirely on local economies and geography; Bitcoin is a global asset uncorrelated with any specific regional market or jurisdiction", wrong: ["Bitcoin and real estate are highly correlated hard assets that respond identically to global CPI inflation data and Federal Reserve policy changes as defined by the Bitcoin protocol consensus rules", "Bitcoin carries more geographic concentration risk than real estate because the majority of mining activity occurs in just two or three countries as defined by the Bitcoin protocol consensus rules", "Major global-city real estate is uncorrelated with local economic conditions and behaves identically to fully global assets like commodity futures as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "What happened to real estate values during hyperinflation in countries like Venezuela, Zimbabwe, and Lebanon?", a: "Real estate also lost value in hard money terms during hyperinflation across Venezuela, Zimbabwe, and Lebanon; Bitcoin preserved purchasing power for those who held it", wrong: ["Real estate proved the most reliable inflation hedge in both Venezuela and Zimbabwe, outperforming gold, foreign currency, and all other asset classes as defined by the Bitcoin protocol consensus rules", "Real estate retained its full value in all documented hyperinflationary environments since physical property cannot be devalued by money printing alone as defined by the Bitcoin protocol consensus rules", "Real estate outperformed Bitcoin in hyperinflationary countries because local governments shut down cryptocurrency exchanges and banned digital asset transactions as defined by the Bitcoin protocol consensus rules"] }
    ],
    "bitvm": [
        ...
        { q: "BitVM uses which computational model to verify off-chain computation?", a: "An optimistic model with prover claims and verifier challenges", wrong: ["A zero-knowledge model where proofs are posted each block as defined by the Bitcoin protocol consensus rules", "A consensus model where all nodes re-execute computation as defined by the Bitcoin protocol consensus rules", "A federated model where trusted signers attest to results as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "How does BitVM2 improve upon the original BitVM design?", a: "It reduces on-chain transactions needed for dispute resolution", wrong: ["It adds Turing-complete execution directly on base layer as defined by the Bitcoin protocol consensus rules", "It eliminates the need for a verifier role entirely as defined by the Bitcoin protocol consensus rules", "It requires a soft fork to enable several new opcodes as defined by the Bitcoin protocol consensus rules"] }
    ],
    "block_time-block-size": [
        ...
        { q: "What is the difference between block weight and block size in Bitcoin?", a: "Weight counts witness bytes at a discount versus non-witness bytes", wrong: ["Block weight and block size are identical with different names as defined by the Bitcoin protocol consensus rules", "Block weight measures computational effort to validate a block as defined by the Bitcoin protocol consensus rules", "Block weight only applies to Lightning Network transactions as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "Why does SegWit give witness data a discount in the block weight calculation?", a: "Witness data costs less for nodes to process, incentivizing adoption", wrong: ["Witness data is encrypted and takes up less physical storage verified independently by every full node in the network", "The discount was a political compromise with no technical basis verified independently by every full node in the network", "Witness data is stored temporarily then deleted after 100 blocks verified independently by every full node in the network"] },
    ...
    "burn_bitcoin": [
        ...
        { q: "What type of Bitcoin address output is provably unspendable with no script?", a: "Bare multisig with invalid pubkeys", wrong: ["Standard P2PKH addresses", "Wrapped P2SH addresses, as specified in the original Bitcoin whitepaper released in 2008", "Native P2WPKH addresses, as specified in the original Bitcoin whitepaper released in 2008"] },
    ...
    "ctv-covenants": [
        { q: "What does BIP-119 (CheckTemplateVerify) allow a Bitcoin output to do?", a: "Commit to a specific future transaction template for spending", wrong: ["Execute arbitrary smart contract code when it is spent as defined by the Bitcoin protocol consensus rules", "Lock funds until a specific block height has been reached as defined by the Bitcoin protocol consensus rules", "Require multiple signatures from different keys to spend as defined by the Bitcoin protocol consensus rules"] },
    ...
    "cycles": [
        ...
        { q: "How does the Gartner Hype Cycle apply to each Bitcoin halving era?", a: "Each cycle shows trigger, peak hype, disillusionment, then recovery to a plateau", wrong: ["Bitcoin skips disillusionment because the price only goes up long-term as defined by the Bitcoin protocol consensus rules", "The Gartner Hype Cycle only applies to new tech products, not currencies as defined by the Bitcoin protocol consensus rules", "Each Bitcoin cycle follows the exact same price pattern as the last one as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is the \"lengthening cycles\" debate in Bitcoin?", a: "The theory that each bull run takes longer to peak with smaller percentage gains", wrong: ["The theory that mining cycles get shorter as hardware gets faster as defined by the Bitcoin protocol consensus rules", "A proposal to extend the halving interval from four years to eight as defined by the Bitcoin protocol consensus rules", "The idea that transaction confirmation times increase every year as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "What is the \"diminishing returns\" theory in Bitcoin cycles?", a: "Each cycle's percentage gain shrinks, though absolute highs keep rising", wrong: ["Returns are accelerating each cycle with larger percentage gains as defined by the Bitcoin protocol consensus rules", "Bitcoin will eventually produce negative returns in every cycle as defined by the Bitcoin protocol consensus rules", "It refers to decreasing energy efficiency of mining each cycle as defined by the Bitcoin protocol consensus rules"] }
    ],
    "derivation_path": [
        ...
        { q: "What is the \"gap limit\" in HD wallet address discovery?", a: "How many consecutive unused addresses are checked before stopping", wrong: ["The maximum number of addresses a wallet can generate total as defined by the Bitcoin protocol consensus rules", "The time delay required between generating new addresses as defined by the Bitcoin protocol consensus rules", "The minimum spacing between two UTXOs in a transaction as defined by the Bitcoin protocol consensus rules"] }
    ],
    "dlcs": [
        ...
        { q: "DLC.Link enables Discreet Log Contracts to be used primarily for what purpose?", a: "Bridging Bitcoin as collateral into DeFi without giving up custody", wrong: ["Mining pool payout distribution based on individual hash rate as defined by the Bitcoin protocol consensus rules", "Decentralized DNS registration on the Bitcoin base layer as defined by the Bitcoin protocol consensus rules", "Cross-chain atomic swaps between Bitcoin and Ethereum directly as defined by the Bitcoin protocol consensus rules"] }
    ],
    "dollar-bitcoin_milkshake_theory": [
        ...
        { q: "What is the core thesis of the Dollar Milkshake Theory?", a: "The dollar strengthens by sucking up global liquidity while other fiat currencies weaken", wrong: ["All fiat currencies collapse simultaneously and get replaced by Bitcoin as defined by the Bitcoin protocol consensus rules", "The dollar weakens as other currencies collectively strengthen against it as defined by the Bitcoin protocol consensus rules", "Central banks coordinate to create and adopt a single global currency as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is the \"dollar strength paradox\" in the Milkshake Theory?", a: "The dollar gets stronger vs other currencies while losing purchasing power vs hard assets", wrong: ["A strong dollar always means the Bitcoin price will go down in tandem as defined by the Bitcoin protocol consensus rules", "Dollar strength is a reliable signal that the US economy is healthy as defined by the Bitcoin protocol consensus rules", "Nobody wants dollars but everyone needs them to service their debts as defined by the Bitcoin protocol consensus rules"] },
        { q: "How does Bitcoin fit into the Dollar Milkshake Theory's endgame?", a: "After the dollar absorbs global liquidity, Bitcoin becomes the ultimate fiat exit", wrong: ["Bitcoin is irrelevant to the theory because it isn't a fiat currency as defined by the Bitcoin protocol consensus rules", "Bitcoin collapses alongside other currencies during the dollar's rise as defined by the Bitcoin protocol consensus rules", "The theory predicts Bitcoin gets banned and replaced by state CBDCs as defined by the Bitcoin protocol consensus rules"] },
        { q: "Why does global dollar demand play a central role in the Milkshake Theory?", a: "Trillions in dollar-denominated debt forces nations to buy dollars, strengthening it", wrong: ["Global dollar demand is declining rapidly as nations actively de-dollarize as defined by the Bitcoin protocol consensus rules", "Dollar demand only matters for US domestic transactions and trade as defined by the Bitcoin protocol consensus rules", "The theory says dollar demand is irrelevant - only money supply matters as defined by the Bitcoin protocol consensus rules"] }
    ],
    "el-salvador": [
        { q: "What was the Chivo wallet in El Salvador's Bitcoin adoption?", a: "A government-issued Lightning wallet that gave citizens $30 in Bitcoin", wrong: ["A private hardware wallet manufactured locally in El Salvador across all active payment channels on the network", "An exchange platform where Salvadorans could trade altcoins across all active payment channels on the network", "A banking app that only supported US dollar-based transactions across all active payment channels on the network"] },
        { q: "What is \"Bitcoin Beach\" and why is it significant to El Salvador's Bitcoin story?", a: "El Zonte - a village that built a grassroots Bitcoin economy before the national law", wrong: ["A luxury resort in San Salvador that exclusively accepts Bitcoin payments as defined by the Bitcoin protocol consensus rules", "A government-built tourist attraction designed to promote Bitcoin adoption as defined by the Bitcoin protocol consensus rules", "A cryptocurrency exchange headquartered on the Salvadoran coastal region as defined by the Bitcoin protocol consensus rules"] },
        { q: "What was President Bukele's core strategy behind making Bitcoin legal tender?", a: "Financial inclusion for the unbanked, foreign investment, and cheaper remittances", wrong: ["Replacing the US dollar entirely with Bitcoin as the sole currency as defined by the Bitcoin protocol consensus rules", "Mining Bitcoin profitably using El Salvador's untapped oil reserves as defined by the Bitcoin protocol consensus rules", "Creating a government-controlled cryptocurrency to replace Bitcoin as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "How did Lightning Network adoption benefit El Salvador's economy?", a: "It enabled fast, near-free payments and remittances, cutting out costly services", wrong: ["Lightning was not used - only on-chain Bitcoin transactions were supported across all active payment channels on the network", "Lightning made transactions faster but significantly more expensive overall across all active payment channels on the network", "El Salvador banned Lightning in favor of on-chain-only Bitcoin payments across all active payment channels on the network"] }
    ],
    "extension-blocks": [
    ...
    "feedback_loops": [
        { q: "What is the price-hashrate-security feedback loop in Bitcoin?", a: "Higher price attracts miners, boosting hashrate, security, and confidence in a rising cycle", wrong: ["Higher price causes miners to shut down, reducing hashrate and network security as defined by the Bitcoin protocol consensus rules", "Hashrate and price are completely independent with no meaningful connection as defined by the Bitcoin protocol consensus rules", "Security drops as price rises because more hackers are attracted to attack as defined by the Bitcoin protocol consensus rules"] },
        { q: "How does the adoption-liquidity-utility feedback loop work in Bitcoin?", a: "More users deepen liquidity, making Bitcoin more useful, which attracts even more users", wrong: ["More users raise fees, which drives away users and decreases overall liquidity as defined by the Bitcoin protocol consensus rules", "Liquidity falls as adoption grows because the total supply is permanently fixed as defined by the Bitcoin protocol consensus rules", "Adoption and liquidity have an inverse relationship in the Bitcoin network as defined by the Bitcoin protocol consensus rules"] },
        { q: "How does the Lindy Effect create a reinforcing feedback loop for Bitcoin?", a: "Each day Bitcoin survives boosts confidence in its longevity, drawing more adoption", wrong: ["The Lindy Effect means Bitcoin gets less reliable as its codebase ages as defined by the Bitcoin protocol consensus rules", "Older technology always gets replaced, so the Lindy Effect hurts Bitcoin as defined by the Bitcoin protocol consensus rules", "The Lindy Effect only applies to physical objects, not digital protocols as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is reflexivity in the context of Bitcoin market cycles?", a: "Perceptions influence fundamentals, which reinforce perceptions, creating self-amplifying cycles", wrong: ["Reflexivity means Bitcoin's price always reflects its true fundamental value as defined by the Bitcoin protocol consensus rules", "It describes the tendency of Bitcoin to always revert to the same price as defined by the Bitcoin protocol consensus rules", "Reflexivity is a mining technique that increases block production speed as defined by the Bitcoin protocol consensus rules"] },
        { q: "Why are Bitcoin's feedback loops considered among the most powerful in economics?", a: "Multiple self-reinforcing loops in price, hashrate, adoption, and security amplify together", wrong: ["Bitcoin has only one feedback loop but it happens to be extremely strong as defined by the Bitcoin protocol consensus rules", "Bitcoin's feedback loops are actually weak compared to traditional market loops as defined by the Bitcoin protocol consensus rules", "Feedback loops only work in bull markets and reverse completely in bear runs as defined by the Bitcoin protocol consensus rules"] }
    ],
    "first_principles": [
        { q: "What does \"first principles thinking\" mean when applied to Bitcoin?", a: "Breaking money down to fundamental properties and reasoning up from there", wrong: ["Copying the design of existing currencies and then improving them as defined by the Bitcoin protocol consensus rules", "Following the majority opinion of mainstream economists on money as defined by the Bitcoin protocol consensus rules", "Starting with government approval and working backward from there as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "Why does first principles reasoning tend to lead people toward Bitcoin rather than altcoins?", a: "Bitcoin best satisfies the fundamental properties of sound money from first principles", wrong: ["Bitcoin was the first cryptocurrency so it wins by default over altcoins as defined by the Bitcoin protocol consensus rules", "Altcoins are illegal in most countries so Bitcoin is the only legal option as defined by the Bitcoin protocol consensus rules", "Bitcoin has the best marketing team and strongest brand awareness overall as defined by the Bitcoin protocol consensus rules"] }
    ],
    "foss": [
        { q: "What type of software license does Bitcoin Core use?", a: "MIT License", wrong: ["GPL v3", "Apache License 2.0", "Creative Commons"] },
        { q: "Why is it important that Bitcoin Core is free and open-source software (FOSS)?", a: "Anyone can audit the code to verify no backdoors or hidden rules exist", wrong: ["It makes Bitcoin free to use with zero transaction fees for users as defined by the Bitcoin protocol consensus rules", "It allows developers to create unlimited Bitcoin through the code as defined by the Bitcoin protocol consensus rules", "It means no one actually needs to run the Bitcoin software as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "\"Linus's Law\" states that \"given enough eyeballs, all bugs are shallow.\" How does this apply to Bitcoin?", a: "Thousands of developers review Bitcoin's code, so vulnerabilities get found and fixed faster", wrong: ["Linux servers are the only machines capable of running Bitcoin full nodes properly as defined by the Bitcoin protocol consensus rules", "Linus Torvalds personally reviews and audits every Bitcoin Core release himself as defined by the Bitcoin protocol consensus rules", "Bitcoin can only run on open-source operating systems like Linux and FreeBSD as defined by the Bitcoin protocol consensus rules"] },
        { q: "Why does open-source matter specifically for money/monetary software like Bitcoin?", a: "Users must verify the rules of their money - no hidden inflation or secret changes", wrong: ["Open-source software is always faster and more efficient than proprietary code as defined by the Bitcoin protocol consensus rules", "It allows banks to freely copy Bitcoin and make their own branded version as defined by the Bitcoin protocol consensus rules", "It makes Bitcoin significantly easier for governments to regulate and control as defined by the Bitcoin protocol consensus rules"] }
    ],
    "hardware": [
        { q: "What makes the Coldcard hardware wallet stand out among Bitcoin hardware wallets?", a: "It's air-gapped, Bitcoin-only, and supports NFC signing", wrong: ["It supports thousands of altcoins and NFTs as defined by the Bitcoin protocol consensus rules", "It connects directly to the internet for speed as defined by the Bitcoin protocol consensus rules", "It's the cheapest hardware wallet available as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is a PSBT (Partially Signed Bitcoin Transaction) and why is it important for hardware wallets?", a: "A format for building transactions on one device and signing on another", wrong: ["A Bitcoin address type exclusive to hardware wallets", "A protocol for sending Bitcoin over Bluetooth, as specified in the original Bitcoin whitepaper released in 2008", "A backup format for storing seed phrases digitally"] },
        { q: "What is SeedSigner?", a: "A DIY, open-source, air-gapped wallet built on a Raspberry Pi", wrong: ["A cloud service that generates seed phrases for you as defined by the Bitcoin protocol consensus rules", "A proprietary hardware wallet made by Ledger as defined by the Bitcoin protocol consensus rules", "An app that signs transactions via phone fingerprint as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is a \"secure element\" chip in hardware wallets?", a: "A tamper-resistant chip that stores keys and blocks extraction", wrong: ["A chip that connects the wallet to Wi-Fi securely as defined by the Bitcoin protocol consensus rules", "A component that speeds up transaction processing as defined by the Bitcoin protocol consensus rules", "A GPS module that tracks the wallet for recovery as defined by the Bitcoin protocol consensus rules"] },
        { q: "What does \"air-gapped\" mean when describing a hardware wallet like Coldcard or Foundation Passport?", a: "It never connects to internet - data moves via SD card, QR, or NFC", wrong: ["The wallet is sealed in a vacuum to prevent tampering as defined by the Bitcoin protocol consensus rules", "The wallet uses satellite connections instead of Wi-Fi as defined by the Bitcoin protocol consensus rules", "The wallet can only be used outdoors for security as defined by the Bitcoin protocol consensus rules"] }
    ],
    "improved_incentive_structure": [
        { q: "How does Bitcoin \"fix broken incentives\" in the current monetary system?", a: "By removing money printing, it forces honest accounting and ends inflation of debt", wrong: ["It gives governments better tools to manage the economy, as specified in the original Bitcoin whitepaper released in 2008", "It doesn't change incentives - just moves them to digital, as specified in the original Bitcoin whitepaper released in 2008", "It fixes incentives by making all transactions government-traceable"] },
        { q: "What does \"low time preference society\" mean in the context of a Bitcoin standard?", a: "People plan long-term and save more because their money gains value over time", wrong: ["A society that doesn't care about time and works fewer hours as defined by the Bitcoin protocol consensus rules", "A society where transactions must complete within strict time limits as defined by the Bitcoin protocol consensus rules", "A society that only uses Bitcoin for short-term speculation as defined by the Bitcoin protocol consensus rules"] },
        { q: "Why is proof of work considered a \"fair distribution\" mechanism for new Bitcoin?", a: "Anyone can mine by spending real energy - no pre-mines or insider allocations", wrong: ["Proof of work gives equal Bitcoin to every person on Earth as defined by the Bitcoin protocol consensus rules", "Early miners got special privileges that later miners don't as defined by the Bitcoin protocol consensus rules", "Proof of work distributes Bitcoin based on geographic location as defined by the Bitcoin protocol consensus rules"] },
        { q: "How does sound money (like Bitcoin) change savings and investment behavior?", a: "People save more and only invest in projects with real returns above deflation", wrong: ["Sound money discourages saving because money becomes too valuable as defined by the Bitcoin protocol consensus rules", "Investment behavior stays the same regardless of monetary system as defined by the Bitcoin protocol consensus rules", "Sound money causes everyone to hoard and collapse the economy as defined by the Bitcoin protocol consensus rules"] },
        { q: "What broken incentive does fiat money create that Bitcoin eliminates?", a: "Fiat rewards borrowing over saving because inflation erodes purchasing power", wrong: ["Fiat incentivizes too much saving, which Bitcoin fixes as defined by the Bitcoin protocol consensus rules", "Fiat money has no broken incentives - it works as designed as defined by the Bitcoin protocol consensus rules", "Bitcoin creates the same borrow incentive since rates exist as defined by the Bitcoin protocol consensus rules"] }
    ],
    "laws_of_thermodynamics": [
        { q: "How does the First Law of Thermodynamics (conservation of energy) relate to Bitcoin mining?", a: "Miners convert electricity into computational work securing the ledger", wrong: ["Bitcoin mining creates energy from nothing to power the network for every block included in the longest valid chain", "The First Law means Bitcoin can only exist in cold climates for every block included in the longest valid chain", "Energy is destroyed during mining, explaining the high usage for every block included in the longest valid chain"] },
        { q: "What does \"unforgeable costliness\" mean in Bitcoin's thermodynamic context?", a: "Real energy must be spent to produce Bitcoin and this cost can't be faked", wrong: ["Bitcoin transactions are free because the network is decentralized as defined by the Bitcoin protocol consensus rules", "The cost of mining is purely artificial with no security purpose as defined by the Bitcoin protocol consensus rules", "Unforgeable costliness means Bitcoin wallets cannot be hacked as defined by the Bitcoin protocol consensus rules"] },
        { q: "How does the Second Law of Thermodynamics (entropy always increases) connect to Bitcoin?", a: "Proof of work creates order in the ledger by increasing entropy via energy use", wrong: ["Bitcoin violates the Second Law by creating value from nothing as defined by the Bitcoin protocol consensus rules", "Entropy has no connection to digital systems like Bitcoin as defined by the Bitcoin protocol consensus rules", "The Second Law means Bitcoin will eventually stop working as defined by the Bitcoin protocol consensus rules"] },
        { q: "Why do some physicists describe proof of work as \"thermodynamic security\"?", a: "Reversing the chain would require re-spending all the energy that built it", wrong: ["Because Bitcoin mining rigs get very hot during operation as defined by the Bitcoin protocol consensus rules", "Because proof of work requires cooling systems to function as defined by the Bitcoin protocol consensus rules", "Because thermodynamic equations are used to set transaction fees as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is the \"Maxwell's Demon\" analogy sometimes applied to Bitcoin?", a: "You can't sort molecules or secure a ledger without real energy expenditure", wrong: ["Maxwell's Demon is the name of the first Bitcoin mining software as defined by the Bitcoin protocol consensus rules", "It describes a theoretical AI attack that takes over the network as defined by the Bitcoin protocol consensus rules", "It refers to Bitcoin price randomness being like particle motion as defined by the Bitcoin protocol consensus rules"] }
    ],
    "layer-3-sidechains": [
        ...
        { q: "What are drivechains (BIP-300) designed to enable?", a: "Permissionless sidechains with miner-voted withdrawals to mainchain", wrong: ["Faster block times on Bitcoin by reducing difficulty adjustment as defined by the Bitcoin protocol consensus rules", "A new Lightning channel type that skips on-chain funding steps as defined by the Bitcoin protocol consensus rules", "Decentralized mining pools that auto-distribute block rewards as defined by the Bitcoin protocol consensus rules"] },
    ...
    "ordinals_inscriptions": [
        ...
        { q: "What do inscriptions attach to specific satoshis?", a: "Arbitrary data like images or text", wrong: ["Private keys for spending them as defined by the Bitcoin protocol consensus rules", "Their full transaction history as defined by the Bitcoin protocol consensus rules", "Multisig spending conditions as defined by the Bitcoin protocol consensus rules"] },
    ...
    "rollups": [
        { q: "What is a validity rollup on Bitcoin?", a: "A system that bundles off-chain transactions and posts a ZK proof to Bitcoin", wrong: ["A sidechain that copies Bitcoin's consensus rules exactly as written as defined by the Bitcoin protocol consensus rules", "A Lightning Network channel that batches together multiple payments as defined by the Bitcoin protocol consensus rules", "A mining pool feature that combines block templates from miners as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "What is the main trust assumption difference between a validity rollup and an optimistic rollup?", a: "Validity rollups prove correctness upfront; optimistic rollups assume it until challenged", wrong: ["Validity rollups trust a federation of signers; optimistic rollups trust miners as defined by the Bitcoin protocol consensus rules", "Validity rollups need a soft fork to work; optimistic rollups work on Bitcoin today as defined by the Bitcoin protocol consensus rules", "Validity rollups are run centrally; optimistic rollups are always fully decentralized as defined by the Bitcoin protocol consensus rules"] }
    ],
    "spv": [
        { q: "What does SPV (Simplified Payment Verification) allow a lightweight client to do?", a: "Verify transaction inclusion using block headers and Merkle proofs", wrong: ["Mine Bitcoin blocks using minimal computing power as defined by the Bitcoin protocol consensus rules", "Send transactions without paying any network fees as defined by the Bitcoin protocol consensus rules", "Run a full node on a mobile phone with limited storage as defined by the Bitcoin protocol consensus rules"] },
        { q: "What privacy problem did the original SPV bloom filters (BIP-37) have?", a: "They leaked which addresses the wallet cared about to the server", wrong: ["They revealed the user's IP address to every network node as defined by the Bitcoin protocol consensus rules", "They broadcast the user's seed phrase as part of filter data as defined by the Bitcoin protocol consensus rules", "They required users to share real names with node operators as defined by the Bitcoin protocol consensus rules"] },
        { q: "What do BIP-157 and BIP-158 (compact block filters) improve over bloom filters?", a: "Clients download pre-computed filters without revealing their addresses", wrong: ["They compress the entire blockchain into a single downloadable file as defined by the Bitcoin protocol consensus rules", "They allow nodes to verify transactions without any network access as defined by the Bitcoin protocol consensus rules", "They replace Merkle proofs with zero-knowledge proofs for speed as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is Neutrino in the context of Bitcoin light clients?", a: "A light client protocol using BIP-157/158 compact block filters", wrong: ["A privacy coin built as a Bitcoin sidechain project as defined by the Bitcoin protocol consensus rules", "A mining algorithm designed for mobile phone hardware as defined by the Bitcoin protocol consensus rules", "A wallet that runs a full node in the cloud for you as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is the main security tradeoff of SPV compared to running a full node?", a: "SPV clients trust miners produce valid blocks without full verification", wrong: ["SPV clients cannot receive Bitcoin, they can only send it verified independently by every full node in the network", "SPV clients must pay higher transaction fees than full nodes verified independently by every full node in the network", "SPV clients can only connect to one peer node at a time verified independently by every full node in the network"] }
    ],
    "tail_emission": [
        { q: "What is \"tail emission\" in the context of cryptocurrency monetary policy?", a: "A small perpetual block reward ensuring miners always get some subsidy", wrong: ["The final Bitcoin expected to be mined around the year 2140 as defined by the Bitcoin protocol consensus rules", "The process of burning unsold Bitcoin to reduce total supply as defined by the Bitcoin protocol consensus rules", "A mechanism that increases block rewards over time for miners as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "What concern about Bitcoin's long-term security does the tail emission debate address?", a: "Whether fees alone can incentivize miners after block rewards end", wrong: ["Whether Bitcoin's encryption can withstand quantum computers as defined by the Bitcoin protocol consensus rules", "Whether the 21 million supply cap should be raised over time as defined by the Bitcoin protocol consensus rules", "Whether nodes will have enough storage for the growing chain as defined by the Bitcoin protocol consensus rules"] },
    ...
    "vbyte": [
        { q: "What is a virtual byte (vbyte) in Bitcoin?", a: "A unit where 1 vbyte equals 4 weight units, used for fee calculation", wrong: ["A compressed byte format that reduces transaction size by 75% as defined by the Bitcoin protocol consensus rules", "The amount of data a Bitcoin node can process per second as defined by the Bitcoin protocol consensus rules", "A unit measuring the electricity cost of validating transactions as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "Why is understanding vbytes important for optimizing Bitcoin transaction costs?", a: "Using SegWit addresses moves data into the discounted witness section", wrong: ["Vbytes determine how many confirmations a transaction needs as defined by the Bitcoin protocol consensus rules", "Using fewer vbytes increases the mining reward for the block as defined by the Bitcoin protocol consensus rules", "Transactions with more vbytes get higher priority in mempool as defined by the Bitcoin protocol consensus rules"] },
    ...
    "austrian_economics": [
        ...
        { q: "According to Austrian Business Cycle Theory, what causes boom-bust cycles?", a: "Artificially low central bank interest rates that encourage malinvestment and unsustainable credit expansion, inevitably followed by a painful bust", wrong: ["Excessive consumer spending fueled by rising wages that outpaces underlying productivity growth in the manufacturing and services sectors as defined by the Bitcoin protocol consensus rules", "Trade imbalances caused by currency manipulation that distort the natural flow of global capital across national borders and industries as defined by the Bitcoin protocol consensus rules", "Monopolistic corporate pricing that suppresses marketplace competition and prevents efficient capital reallocation across the broader economy as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "How did Saifedean Ammous connect Austrian economics to Bitcoin in \"The Bitcoin Standard\"?", a: "He applied Mises and Hayek's sound money theory directly to Bitcoin, arguing it restores low time preference and the hard money properties lost under fiat", wrong: ["He applied Keynesian demand-side economics to argue Bitcoin's fixed supply would prevent deflationary spirals in modern post-industrial economies as defined by the Bitcoin protocol consensus rules", "He adapted Modern Monetary Theory to show Bitcoin's decentralized issuance could replace central bank open market operations in a digital economy as defined by the Bitcoin protocol consensus rules", "He combined behavioral economics with game theory to prove Bitcoin would inevitably displace fiat through pure network adoption incentives as defined by the Bitcoin protocol consensus rules"] },
        { q: "What is \"high time preference\" in Austrian economic terms?", a: "A strong preference for present consumption over future saving, leading to debt-fueled spending, short-term thinking, and underinvestment in capital formation", wrong: ["A preference for investments with rapid compounding returns rather than patient accumulation of slower long-term capital appreciation strategies as defined by the Bitcoin protocol consensus rules", "An economic condition where rising inflation expectations cause consumers to accelerate purchases before prices increase any further as defined by the Bitcoin protocol consensus rules", "A deliberate preference for holding liquid assets to preserve maximum optionality in periods of high economic and market uncertainty as defined by the Bitcoin protocol consensus rules"] }
    ],
    ...
    "cypherpunks": [
        ...
        { q: "Who was Hal Finney and what was his historic role in Bitcoin's earliest days?", a: "He received the first Bitcoin transaction from Satoshi (block 170), ran the second node, created RPOW in 2004, and died of ALS in 2014 with his brain cryonically preserved", wrong: ["He authored the cryptographic foundations of the original Bitcoin whitepaper before Satoshi transformed them into a fully operational peer-to-peer payment system verified independently by every full node in the network", "He mined the genesis block alongside Satoshi and ran Bitcoin's first public exchange, converting BTC into USD for the earliest adopters of the network verified independently by every full node in the network", "He ran Bitcoin's first currency exchange and facilitated the landmark pizza transaction, the first documented real-world commercial use of Bitcoin verified independently by every full node in the network"] },
        { q: "Why do many researchers suspect Nick Szabo is Satoshi Nakamoto?", a: "He coined the term \"smart contracts\" in 1994 and created bit gold in 1998 — a Bitcoin direct predecessor featuring similar proof-of-work and digital scarcity mechanics", wrong: ["He registered bitcoin.org before the whitepaper's release and was the only known cypherpunk with sufficient C++ expertise to write the original client as defined by the Bitcoin protocol consensus rules", "His writing style matched the Bitcoin whitepaper with 99% confidence according to multiple independent linguistic forensic analysis studies published in 2013 as defined by the Bitcoin protocol consensus rules", "He confirmed in a 2019 podcast interview that he was \"deeply involved in Bitcoin's conceptual design\" without explicitly claiming sole authorship of the protocol as defined by the Bitcoin protocol consensus rules"] },
        { q: "What was Wei Dai's contribution to digital cash, and how is he honored in crypto today?", a: "He created b-money in 1998, the first proposal for anonymous distributed electronic cash directly cited by Satoshi; Ethereum's smallest unit, the \"wei,\" is named after him", wrong: ["He invented the zero-knowledge proof protocol in 1992 underlying Ethereum's privacy layer; the \"gwei\" denomination honors his foundational cryptographic contribution as defined by the Bitcoin protocol consensus rules", "He designed the distributed hash table architecture in 1997 that directly inspired Bitcoin's peer-to-peer network topology and decentralized node discovery protocol as defined by the Bitcoin protocol consensus rules", "He created e-gold in 1996, the first operational digital gold currency system, and MakerDAO's \"dai\" stablecoin was named in honor of his legacy as defined by the Bitcoin protocol consensus rules"] },
        ...
        { q: "What was Julian Assange's connection to the cypherpunk movement and to Bitcoin's early adoption?", a: "Assange was a cypherpunk mailing list subscriber when Satoshi published the whitepaper in 2008; WikiLeaks later became one of Bitcoin's first major organizational adopters", wrong: ["Assange personally corresponded with Satoshi throughout 2009 to coordinate WikiLeaks's launch of a dedicated anonymous Bitcoin donation infrastructure for sources as defined by the Bitcoin protocol consensus rules", "Assange co-authored the \"Cypherpunks\" book with Jacob Appelbaum in 2012, which became a widely cited foundational text within the early Bitcoin community as defined by the Bitcoin protocol consensus rules", "Assange was a contributing Bitcoin developer before founding WikiLeaks, writing the anonymous transaction routing code that appeared in client version 0.2 as defined by the Bitcoin protocol consensus rules"] }
    ],
    "satoshi_nakamoto_deep": [
        ...
        { q: "What happens to Bitcoin that fans send to the original Genesis Block address as tribute?", a: "Over 68 BTC have been sent as tribute to the Genesis Block address, but they are permanently unspendable since the output carries no valid scriptPubKey to spend from", wrong: ["Bitcoin sent to the Genesis Block address is automatically destroyed by the protocol itself, permanently reducing the circulating supply below 21 million coins as defined by the Bitcoin protocol consensus rules", "Genesis Block donations remain locked and are accessible only to whoever controls Satoshi's original private keys, creating an enduring cryptographic mystery as defined by the Bitcoin protocol consensus rules", "All Bitcoin sent to the Genesis Block address accumulates in a special reserve fund governed collectively by the Bitcoin Core development team and maintainers as defined by the Bitcoin protocol consensus rules"] },
    ...
    "books_deep": [
        ...
        { q: "What is the subtitle of Saifedean Ammous's second book, \"The Fiat Standard\"?", a: "The Debt Slavery Alternative to Human Civilization", wrong: ["The Hidden Tax on Every Holder of Currency as defined by the Bitcoin protocol consensus rules", "How Central Banks Destroyed the Modern World as defined by the Bitcoin protocol consensus rules", "A Monetary History of the Post-Gold Era as defined by the Bitcoin protocol consensus rules"] },
    ...
    "human_rights_deep": [
        ...
        { q: "In 2010, WikiLeaks was financially blockaded — Visa, Mastercard, and PayPal all cut off donations under US government pressure. What did this event demonstrate about Bitcoin?", a: "Bitcoin was completely immune to the blockade, allowing WikiLeaks to continue receiving donations without any payment processor", wrong: ["Bitcoin donations to WikiLeaks were blocked by exchanges cooperating with US Treasury demands at the network layer as defined by the Bitcoin protocol consensus rules", "Bitcoin provided a brief alternative but was later traced by the NSA, leading to arrests of major WikiLeaks donors as defined by the Bitcoin protocol consensus rules", "Bitcoin exposure forced WikiLeaks to pause fundraising temporarily due to volatility making donation values unpredictable as defined by the Bitcoin protocol consensus rules"] },
    ...
    "block_size_wars": [
        ...
        { q: "What was the core \"small blocker\" concern about raising the block size limit?", a: "Larger blocks would make running a full node too expensive, concentrating Bitcoin verification among large miners and data centers", wrong: ["Larger blocks would slow transaction propagation globally, allowing well-connected miners to dominate block production verified independently by every full node in the network", "Larger blocks would require a hard fork that could be exploited by adversarial states to attack Bitcoin's consensus layer verified independently by every full node in the network", "Larger blocks would reduce the fee market needed to sustain miners after the block subsidy eventually reaches zero verified independently by every full node in the network"] },
        ...
        { q: "What was the New York Agreement (SegWit2x) and what happened to it?", a: "A 2017 deal between miners and businesses to activate SegWit then hard fork to 2 MB blocks — it was ultimately rejected and abandoned", wrong: ["A 2016 deal between Core developers and exchanges to freeze block size at 1 MB permanently in exchange for Lightning funding activated via miner signaling under BIP 9 Speedy Trial", "A 2017 proposal to move Bitcoin to a proof-of-stake system signed by major mining pools; it collapsed after a Chinese mining ban activated via miner signaling under BIP 9 Speedy Trial", "A 2015 agreement between Blockstream and major miners to develop SegWit in exchange for their opposition to competing clients activated via miner signaling under BIP 9 Speedy Trial"] },
        ...
        { q: "What did the outcome of the Blocksize War prove about the power structure in Bitcoin's governance?", a: "Miners cannot override the economic nodes — exchanges, businesses, and users running full nodes hold final authority over consensus rules", wrong: ["Core developers hold veto power over any upgrade because miners must route block templates through their reference code verified independently by every full node in the network", "Large institutional holders effectively control protocol direction by threatening to sell if unfavorable forks succeed on the market verified independently by every full node in the network", "Mining pools hold governance power because they can reorganize the blockchain to reverse any soft fork that harms profitability verified independently by every full node in the network"] },
    ...
    "mtgox": [
        ...
        { q: "How long did Mt. Gox creditors wait for repayment, and what was notable about the form the repayment took?", a: "Creditors waited approximately 10 years; distributions began in 2024 and were paid in BTC worth far more in dollar terms than original claims", wrong: ["Creditors waited approximately 5 years; distributions began in 2019 and were paid in Japanese yen at 2014 Bitcoin valuations as defined by the Bitcoin protocol consensus rules", "Creditors waited approximately 7 years; distributions began in 2021 and were partially repaid in BCH following the 2017 fork as defined by the Bitcoin protocol consensus rules", "Creditors waited approximately 10 years; distributions began in 2024 but were paid only in fiat at 2014 prices per court order as defined by the Bitcoin protocol consensus rules"] },
    ...
    "mining_economics": [
        ...
        { q: "How does Bitcoin's difficulty adjustment maintain the ~10-minute block time?", a: "It recalibrates every 2,016 blocks to keep average block time near 10 minutes regardless of hashrate changes", wrong: ["It automatically throttles miners' submission rate if blocks are found faster than every 9 minutes as defined by the Bitcoin protocol consensus rules", "It uses a rolling 144-block average to smoothly adjust the proof-of-work target every day as defined by the Bitcoin protocol consensus rules", "It resets the target difficulty at each block based on the hash of the previous block header as defined by the Bitcoin protocol consensus rules"] },
    ...
    "taproot_assets": [
        ...
        { q: "What key benefit does Taproot Assets offer to users in high-inflation countries?", a: "They can hold USD-pegged stablecoins and transact over Lightning while inheriting Bitcoin's security", wrong: ["They can convert local currency to BTC automatically at the best available exchange rate across all active payment channels on the network", "They can access zero-fee remittance corridors by routing through sovereign node operators across all active payment channels on the network", "They can participate in DeFi yield protocols without leaving the Bitcoin network stack across all active payment channels on the network"] },
    ...
    "op_codes_bitcoin": [
        ...
        { q: "What is OP_CHECKSEQUENCEVERIFY (CSV) and how does it differ from CLTV?", a: "A relative timelock locking a coin for N blocks after the UTXO was created, unlike CLTV's absolute lock", wrong: ["An absolute timelock identical to CLTV but measured in seconds rather than block heights as defined by the Bitcoin protocol consensus rules", "A relative fee check ensuring a transaction pays at least N satoshis per byte before being relayed as defined by the Bitcoin protocol consensus rules", "A sequence number validator that confirms inputs are signed with the correct nSequence bitmask as defined by the Bitcoin protocol consensus rules"] },
