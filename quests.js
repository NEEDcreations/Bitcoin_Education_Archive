const QUESTION_BANK = {
    ...
    'bitcoin_vs_real_estate': [
        { q: 'What key property rights advantage does Bitcoin have over real estate?', a: 'Bitcoin cannot be seized, taxed annually, or confiscated through eminent domain — you truly own it with your keys', wrong: ['Real estate provides stronger property rights because it is protected by law', 'Bitcoin and real estate have identical property rights protections', 'Bitcoin ownership is not recognized by any legal system'] },
        { q: 'How do maintenance costs compare between Bitcoin and real estate?', a: 'Bitcoin has near-zero maintenance costs, while real estate requires ongoing property taxes, insurance, repairs, and upkeep', wrong: ['Bitcoin requires expensive monthly server fees to maintain ownership', 'Real estate has no maintenance costs once the mortgage is paid off', 'Both assets have roughly equivalent ongoing maintenance costs'] },
        { q: 'Why is Bitcoin considered superior to real estate in terms of portability?', a: 'A Bitcoin seed phrase can be memorized and carried across any border — real estate is permanently fixed to one location', wrong: ['Real estate deeds can also be memorized and are equally portable', 'Bitcoin is less portable because it requires internet access', 'Portability is irrelevant when comparing stores of value'] },
        { q: 'How does Bitcoin\'s divisibility compare to real estate?', a: 'Bitcoin is divisible to 100 million satoshis per coin, while real estate cannot be meaningfully divided into micro-units', wrong: ['Real estate is more divisible because you can rent individual rooms', 'Bitcoin cannot be divided — you must buy a whole coin', 'Both assets offer identical divisibility through financial derivatives'] },
        { q: 'What risk does real estate face that Bitcoin does not, related to government power?', a: 'Confiscation risk through eminent domain, property tax liens, and government seizure — Bitcoin with self-custody avoids all three', wrong: ['Governments can easily confiscate Bitcoin but not real estate', 'Neither asset faces any confiscation risk in democratic countries', 'Bitcoin faces greater government risk because it can be banned'] },
    ],
    ...
    'bitvm': [
        ...
        { q: 'BitVM uses which computational model to verify off-chain computation?', a: 'An optimistic model where a prover makes a claim and a verifier can challenge it', wrong: ['A zero-knowledge model where proofs are posted on-chain every block', 'A consensus model where all nodes re-execute the computation', 'A federated model where trusted signers attest to results'] },
        ...
        { q: 'How does BitVM2 improve upon the original BitVM design?', a: 'It reduces the number of on-chain transactions needed for dispute resolution', wrong: ['It adds Turing-complete execution directly on the base layer', 'It eliminates the need for a verifier entirely', 'It requires a soft fork to enable new opcodes'] },
    ],

    'block_time-block-size': [
        { q: 'Why did Satoshi choose approximately 10 minutes for Bitcoin\'s target block time?', a: 'It balances transaction throughput with the time needed for blocks to propagate across the global network', wrong: ['It was arbitrary and has no technical justification', 'Faster blocks would make mining unprofitable due to electricity costs', 'It matches the average time for international wire transfers'] },
        { q: 'What is the difference between block weight and block size in Bitcoin?', a: 'Block weight counts witness data at a discount (1 weight unit per byte vs 4 for non-witness), replacing the old byte-based size limit', wrong: ['Block weight and block size are the same measurement with different names', 'Block weight measures the computational effort to validate a block', 'Block weight only applies to Lightning Network transactions'] },
        { q: 'What is the maximum block weight allowed by Bitcoin\'s consensus rules?', a: '4 million weight units (4 MWU)', wrong: ['1 megabyte', '8 million weight units', '2 million weight units'] },
        { q: 'Why does SegWit give witness data a discount in the block weight calculation?', a: 'Witness data is less costly for nodes to process and store, and the discount incentivizes SegWit adoption', wrong: ['Witness data is encrypted and therefore takes up less physical space', 'The discount was a compromise to end the Blocksize Wars with no real technical reason', 'Witness data is only stored temporarily and deleted after 100 blocks'] },
        { q: 'What is "blockspace economics" in Bitcoin?', a: 'The market for limited block space where users bid fees to have their transactions included', wrong: ['A government program to subsidize Bitcoin mining in renewable energy zones', 'The cost of running a full node measured in electricity and bandwidth', 'A pricing model for Lightning Network channel capacity'] },
    ],
    ...
    'burn_bitcoin': [
        ...
        { q: 'What does it mean for Bitcoin to be provably unspendable through OP_RETURN?', a: 'The output has no valid script path to spend it', wrong: ['It requires a special key to spend', 'It can only be spent after 100 blocks', 'It is temporarily locked'] },
        ...
        { q: 'What type of Bitcoin address output is provably unspendable with no script?', a: 'Bare multisig with invalid pubkeys', wrong: ['P2PKH addresses', 'P2SH addresses', 'P2WPKH addresses'] },
        { q: 'What is proof of burn in the context of cryptocurrency?', a: 'Demonstrating coins were sent to an unspendable address', wrong: ['Mining without receiving rewards', 'Deleting private keys', 'Sending coins to a dead wallet'] },
    ],

    'consensus_pt2': [
        { q: 'What does finality mean in Nakamoto Consensus?', a: 'Transactions become exponentially harder to reverse with each confirmation', wrong: ['Transactions are instantly irreversible', 'Miners vote on validity', 'The longest chain is always correct'] },
        { q: 'What are orphan blocks in Bitcoin?', a: 'Valid blocks that were not included in the main chain', wrong: ['Invalid blocks rejected by nodes', 'Blocks with no transactions', 'Genesis block variants'] },
        { q: 'What is selfish mining in Bitcoin?', a: 'Withholding found blocks to gain unfair advantage', wrong: ['Mining without pool rewards', 'Using renewable energy only', 'Refusing to validate transactions'] },
        { q: 'What is the typical Bitcoin block propagation time?', a: '1-10 seconds globally', wrong: ['About 60 seconds', 'Instantaneous', 'Approximately 10 minutes'] },
        { q: 'What was the FIBRE network designed to improve?', a: 'Block propagation speed between miners', wrong: ['Lightning payment routing', 'Wallet synchronization', 'Mining difficulty calculations'] },
    ],

    'cryptography_pt3': [
        { q: 'What advantage do Schnorr signatures provide over ECDSA in Bitcoin?', a: 'Key aggregation — multiple signers can produce a single compact signature, enabling efficient multisig', wrong: ['Schnorr signatures are quantum-resistant while ECDSA is not', 'Schnorr signatures are smaller in all cases regardless of signers', 'Schnorr was available in Bitcoin from day one but was never used'] },
        { q: 'What is MuSig2 in Bitcoin?', a: 'A two-round multi-signature scheme using Schnorr that produces a single signature indistinguishable from a regular one', wrong: ['A second version of the Bitcoin music streaming protocol', 'A mining algorithm that replaces SHA-256', 'A Lightning Network routing protocol for two-hop payments'] },
        { q: 'What are adaptor signatures used for in Bitcoin?', a: 'Enabling conditional payments where a signature reveals a secret, useful for atomic swaps and DLCs', wrong: ['Converting old transaction formats to be compatible with Taproot', 'Signing transactions with multiple keys simultaneously', 'Adapting Bitcoin Script to support Turing-complete smart contracts'] },
        { q: 'What is a Pedersen commitment?', a: 'A cryptographic commitment that hides a value while still allowing mathematical verification of properties like sums', wrong: ['A legal agreement between Bitcoin developers to maintain the protocol', 'A type of transaction fee commitment included in every block', 'A signed pledge by miners to follow consensus rules'] },
        { q: 'What is the current status of zero-knowledge proofs on Bitcoin?', a: 'They are being explored for privacy and scalability, with projects like BitVM and validity rollups advancing the concept', wrong: ['They have been fully implemented in Bitcoin Core since Taproot', 'They are impossible on Bitcoin due to Script limitations', 'They were removed from Bitcoin in 2015 for security reasons'] },
    ],

    'ctv-covenants': [
        { q: 'What does BIP-119 (CheckTemplateVerify) allow a Bitcoin output to do?', a: 'Commit to a specific future transaction template that must be used to spend it', wrong: ['Execute arbitrary smart contract code when spent', 'Lock funds until a specific block height is reached', 'Require multiple signatures from different keys to spend'] },
        { q: 'What is a "covenant" in the context of Bitcoin?', a: 'A restriction on how a UTXO can be spent in future transactions', wrong: ['A multisig arrangement between federation members', 'A legal agreement between Lightning channel partners', 'A time-locked contract that expires after a set period'] },
        { q: 'What Bitcoin use case does CTV enable through "congestion control"?', a: 'Batching many payments into a single on-chain transaction that recipients can claim later', wrong: ['Automatically adjusting transaction fees based on mempool size', 'Throttling the number of transactions a wallet can send per block', 'Limiting block size during periods of high network activity'] },
        { q: 'What is OP_VAULT designed to provide using covenants?', a: 'Secure vaulted custody with a time-delayed withdrawal and emergency clawback', wrong: ['A decentralized exchange for atomic swaps', 'Cold storage that automatically moves funds to hardware wallets', 'Multi-party escrow for marketplace transactions'] },
        { q: 'Why are covenants considered controversial by some Bitcoiners?', a: 'They could restrict how coins are spent, potentially enabling censorship or blacklisting', wrong: ['They require a hard fork that would split the network', 'They make all transactions publicly visible and remove privacy', 'They increase block size beyond what nodes can handle'] },
    ],

    'cycles': [
        { q: 'What drives Bitcoin\'s historically observed ~4-year market cycle?', a: 'The halving event, which cuts the new supply of Bitcoin in half approximately every 4 years', wrong: ['Government regulations that change every 4 years with election cycles', 'A software update that resets the network every 4 years', 'Random market psychology with no structural driver'] },
        { q: 'How does the Gartner Hype Cycle apply to each Bitcoin halving era?', a: 'Each cycle shows a trigger → peak of inflated expectations → trough of disillusionment → slope of enlightenment → plateau', wrong: ['Bitcoin skips the disillusionment phase because it always goes up', 'The Gartner Hype Cycle only applies to new technology products, not currencies', 'Each Bitcoin cycle follows the exact same price pattern as the previous one'] },
        { q: 'What is the "lengthening cycles" debate in Bitcoin?', a: 'The theory that each successive bull market takes longer to peak and produces smaller percentage gains as Bitcoin matures', wrong: ['The theory that Bitcoin mining cycles are getting shorter with faster hardware', 'A proposal to extend the halving interval from 4 years to 8 years', 'The idea that Bitcoin transaction confirmation times are increasing each year'] },
        { q: 'Which on-chain metrics are commonly used to analyze Bitcoin market cycles?', a: 'MVRV (Market Value to Realized Value), NUPL (Net Unrealized Profit/Loss), and Pi Cycle Top indicator', wrong: ['GDP growth rate, unemployment figures, and consumer price index', 'Twitter follower count, Google Trends, and Reddit subscriber numbers', 'Block size, node count, and developer commit frequency'] },
        { q: 'What is the "diminishing returns" theory in Bitcoin cycles?', a: 'Each cycle\'s percentage gain from bottom to top is smaller than the previous cycle, though each sets higher absolute highs', wrong: ['Bitcoin\'s returns are accelerating with each cycle, producing larger percentage gains', 'Diminishing returns means Bitcoin will eventually have negative returns every cycle', 'It refers to the decreasing energy efficiency of Bitcoin mining each cycle'] },
    ],

    'derivation_path': [
        { q: 'What does a BIP-32 HD (Hierarchical Deterministic) wallet allow you to do?', a: 'Generate an unlimited number of key pairs from a single master seed', wrong: ['Store multiple cryptocurrencies in a single private key', 'Recover funds without a seed phrase using only your email', 'Connect directly to the Bitcoin network without running a node'] },
        { q: 'In the derivation path m/84\'/0\'/0\'/0/0, what does the "84" represent?', a: 'BIP-84, indicating Native SegWit (bech32) address format', wrong: ['The year 1984 when public key cryptography was standardized', 'The 84th account in the wallet', 'The number of addresses that can be derived'] },
        { q: 'Which BIP defines the derivation path standard for Taproot (bc1p) addresses?', a: 'BIP-86', wrong: ['BIP-84', 'BIP-49', 'BIP-44'] },
        { q: 'Why is sharing an xpub (extended public key) a privacy concern?', a: 'Anyone with your xpub can see ALL addresses and balances derived from that account', wrong: ['An xpub allows someone to spend your Bitcoin without the private key', 'Sharing an xpub broadcasts your seed phrase to the network', 'An xpub reveals your real name and identity on the blockchain'] },
        { q: 'What is the "gap limit" in HD wallet address discovery?', a: 'The number of consecutive unused addresses a wallet checks before it stops looking for more', wrong: ['The maximum number of addresses a wallet can generate in total', 'The time delay between generating new receiving addresses', 'The minimum space between two UTXOs in the same transaction'] },
    ],

    'difficulty-adjustment_pt2': [
        { q: 'What algorithm does Bitcoin use for difficulty adjustment?', a: 'Direct calculation based on actual vs target time', wrong: ['PID controller algorithm', 'Neural network prediction', 'Moving average of hash rate'] },
        { q: 'How many blocks is the difficulty retarget period?', a: '2,016 blocks', wrong: ['1,008 blocks', '4,032 blocks', '10,080 blocks'] },
        { q: 'What causes hash rate oscillation between mining pools?', a: 'Miners chasing short-term luck variance', wrong: ['Bitcoin core software updates', 'Daily difficulty changes', 'Network communication delays'] },
        { q: 'What variance measure describes pool mining luck?', a: 'Statistical variance around expected block discovery', wrong: ['Fixed reward percentage', 'Difficulty share ratio', 'Network propagation delay'] },
    ...
    'dlcs': [
        { q: 'In a Discreet Log Contract (DLC), what role does an oracle play?', a: 'It signs a real-world outcome that automatically determines how the contract pays out', wrong: ['It holds the funds in escrow until both parties agree on the result', 'It executes smart contract code on a sidechain', 'It mines the transaction that settles the contract'] },
        { q: 'What cryptographic technique makes DLCs "discreet" — invisible to the oracle?', a: 'Adaptor signatures that use the oracle\'s attestation without revealing the contract exists', wrong: ['Zero-knowledge proofs that hide the contract terms from all participants', 'Ring signatures that mix the contract with unrelated transactions', 'Homomorphic encryption that processes data without decrypting it'] },
        { q: 'What is a common real-world use case for Discreet Log Contracts?', a: 'Financial derivatives and betting contracts settled by price or event data', wrong: ['Decentralized file storage on the Bitcoin network', 'Privacy-preserving identity verification', 'Automated market making for Lightning liquidity'] },
        { q: 'Why is the oracle in a DLC considered more trustworthy than traditional smart contract oracles?', a: 'The oracle doesn\'t know the contract exists or what bets depend on its attestation', wrong: ['The oracle stakes Bitcoin collateral that is slashed if it lies', 'Multiple oracles must all agree before any payout occurs', 'The oracle runs inside a trusted execution environment on-chain'] },
        { q: 'DLC.Link enables Discreet Log Contracts to be used primarily for what purpose?', a: 'Bridging Bitcoin as collateral into DeFi protocols without giving up custody', wrong: ['Mining pool payout distribution based on hash rate contribution', 'Decentralized DNS registration on the Bitcoin blockchain', 'Cross-chain atomic swaps between Bitcoin and Ethereum'] },
    ],

    'dollar-bitcoin_milkshake_theory': [
        { q: 'Who originated the "Dollar Milkshake Theory"?', a: 'Brent Johnson of Santiago Capital', wrong: ['Ray Dalio of Bridgewater Associates', 'Michael Saylor of MicroStrategy', 'Saifedean Ammous, author of The Bitcoin Standard'] },
        { q: 'What is the core thesis of the Dollar Milkshake Theory?', a: 'The US dollar will strengthen against other fiat currencies by "sucking up" global liquidity like a milkshake through a straw', wrong: ['All fiat currencies will collapse simultaneously and be replaced by Bitcoin', 'The dollar will weaken as other currencies strengthen against it', 'Central banks will coordinate to create a single global currency'] },
        { q: 'What is the "dollar strength paradox" in the Milkshake Theory?', a: 'The dollar can get stronger relative to other currencies even while losing purchasing power against hard assets like Bitcoin', wrong: ['A strong dollar always means Bitcoin price goes down', 'Dollar strength means the US economy is always healthy', 'The paradox is that no one wants dollars but everyone needs them'] },
        { q: 'How does Bitcoin fit into the Dollar Milkshake Theory\'s endgame?', a: 'After the dollar absorbs global liquidity and other currencies fail, Bitcoin emerges as the ultimate exit from all fiat', wrong: ['Bitcoin is irrelevant to the theory because it is not a fiat currency', 'Bitcoin gets destroyed along with other currencies in the dollar\'s rise', 'The theory predicts Bitcoin will be banned and replaced by CBDCs'] },
        { q: 'Why does global dollar demand play a central role in the Milkshake Theory?', a: 'Trillions in dollar-denominated debt worldwide forces nations to acquire dollars, strengthening it even as the US prints more', wrong: ['Global dollar demand is decreasing rapidly as nations de-dollarize', 'Dollar demand only matters for US domestic transactions', 'The theory says dollar demand is irrelevant — only supply matters'] },
    ],

    'el-salvador': [
        { q: 'What was the Chivo wallet in El Salvador\'s Bitcoin adoption?', a: 'The government-issued Bitcoin and Lightning wallet that gave citizens $30 in Bitcoin to encourage adoption', wrong: ['A private hardware wallet manufactured in El Salvador', 'An exchange where Salvadorans could trade altcoins', 'A banking app that only supported US dollar transactions'] },
        { q: 'What is "Bitcoin Beach" and why is it significant to El Salvador\'s Bitcoin story?', a: 'El Zonte — a coastal village that became a grassroots Bitcoin circular economy before the national law, proving real-world adoption', wrong: ['A luxury resort in San Salvador that only accepts Bitcoin', 'A government-built tourist attraction to promote Bitcoin', 'A cryptocurrency exchange headquartered on the Salvadoran coast'] },
        { q: 'What was President Bukele\'s core strategy behind making Bitcoin legal tender?', a: 'Financial inclusion for the unbanked population, attracting foreign investment, and reducing costly remittance fees', wrong: ['Replacing the US dollar entirely with Bitcoin as the sole currency', 'Mining Bitcoin using El Salvador\'s oil reserves', 'Creating a government-controlled cryptocurrency to replace Bitcoin'] },
        { q: 'How did the World Bank and IMF respond to El Salvador\'s Bitcoin law?', a: 'They pushed back — the World Bank refused to help implement it, and the IMF warned of financial risks', wrong: ['Both organizations fully endorsed and funded the initiative', 'The IMF helped design the Chivo wallet infrastructure', 'The World Bank provided a $500 million loan specifically for Bitcoin adoption'] },
        { q: 'How did Lightning Network adoption benefit El Salvador\'s economy?', a: 'It enabled fast, near-free transactions for daily purchases and cross-border remittances, reducing dependency on costly services like Western Union', wrong: ['Lightning was not used in El Salvador — only on-chain transactions were supported', 'Lightning made transactions more expensive but faster than traditional banking', 'El Salvador banned Lightning in favor of on-chain transactions only'] },
    ],

    'evidence-against-alts_pt2': [
        { q: 'What is a "premine" and why is it a red flag for altcoins?', a: 'Founders create and keep a large share of tokens before public launch, giving insiders an unfair advantage — Bitcoin had no premine', wrong: ['A premine is a security feature that makes blockchains launch faster', 'A premine means the coin was tested before release, which is good practice', 'A premine is when miners validate the genesis block for free'] },
        { q: 'Why are many altcoin tokens considered securities under US law?', a: 'They pass the Howey Test — investors buy tokens expecting profits from the efforts of a centralized team or foundation', wrong: ['The SEC automatically classifies all digital assets as securities', 'Any token traded on an exchange is legally a security', 'Only tokens with a market cap above $1 billion qualify as securities'] },
        { q: 'What is "VC token dumping" in the altcoin market?', a: 'Venture capital firms invest early at discounted prices, then sell their tokens on retail investors after public launch', wrong: ['VCs donate their tokens to charity after a project launches', 'VCs lock their tokens permanently to support the project', 'VCs use their tokens to fund Bitcoin development'] },
        { q: 'Why do Bitcoiners argue that "decentralized governance tokens" are an oxymoron?', a: 'Token-based voting concentrates power with the largest holders (whales and VCs), recreating the inequality they claim to solve', wrong: ['Governance tokens are too technically complex to be implemented properly', 'Bitcoin already uses governance tokens internally for upgrades', 'Governance tokens are illegal in all jurisdictions'] },
        { q: 'What key evidence do Bitcoiners cite about Ethereum\'s centralization?', a: 'Ethereum had a massive premine (72 million ETH to insiders), has changed its monetary policy multiple times, and relies on a known leadership team', wrong: ['Ethereum runs on a single server controlled by Vitalik Buterin', 'Ethereum transactions require government approval', 'Ethereum has fewer nodes than a typical corporate database'] },
    ],

    'extension-blocks': [
        { q: 'What are extension blocks in Bitcoin?', a: 'Additional block space that runs alongside the main chain without changing the existing block structure', wrong: ['Larger blocks created by increasing the block size limit through a hard fork', 'Blocks reserved exclusively for Lightning Network channel openings', 'Compressed blocks that store only transaction hashes to save space'] },
        { q: 'How do extension blocks differ from a traditional soft fork upgrade?', a: 'Old nodes see extension block transactions as "anyone-can-spend" outputs, creating a separate validation zone', wrong: ['Extension blocks require all nodes to upgrade simultaneously or be forked off', 'Extension blocks reduce the block size to improve decentralization', 'Extension blocks are identical to soft forks but with a different name'] },
        { q: 'What is the "space chain" concept related to extension blocks?', a: 'A proposal for a separate chain whose block space is auctioned via the Bitcoin main chain', wrong: ['A satellite network that broadcasts Bitcoin blocks from orbit', 'A virtual reality environment where Bitcoin nodes run in 3D space', 'A blockchain optimized for storing files larger than 4MB'] },
        { q: 'What is a key trade-off of extension blocks?', a: 'They create a "soft" two-tier system where non-upgraded nodes cannot fully validate extension block transactions', wrong: ['They require all miners to upgrade their hardware simultaneously', 'They permanently increase the main block size for all future blocks', 'They make Lightning Network channels incompatible with the base layer'] },
        { q: 'Why have extension block proposals historically been controversial in Bitcoin development?', a: 'They risk creating a "second-class" validation zone that could weaken full node security guarantees', wrong: ['They were proposed by altcoin developers trying to undermine Bitcoin', 'They would eliminate the need for the Lightning Network entirely', 'They require replacing SHA-256 with a different hashing algorithm'] },
    ],

    'feedback_loops': [
        { q: 'What is the price-hashrate-security feedback loop in Bitcoin?', a: 'Higher price attracts more miners → more hashrate → more security → more confidence → higher price', wrong: ['Higher price causes miners to shut down → less hashrate → less security', 'Hashrate and price are completely independent of each other', 'Security decreases as price increases because more hackers are attracted'] },
        { q: 'How does the adoption-liquidity-utility feedback loop work in Bitcoin?', a: 'More users → deeper liquidity → more useful as money → attracts more users', wrong: ['More users → higher fees → fewer users → less liquidity', 'Liquidity decreases as adoption increases because supply is fixed', 'Adoption and liquidity have an inverse relationship in Bitcoin'] },
        { q: 'How does the Lindy Effect create a reinforcing feedback loop for Bitcoin?', a: 'Every day Bitcoin survives increases confidence in its longevity, which attracts more adoption, further extending its lifespan', wrong: ['The Lindy Effect means Bitcoin becomes less reliable over time as code ages', 'Older technology always gets replaced, so the Lindy Effect works against Bitcoin', 'The Lindy Effect only applies to physical objects, not digital protocols'] },
        { q: 'What is reflexivity in the context of Bitcoin market cycles?', a: 'Market participants\' perceptions influence fundamentals, which in turn influence perceptions, creating self-reinforcing cycles', wrong: ['Reflexivity means Bitcoin\'s price always reflects its true fundamental value', 'It describes the tendency of Bitcoin to always return to the same price', 'Reflexivity is a mining technique that increases block production speed'] },
        { q: 'Why are Bitcoin\'s feedback loops considered among the most powerful in economics?', a: 'Multiple self-reinforcing loops (price, hashrate, adoption, security, media) all amplify each other simultaneously', wrong: ['Bitcoin has only one feedback loop, but it is very strong', 'Bitcoin\'s feedback loops are actually considered weak compared to traditional markets', 'Feedback loops only work during bull markets and reverse completely in bear markets'] },
    ],

    'first_principles': [
        { q: 'What does "first principles thinking" mean when applied to Bitcoin?', a: 'Breaking money down to its fundamental properties and reasoning up from there', wrong: ['Copying the design of existing currencies and improving them', 'Following the majority opinion of economists', 'Starting with government approval and working backward'] },
        ...
        { q: 'When reasoning from first principles about what makes good money, which set of properties matters most?', a: 'Scarcity, portability, durability, divisibility, and fungibility', wrong: ['Government backing, legal enforcement, and central bank control', 'Physical weight, aesthetic beauty, and historical tradition', 'Ease of printing, widespread banking access, and inflation targeting'] },
        { q: 'First principles thinking reveals what fundamental connection between energy and money?', a: 'Money should represent stored energy/work — unforgeable costliness gives it value', wrong: ['Money has no relationship to energy — it is purely a social construct', 'Only physical commodities can serve as money because they contain atoms', 'Energy is irrelevant to money as long as a government declares it legal tender'] },
        { q: 'Why does first principles reasoning tend to lead people toward Bitcoin rather than altcoins?', a: 'Bitcoin best satisfies the fundamental properties of sound money discovered through first principles analysis', wrong: ['Bitcoin was the first cryptocurrency so it wins by default', 'Altcoins are illegal in most countries', 'Bitcoin has the best marketing team and brand awareness'] },
    ],

    'foss': [
        { q: 'What type of software license does Bitcoin Core use?', a: 'MIT License', wrong: ['GPL v3', 'Apache License 2.0', 'Creative Commons'] },
        { q: 'Why is it important that Bitcoin Core is free and open-source software (FOSS)?', a: 'Anyone can audit the code to verify there are no backdoors or hidden rules', wrong: ['It makes Bitcoin free to use with no transaction fees', 'It allows developers to create unlimited Bitcoin', 'It means no one needs to run the software'] },
        { q: 'What are "reproducible builds" in the context of Bitcoin Core?', a: 'A process where anyone can compile the source code and verify the resulting binary matches the official release', wrong: ['A method of mining Bitcoin blocks more efficiently', 'A way to duplicate Bitcoin wallets for backup purposes', 'A technique for copying the blockchain to new nodes faster'] },
        { q: '"Linus\'s Law" states that "given enough eyeballs, all bugs are shallow." How does this apply to Bitcoin?', a: 'With thousands of developers reviewing Bitcoin\'s open-source code, vulnerabilities are more likely to be found and fixed', wrong: ['Linux servers are required to run Bitcoin nodes', 'Linus Torvalds personally audits Bitcoin Core releases', 'Bitcoin can only run on open-source operating systems'] },
        { q: 'Why does open-source matter specifically for money/monetary software like Bitcoin?', a: 'Users must be able to verify the rules of their money — no hidden inflation, no backdoors, no secret changes', wrong: ['Open-source software is always faster than proprietary software', 'It allows banks to copy Bitcoin and make their own version', 'It makes Bitcoin easier to regulate by governments'] },
    ],

    'geopolitics___macroeconomics_pt2': [
        { q: 'What is the BRICS de-dollarization movement and how does it relate to Bitcoin?', a: 'BRICS nations (Brazil, Russia, India, China, South Africa) are reducing dollar dependence — creating demand for neutral reserve assets like Bitcoin', wrong: ['BRICS is a Bitcoin mining pool operated by five countries', 'BRICS nations have collectively banned Bitcoin to protect their currencies', 'BRICS de-dollarization means these countries are adopting Bitcoin as legal tender'] },
        { q: 'What is the "strategic Bitcoin reserve" concept that multiple nations are exploring?', a: 'Nations accumulating Bitcoin as a reserve asset alongside gold, recognizing its scarcity and potential as a geopolitical hedge', wrong: ['A requirement for nations to back their fiat currency with Bitcoin', 'A UN mandate that all countries must hold at least 1,000 Bitcoin', 'A program where central banks replace all gold reserves with Bitcoin'] },
        { q: 'What is the "nation-state game theory" argument for Bitcoin adoption?', a: 'If one nation accumulates Bitcoin and others don\'t, the early adopter gains a massive strategic advantage — forcing others to follow or fall behind', wrong: ['Nations that ban Bitcoin will become stronger because they control their money supply', 'Game theory proves that no nation will ever adopt Bitcoin voluntarily', 'Nation-state game theory only applies to countries with nuclear weapons'] },
        { q: 'How do proponents respond to critics who say Bitcoin is used for sanctions evasion?', a: 'Bitcoin\'s transparent public ledger actually makes it easier to trace than cash, and sanctions enforcement has successfully tracked Bitcoin transactions', wrong: ['Bitcoin supporters agree that sanctions evasion is Bitcoin\'s primary use case', 'Bitcoin transactions are completely invisible to all governments', 'Sanctions don\'t apply to digital assets under international law'] },
        { q: 'How do institutions like the IMF and World Bank view Bitcoin?', a: 'Generally hostile — they have pressured countries like El Salvador to drop Bitcoin legal tender status, seeing it as a threat to their influence over developing nations', wrong: ['The IMF and World Bank are the largest institutional holders of Bitcoin', 'Both organizations have endorsed Bitcoin as the future of global finance', 'The IMF created Bitcoin as a replacement for Special Drawing Rights (SDRs)'] },
    ],

    'governance_pt2': [
        { q: 'Who can submit a Bitcoin Improvement Proposal (BIP)?', a: 'Anyone in the Bitcoin community', wrong: ['Only Bitcoin Core maintainers', 'Only miners with significant hash rate', 'Only registered developers'] },
        { q: 'What activation method does BIP 9 use?', a: 'Miner signaling with version bits', wrong: ['User activation only', 'Mandatory soft fork', 'Core developer approval'] },
        { q: 'What activation method does BIP 8 use?', a: 'Mandatory activation with optional miner signaling', wrong: ['Complete miner control', 'Exchange approval', 'Proof of stake voting'] },
        ...
        { q: 'What was Speedy Trial used for in Bitcoin?', a: 'Taproot activation with a shortened signaling period', wrong: ['SegWit activation', 'Lightning deployment', 'Difficulty adjustment changes'] },
    ],

    'hardware': [
        { q: 'What makes the Coldcard hardware wallet stand out among Bitcoin hardware wallets?', a: 'It is air-gapped, Bitcoin-only, and supports NFC signing — considered the gold standard', wrong: ['It supports thousands of altcoins and NFTs', 'It connects directly to the internet for instant transactions', 'It is the cheapest hardware wallet on the market'] },
        { q: 'What is a PSBT (Partially Signed Bitcoin Transaction) and why is it important for hardware wallets?', a: 'A standardized format that allows transactions to be constructed on one device and signed on an air-gapped hardware wallet', wrong: ['A type of Bitcoin address used exclusively by hardware wallets', 'A protocol for sending Bitcoin over Bluetooth between devices', 'A backup format for storing seed phrases digitally'] },
        { q: 'What is SeedSigner?', a: 'A DIY, open-source, air-gapped, and stateless hardware wallet built on a Raspberry Pi', wrong: ['A cloud service that generates seed phrases for you', 'A proprietary hardware wallet made by Ledger', 'An app that signs transactions using your phone\'s fingerprint sensor'] },
        { q: 'What is a "secure element" chip in hardware wallets?', a: 'A tamper-resistant chip that stores private keys and prevents physical extraction of secrets', wrong: ['A chip that connects the wallet to Wi-Fi securely', 'A component that speeds up Bitcoin transaction processing', 'A GPS module that tracks the wallet\'s location for recovery'] },
        { q: 'What does "air-gapped" mean when describing a hardware wallet like Coldcard or Foundation Passport?', a: 'The device never connects to the internet or a computer — transactions are transferred via SD card, QR code, or NFC', wrong: ['The wallet is sealed in a vacuum to prevent physical tampering', 'The wallet uses satellite connections instead of Wi-Fi', 'The wallet can only be used outdoors for security reasons'] },
    ],
    ...
    'history_pt3': [
        ...
        { q: 'What was the main conflict of the Blocksize War?', a: 'Whether to increase block size limit', wrong: ['Changing Bitcoin to proof of stake', 'Adding privacy features', 'Changing the halving schedule'] },
        { q: 'What was SegWit2x?', a: 'Failed agreement to double block size after SegWit', wrong: ['Successful hard fork increasing block size', 'New mining algorithm', 'Second layer protocol'] },
    ],

    'improved_incentive_structure': [
        { q: 'How does Bitcoin "fix broken incentives" in the current monetary system?', a: 'By removing the ability to print money, it forces honest accounting and eliminates the incentive to inflate away debt', wrong: ['Bitcoin gives governments better tools to manage the economy', 'Bitcoin doesn\'t change incentives — it just moves them to a digital platform', 'Bitcoin fixes incentives by making all transactions public and traceable by governments'] },
        { q: 'What does "low time preference society" mean in the context of a Bitcoin standard?', a: 'People plan for the long term, save more, and invest in durable things because their money gains value over time', wrong: ['A society that doesn\'t care about time and works fewer hours', 'A society where transactions must be completed within strict time limits', 'A society that only uses Bitcoin for short-term speculation'] },
        { q: 'Why is proof of work considered a "fair distribution" mechanism for new Bitcoin?', a: 'Anyone can mine by expending real energy — no pre-mines, no insider allocations, no venture capital favoritism', wrong: ['Proof of work gives equal Bitcoin to every person on Earth', 'Early miners were given special privileges that later miners do not have', 'Proof of work distributes Bitcoin based on geographic location'] },
        { q: 'How does sound money (like Bitcoin) change savings and investment behavior?', a: 'People save more because their money appreciates, and only invest in projects that generate real returns above the deflation rate', wrong: ['Sound money discourages all saving because money becomes too valuable to hold', 'Investment behavior stays exactly the same regardless of monetary system', 'Sound money causes everyone to hoard and never spend, collapsing the economy'] },
        { q: 'What broken incentive does fiat money create that Bitcoin eliminates?', a: 'Fiat incentivizes borrowing and spending over saving because inflation constantly erodes purchasing power', wrong: ['Fiat incentivizes too much saving, which Bitcoin fixes by encouraging spending', 'Fiat money has no broken incentives — it works exactly as designed', 'Bitcoin creates the same incentive to borrow because interest rates still exist'] },
    ],

    'investment-strategy_pt4': [
        { q: 'What is UTXO management for tax optimization?', a: 'Selecting specific UTXOs with known cost bases when spending, to minimize capital gains taxes by choosing higher-cost-basis coins', wrong: ['Automatically hiding Bitcoin transactions from the IRS', 'Converting all UTXOs to altcoins before tax season', 'Splitting Bitcoin into exactly $600 transactions to avoid reporting'] },
        { q: 'What does "step-up in cost basis" mean for inherited Bitcoin?', a: 'When Bitcoin is inherited, the cost basis resets to the market price at the time of the original owner\'s death, potentially eliminating years of capital gains taxes', wrong: ['Inherited Bitcoin is always taxed at the original purchase price', 'There is no tax on inherited Bitcoin in any jurisdiction', 'The cost basis doubles each year the Bitcoin is held after inheritance'] },
        { q: 'What is a Bitcoin IRA?', a: 'A self-directed individual retirement account that allows holding Bitcoin with tax-advantaged growth (tax-deferred or tax-free)', wrong: ['A government program that pays retirees in Bitcoin', 'An insurance policy that replaces your Bitcoin if it is lost or stolen', 'A savings account at a bank that earns interest in Bitcoin'] },
        { q: 'Why do experienced Bitcoiners warn against lending out your Bitcoin?', a: 'Lending platforms introduce counterparty risk — companies like Celsius, BlockFi, and Voyager collapsed, and depositors lost their Bitcoin', wrong: ['Bitcoin code prevents any form of lending or borrowing', 'Lending Bitcoin causes the network to slow down significantly', 'Bitcoin interest rates are always negative, meaning you lose money'] },
        { q: 'Why is cost basis tracking important for Bitcoin investors?', a: 'Each Bitcoin purchase creates a separate tax lot with its own cost basis — accurate tracking is needed to correctly calculate capital gains or losses', wrong: ['Cost basis tracking is only required for Bitcoin held on exchanges', 'The IRS uses a flat tax rate on all Bitcoin regardless of purchase price', 'Cost basis is irrelevant because Bitcoin is classified as a currency, not property'] },
    ],

    'laws_of_thermodynamics': [
        { q: 'How does the First Law of Thermodynamics (conservation of energy) relate to Bitcoin mining?', a: 'Miners convert electricity into computational work that secures the ledger — energy is conserved, not destroyed', wrong: ['Bitcoin mining creates energy from nothing to power the network', 'The First Law means Bitcoin can only exist in cold temperatures', 'Energy is destroyed during mining, which is why it uses so much power'] },
        { q: 'What does "unforgeable costliness" mean in Bitcoin\'s thermodynamic context?', a: 'Real energy must be expended to produce new Bitcoin, and this cost cannot be faked or shortcut', wrong: ['Bitcoin transactions are free because the network is decentralized', 'The cost of mining is purely artificial and serves no security purpose', 'Unforgeable costliness means Bitcoin wallets cannot be hacked'] },
        { q: 'How does the Second Law of Thermodynamics (entropy always increases) connect to Bitcoin?', a: 'Bitcoin\'s proof of work creates order (a secure ledger) by increasing entropy in the physical world through energy expenditure', wrong: ['Bitcoin violates the Second Law by creating value from nothing', 'Entropy has no connection to digital systems like Bitcoin', 'The Second Law means Bitcoin will eventually stop working'] },
        { q: 'Why do some physicists describe proof of work as "thermodynamic security"?', a: 'Reversing the blockchain would require re-expending all the energy used to build it, which is physically impractical', wrong: ['Because Bitcoin mining rigs get very hot during operation', 'Because proof of work requires cooling systems to function', 'Because thermodynamic equations are used to set transaction fees'] },
        { q: 'What is the "Maxwell\'s Demon" analogy sometimes applied to Bitcoin?', a: 'Just as Maxwell\'s Demon cannot sort molecules without expending energy, you cannot secure a ledger without real-world energy cost', wrong: ['Maxwell\'s Demon is the name of the first Bitcoin mining software', 'It describes a theoretical attack where a demon-like AI takes over the network', 'It refers to the randomness of Bitcoin price movements being similar to particle motion'] },
    ],

    'layer-2-lightning_pt4': [
        { q: 'What is a submarine swap on the Lightning Network?', a: 'A trustless exchange between on-chain Bitcoin and off-chain Lightning Bitcoin using HTLCs', wrong: ['A method to secretly close Lightning channels', 'A way to mine Bitcoin through Lightning nodes', 'An attack that drains funds from Lightning channels'] },
        { q: 'What is Multi-Path Payments (MPP) on Lightning?', a: 'Splitting a single payment across multiple routes to send amounts larger than any single channel capacity', wrong: ['Making multiple payments to different recipients simultaneously', 'A backup payment method if Lightning is down', 'Sending the same payment through multiple channels for redundancy'] },
        { q: 'What is AMP (Atomic Multi-Path Payments) on Lightning?', a: 'A payment method where split parts are only redeemable together, ensuring all-or-nothing delivery', wrong: ['An amplifier that boosts signal strength between Lightning nodes', 'A method to increase the maximum channel capacity', 'An automatic mining protocol built into Lightning'] },
        { q: 'What is trampoline routing on the Lightning Network?', a: 'Delegating route calculation to intermediate "trampoline" nodes, reducing the burden on mobile wallets', wrong: ['Bouncing payments off satellites for global coverage', 'A routing method that prioritizes the fastest path', 'A security protocol that encrypts payment amounts'] },
        { q: 'What are channel factories in the Lightning Network?', a: 'A mechanism allowing multiple parties to open many channels from a single on-chain transaction', wrong: ['Physical facilities where Lightning nodes are manufactured', 'Automated systems that create channels based on demand', 'Software that converts on-chain wallets into Lightning wallets'] },
    ],

    'layer-2-lightning_pt5': [
        { q: 'What is liquidity management on the Lightning Network?', a: 'Balancing inbound and outbound capacity across channels to reliably send and receive payments', wrong: ['Converting Lightning Bitcoin back to on-chain Bitcoin', 'Managing the total supply of Bitcoin on Lightning', 'Keeping Lightning nodes connected to the internet'] },
        { q: 'What is channel rebalancing on Lightning?', a: 'Moving funds between your own channels to redistribute liquidity where it is needed', wrong: ['Closing and reopening channels with new parameters', 'Resetting channel state to the initial funding amount', 'Transferring channel ownership to another node'] },
        { q: 'What is a Lightning Service Provider (LSP)?', a: 'A service that provides liquidity and channel management to help users connect to Lightning easily', wrong: ['An internet service provider that specializes in Bitcoin traffic', 'A government-licensed Lightning Network operator', 'A cloud service that mines Bitcoin using Lightning fees'] },
        { q: 'What are zero-conf channels on Lightning?', a: 'Channels that can be used immediately without waiting for on-chain confirmation of the funding transaction', wrong: ['Channels that require zero Bitcoin to open', 'Channels that provide zero-fee routing', 'Channels that cannot be closed once opened'] },
        { q: 'How do Lightning routing nodes earn revenue?', a: 'By charging small fees for forwarding payments through their channels', wrong: ['By mining new Bitcoin on the Lightning layer', 'By collecting a percentage of all Lightning transactions globally', 'By selling their channel data to analytics companies'] },
    ],

    'layer-3-sidechains': [
        { q: 'How does the Liquid Network secure its two-way peg to Bitcoin?', a: 'Through a federation of functionaries who collectively manage the peg using multisig', wrong: ['Through merge-mining where Bitcoin miners also validate Liquid blocks', 'Through a trustless smart contract on Bitcoin\'s base layer', 'Through proof-of-stake consensus among Liquid token holders'] },
        { q: 'How does RSK (Rootstock) secure its sidechain?', a: 'Through merge-mining, where Bitcoin miners simultaneously validate RSK blocks', wrong: ['Through a federation of trusted companies that sign blocks', 'Through proof-of-stake where RSK token holders vote on blocks', 'Through BitVM fraud proofs posted to the Bitcoin base layer'] },
        { q: 'What consensus mechanism does Stacks use to connect to Bitcoin?', a: 'Proof of Transfer (PoX), where miners spend BTC to participate in block production', wrong: ['Proof of Work using the same SHA-256 algorithm as Bitcoin', 'Delegated Proof of Stake with elected block producers', 'Proof of Authority using a fixed set of validators'] },
        { q: 'What are drivechains (BIP-300) designed to enable?', a: 'Permissionless sidechains where miners vote to approve withdrawals back to the Bitcoin main chain', wrong: ['Faster block times on the Bitcoin base layer by reducing difficulty', 'A new type of Lightning channel that doesn\'t require on-chain funding', 'Decentralized mining pools that distribute rewards automatically'] },
        { q: 'What is the primary security concern with federated sidechains like Liquid?', a: 'Users must trust the federation members not to collude and steal the pegged funds', wrong: ['The sidechain can reverse Bitcoin base layer transactions', 'Federated sidechains consume more energy than Bitcoin\'s proof-of-work', 'The federation can change Bitcoin\'s 21 million supply cap'] },
    ],

    'maximalism_pt2': [
        { q: 'What do Bitcoiners mean when they call "toxic maximalism" an immune response?', a: 'Aggressive pushback against scams and altcoins protects newcomers from losing money, acting like an immune system for the ecosystem', wrong: ['Maximalists spread computer viruses to attack altcoin networks', 'Toxic maximalism is a disease that Bitcoin developers are trying to cure', 'It means Bitcoin\'s code rejects transactions from altcoin users'] },
        { q: 'What is the "altcoin opportunity cost" argument made by Bitcoin maximalists?', a: 'Every dollar spent on altcoins could have been invested in Bitcoin, which has outperformed virtually all altcoins over long time horizons', wrong: ['Altcoins are free to create, so there is no opportunity cost', 'Bitcoin maximalists believe altcoins increase Bitcoin\'s value', 'Opportunity cost only applies to fiat currencies, not digital assets'] },
        { q: 'Why do Bitcoin-only companies (like Swan, River, and Unchained) choose not to support altcoins?', a: 'They believe promoting altcoins is ethically wrong because most will fail, harming their customers — and Bitcoin is the only truly decentralized money', wrong: ['They lack the technical capability to support other cryptocurrencies', 'Government regulations prohibit companies from offering multiple cryptocurrencies', 'Supporting altcoins would make their software run too slowly'] },
        { q: 'What is the "ethical case" for Bitcoin maximalism?', a: 'Promoting unregistered securities (altcoins) to unsophisticated investors causes real financial harm — Bitcoin maximalism protects people', wrong: ['Bitcoin maximalists believe they have a religious duty to spread Bitcoin', 'Ethics require that all financial technologies be treated equally', 'The ethical case is that Bitcoin miners deserve all crypto profits'] },
        { q: 'How does Gresham\'s Law ("bad money drives out good") apply to the Bitcoin maximalism argument?', a: 'People spend inflationary fiat and altcoins while hoarding Bitcoin, because Bitcoin is the hardest (best) money — confirming its superiority', wrong: ['Gresham\'s Law proves that cheaper altcoins will replace Bitcoin over time', 'It means governments will force citizens to use Bitcoin instead of fiat', 'Gresham\'s Law only applies to physical coins, not digital currencies'] },
    ],

    'mining_pt5': [
        { q: 'What does Stratum V2 allow individual miners to do that Stratum V1 does not?', a: 'Choose their own transactions for block templates', wrong: ['Mine without an internet connection', 'Bypass the difficulty adjustment', 'Mine multiple coins simultaneously'] },
        { q: 'What is a block template in Bitcoin mining?', a: 'A candidate block assembled with selected transactions before a valid hash is found', wrong: ['A blueprint for building ASIC hardware', 'A backup copy of the blockchain', 'A template for creating new mining pools'] },
        { q: 'What is a selfish mining attack?', a: 'A strategy where a miner withholds discovered blocks to gain an unfair advantage', wrong: ['Refusing to share mining profits with pool members', 'Mining only low-fee transactions for personal gain', 'Using stolen electricity to power mining rigs'] },
        { q: 'What is ASICBoost?', a: 'A technique to optimize SHA-256 computations and reduce energy per hash', wrong: ['A brand of mining hardware from Bitmain', 'A firmware update that doubles hash rate', 'A method to cool ASIC miners using liquid nitrogen'] },
        { q: 'What is the difference between overt and covert ASICBoost?', a: 'Overt ASICBoost is visible in the block header; covert ASICBoost is hidden', wrong: ['Overt is legal and covert is illegal', 'Overt uses more energy than covert', 'Overt works on all ASICs while covert only works on Bitmain hardware'] },
    ],

    'mining_pt6': [
        { q: 'What is "hash price" in Bitcoin mining economics?', a: 'The revenue earned per unit of hash rate per day (e.g., dollars per TH/s/day)', wrong: ['The cost to purchase a single ASIC miner', 'The price of Bitcoin divided by the hash rate', 'The electricity cost per hash computed'] },
        { q: 'What typically happens to less efficient miners shortly after a Bitcoin halving?', a: 'They become unprofitable and may shut down, causing miner capitulation', wrong: ['They automatically receive double the block reward', 'Their hash rate doubles to compensate', 'They switch to mining altcoins permanently'] },
        { q: 'What is the typical operational lifespan of an ASIC miner before it becomes obsolete?', a: 'Approximately 3 to 5 years depending on efficiency and electricity costs', wrong: ['About 6 months before the next model arrives', 'Over 20 years with proper maintenance', 'Exactly one halving cycle (4 years)'] },
        { q: 'What is the breakeven electricity cost for Bitcoin mining?', a: 'The price per kWh at which mining revenue exactly equals electricity costs', wrong: ['A fixed rate set by Bitcoin Core developers', 'The average electricity cost across all countries', 'The minimum electricity needed to power one ASIC'] },
        { q: 'As block subsidies decrease over time, what must replace them to incentivize miners?', a: 'Transaction fees from users competing for block space', wrong: ['Government subsidies for miners', 'Increased mining difficulty to raise Bitcoin price', 'New coins created through hard forks'] },
    ],

    'misconceptions-fud_pt2': [
        { q: 'How do Bitcoiners respond to "Bitcoin is too volatile to be money"?', a: 'Every 4-year period in Bitcoin\'s history has been profitable — volatility is the price of monetization, and it decreases over time', wrong: ['They agree that volatility makes Bitcoin unsuitable as money', 'They say volatility doesn\'t exist if you don\'t check the price', 'They point out that Bitcoin\'s price is actually fixed by miners'] },
        { q: 'Why is the claim "quantum computers will break Bitcoin" misleading?', a: 'Quantum computers powerful enough to threaten Bitcoin don\'t exist yet, and Bitcoin can upgrade its cryptography before they do', wrong: ['Bitcoin already uses quantum-resistant encryption', 'Quantum computers actually make Bitcoin stronger', 'Satoshi Nakamoto designed Bitcoin to be immune to all future computing advances'] },
        { q: 'What is the rebuttal to "Tether (USDT) props up Bitcoin\'s price"?', a: 'Bitcoin existed and grew for years before Tether, and Bitcoin\'s price has continued rising through multiple Tether controversies', wrong: ['Tether actually holds 100% of its reserves in Bitcoin', 'Bitcoin\'s price is determined solely by mining costs, not stablecoins', 'Tether was created by Satoshi Nakamoto as part of Bitcoin\'s design'] },
        { q: 'Why is "Bitcoin is a pyramid scheme" factually wrong?', a: 'Bitcoin has no company, no CEO, no promises of returns, and no one profits from recruiting — it is open-source code anyone can audit', wrong: ['Because early adopters are guaranteed profits by the protocol', 'Because Satoshi Nakamoto still runs the company behind Bitcoin', 'Because Bitcoin pays dividends to long-term holders'] },
        { q: 'What is the strongest argument against "governments will ban Bitcoin"?', a: 'China banned Bitcoin multiple times yet it kept growing, and major nations like the US have approved Bitcoin ETFs instead of banning it', wrong: ['Governments lack the technology to detect Bitcoin transactions', 'International law prevents any country from banning financial technology', 'Bitcoin automatically shuts down in countries where it is banned'] },
    ],

    'nodes_pt4': [
        { q: 'What is the key difference between a pruned node and an archival (full) node?', a: 'A pruned node deletes old block data after verifying it, while an archival node keeps all historical data', wrong: ['A pruned node skips transaction verification to save time', 'An archival node only stores the UTXO set', 'A pruned node cannot validate new transactions'] },
        { q: 'What does BIP 152 (Compact Block Relay) do?', a: 'Reduces bandwidth by sending only short transaction IDs instead of full transactions in blocks', wrong: ['Compresses the entire blockchain into a smaller file', 'Allows nodes to skip blocks during initial sync', 'Encrypts block data for privacy between nodes'] },
        { q: 'What is the Erlay protocol designed to improve?', a: 'Transaction relay efficiency by reducing redundant bandwidth between nodes', wrong: ['Block mining speed by optimizing nonce searches', 'Lightning channel capacity management', 'Wallet synchronization for mobile devices'] },
        { q: 'What is the UTXO set and why does its size matter for nodes?', a: 'It is the set of all unspent transaction outputs that nodes must keep in memory for fast validation', wrong: ['It is a list of all transactions ever made on Bitcoin', 'It is a record of all mining rewards distributed', 'It is a backup of private keys stored across the network'] },
        { q: 'What is a major concern about running a Bitcoin full node related to bandwidth?', a: 'Nodes must relay transactions and blocks, consuming significant upload bandwidth', wrong: ['Nodes must pay per-megabyte fees to the Bitcoin network', 'Bandwidth usage is zero once the initial block download completes', 'Only mining nodes use significant bandwidth'] },
    ],

    'nodes_pt5': [
        { q: 'What programming language is Bitcoin Core primarily written in?', a: 'C++', wrong: ['Rust', 'Python', 'Java'] },
        { q: 'What is btcd?', a: 'An alternative full node implementation of Bitcoin written in Go', wrong: ['A Bitcoin wallet for desktop computers', 'A command-line tool for Bitcoin mining', 'A testing framework for Bitcoin Core'] },
        { q: 'What are reproducible builds in the context of Bitcoin Core?', a: 'A process where anyone can compile the source code and get an identical binary, proving no malicious code was added', wrong: ['A system where Bitcoin Core automatically updates itself', 'A method to restore a node from backup', 'A way to compile Bitcoin Core faster on multiple CPUs'] },
        { q: 'What is Guix used for in Bitcoin Core development?', a: 'A build system that enables fully reproducible and verifiable builds of Bitcoin Core', wrong: ['A graphical user interface for running a Bitcoin node', 'A security scanner that checks for vulnerabilities', 'A package manager for installing Bitcoin wallets'] },
        { q: 'Why is it important to be able to build Bitcoin Core from source?', a: 'To verify the code yourself and ensure the binary you run matches the open-source code', wrong: ['Because pre-built binaries are not available for download', 'To get access to hidden developer features', 'Because building from source makes the node run faster'] },
    ],

    'op-codes_pt2': [
        { q: 'What would OP_CAT enable if re-enabled?', a: 'Concatenation of data values in script', wrong: ['Comparison of timestamps', 'Multiplication of values', 'Direct signature verification'] },
        { q: 'What does OP_CHECKSIGFROMSTACK verify?', a: 'Arbitrary message signatures provided on stack', wrong: ['Only standard transaction signatures', 'Miners\' block signatures', 'Lightning commitment signatures'] },
        { q: 'What is Simplicity designed for in Bitcoin?', a: 'Formal verification of smart contracts', wrong: ['Faster transaction processing', 'Simplified wallet addresses', 'Reduced block size'] },
        { q: 'What is a major limitation of Bitcoin Script?', a: 'No loops or complex control flow', wrong: ['Maximum 100 op codes per script', 'Cannot handle signatures', 'Requires internet connection'] },
        { q: 'Which script type introduced with Taproot?', a: 'P2TR (Pay to Taproot)', wrong: ['P2SH', 'P2WSH', 'P2PKH'] },
    ],

    'orange-pilling_pt2': [
        { q: 'What is the "Socratic method" approach to orange-pilling someone about Bitcoin?', a: 'Asking thoughtful questions that lead the person to discover Bitcoin\'s value on their own, rather than lecturing them', wrong: ['Quoting ancient Greek philosophers who predicted digital money', 'Having the person read the Bitcoin whitepaper out loud', 'Debating them aggressively until they agree Bitcoin is good'] },
        { q: 'What is "progressive disclosure" when introducing someone to Bitcoin?', a: 'Starting with simple concepts they care about (inflation, saving) and gradually introducing deeper topics as they become curious', wrong: ['Showing them Bitcoin\'s full transaction history from the genesis block', 'Revealing your entire Bitcoin portfolio to build trust', 'Disclosing all of Bitcoin\'s technical flaws upfront to seem honest'] },
        { q: 'When orange-pilling family members, what is generally the most effective approach?', a: 'Focus on what THEY care about — their savings, their retirement, their kids\' future — not technical details', wrong: ['Send them a 3-hour technical podcast about mining and hash rates', 'Buy them Bitcoin without asking and tell them later', 'Bring it up at every family dinner until they agree'] },
        { q: 'What is one of the biggest mistakes people make when trying to orange-pill someone?', a: 'Information overload — dumping too much technical detail at once instead of planting seeds', wrong: ['Being too patient and waiting for them to ask questions', 'Starting with how Bitcoin helps people in developing countries', 'Recommending they read a book about money'] },
    ...
    'ordinals_inscriptions': [
        ...
        { q: 'How are individual satoshis numbered in Ordinal Theory?', a: 'Sequentially based on mining order since genesis', wrong: ['Randomly assigned', 'By transaction ID', 'By wallet address'] },
        { q: 'What do inscriptions attach to specific satoshis?', a: 'Arbitrary data like images or text', wrong: ['Private keys', 'Transaction history', 'Spending conditions'] },
        { q: 'What token standard emerged from Ordinals functionality?', a: 'BRC-20', wrong: ['ERC-20', 'RGB', 'Omni Layer'] },
        { q: 'What was a major criticism of Ordinals regarding block space?', a: 'They consume limited block space for non-monetary data', wrong: ['They reduce mining rewards', 'They break consensus rules', 'They steal other users\' funds'] },
    ],

    'philosophy_pt2': [
        { q: 'Who wrote the Cypherpunk Manifesto?', a: 'Eric Hughes', wrong: ['Timothy May', 'Julian Assange', 'Nick Szabo'] },
        { q: 'What concept did Timothy May advocate for in Crypto-Anarchist Manifesto?', a: 'Using cryptography to protect individual privacy from state', wrong: ['Centralized digital currencies', 'Government regulation of encryption', 'Corporate control of networks'] },
        { q: 'What was Nick Szabo\'s Bit Gold concept?', a: 'Digital collectible with proof of work', wrong: ['Stablecoin backed by physical gold', 'Centralized digital currency', 'Lightning payment channel'] },
        { q: 'What did Wei Dai propose with b-money?', a: 'Anonymous digital cash with decentralized minting', wrong: ['Blockchain with mining rewards', 'Central bank digital currency', 'Gold-backed certificates'] },
        { q: 'What was Hal Finney\'s famous contribution to Bitcoin?', a: 'Received first Bitcoin transaction from Satoshi', wrong: ['Wrote the original whitepaper', 'Created the mining algorithm', 'Founded the first exchange'] },
    ],

    'privacy-nonkyc_pt4': [
        { q: 'What is a PayJoin (P2EP) transaction?', a: 'A transaction where both sender and receiver contribute inputs, making it harder to identify who paid whom', wrong: ['A payment split equally between two parties', 'A transaction that requires two confirmations', 'A joint mining operation between two pools'] },
        { q: 'What are Silent Payments (BIP 352)?', a: 'A protocol allowing receivers to publish one static address while each payment generates a unique on-chain address', wrong: ['Transactions that do not appear in the mempool', 'Payments that are delayed until network activity is low', 'A method to send Bitcoin without any transaction fees'] },
        { q: 'What is the Boltzmann entropy score used for in Bitcoin privacy?', a: 'Measuring the ambiguity of a transaction\'s input-output linkages to assess its privacy level', wrong: ['Calculating the thermodynamic energy cost of a Bitcoin transaction', 'Rating the randomness quality of a wallet\'s seed phrase', 'Scoring how quickly a transaction will be confirmed'] },
        { q: 'What does Whirlpool do for Bitcoin privacy?', a: 'A CoinJoin implementation that mixes UTXOs with other users to break transaction history links', wrong: ['A VPN service specifically designed for Bitcoin nodes', 'A method to encrypt Bitcoin transactions end-to-end', 'A privacy coin that runs on the Bitcoin blockchain'] },
        { q: 'What is CoinSwap and how does it differ from CoinJoin?', a: 'CoinSwap uses atomic swaps between users so the mixing is invisible on-chain, unlike CoinJoin\'s visible equal-output pattern', wrong: ['CoinSwap converts Bitcoin to privacy coins and back, while CoinJoin mixes UTXOs', 'CoinSwap is faster but less private than CoinJoin', 'CoinSwap requires a trusted third party while CoinJoin does not'] },
    ],

    'problems-of-money_pt4': [
        { q: 'The Cantillon Effect describes how newly printed money benefits those closest to its creation. Who benefits MOST from money printing?', a: 'Banks, financial institutions, and the politically connected who receive new money first, before prices rise', wrong: ['Average workers who receive higher wages from inflation', 'Retirees on fixed incomes who get cost-of-living adjustments', 'Small business owners who can raise prices immediately'] },
        { q: 'What is Quantitative Easing (QE)?', a: 'A central bank policy of creating new money to buy government bonds and financial assets, expanding the money supply', wrong: ['A method of reducing government debt by printing less money', 'A technique for making international trade easier between nations', 'A system where banks lend money to each other at zero interest'] },
        { q: 'What is the "repo market" and why does it matter for understanding fiat fragility?', a: 'A short-term lending market where banks borrow overnight using bonds as collateral — its near-collapse in 2019 showed how fragile the financial system is', wrong: ['A marketplace where banks sell repossessed homes and cars', 'A government repository that stores physical gold reserves', 'A database where central banks track all global money transfers'] },
        { q: 'What is the "Eurodollar system"?', a: 'US dollars held and created by banks outside the United States, forming a massive shadow banking system beyond any single government\'s control', wrong: ['The system that converts Euros to Dollars at European airports', 'A European Central Bank program to replace the Dollar with the Euro', 'A cryptocurrency backed by both Euros and US Dollars'] },
        { q: 'What does the decline of the "petrodollar" system mean for global finance?', a: 'As oil-producing nations move away from pricing oil exclusively in US dollars, the dollar\'s global reserve currency status weakens', wrong: ['Oil companies are switching to mining Bitcoin instead of drilling', 'The petrodollar decline means oil is becoming worthless', 'European nations are being forced to buy oil with gold instead of dollars'] },
    ],

    'regulation_pt3': [
        { q: 'In the US, what is the key difference between SEC and CFTC jurisdiction over digital assets?', a: 'The SEC regulates securities (most altcoins), while the CFTC classifies and regulates Bitcoin as a commodity', wrong: ['The SEC handles Bitcoin and the CFTC handles all altcoins', 'Both agencies have identical authority over all digital assets', 'The CFTC only regulates physical commodities like gold and oil, not digital assets'] },
        { q: 'What is the Howey Test and how does it apply to Bitcoin?', a: 'A legal test for whether something is a security — Bitcoin is NOT a security because there is no central team or enterprise from which investors expect profits', wrong: ['The Howey Test is a technical test that measures blockchain decentralization', 'Bitcoin fails the Howey Test, which is why the SEC regulates it as a security', 'The Howey Test determines the tax rate on cryptocurrency investments'] },
        { q: 'What is the "Travel Rule" in cryptocurrency regulation?', a: 'A FATF requirement that financial institutions must share sender and receiver identity information for transactions above a certain threshold', wrong: ['A rule that Bitcoin can only be sent within the same country', 'A regulation requiring Bitcoin miners to register their physical location', 'A law that prohibits carrying hardware wallets across international borders'] },
        { q: 'What is the FATF and why is it significant for Bitcoin privacy?', a: 'The Financial Action Task Force — an international body pushing global KYC/AML standards that threaten the privacy of self-custodial Bitcoin use', wrong: ['A Bitcoin mining consortium that sets difficulty levels worldwide', 'A nonprofit organization that promotes Bitcoin adoption in developing countries', 'A technology standard for faster Bitcoin transaction processing'] },
        { q: 'How have US states approached Bitcoin regulation differently from the federal government?', a: 'Several states have passed pro-Bitcoin laws, including strategic Bitcoin reserve bills and exemptions from state money transmitter licenses', wrong: ['All 50 states have identical cryptocurrency regulations matching federal law', 'States are prohibited from creating any cryptocurrency-related legislation', 'Most states have banned Bitcoin mining due to energy concerns'] },
    ],

    'rollups': [
        { q: 'What is a validity rollup on Bitcoin?', a: 'A system that bundles transactions off-chain and posts a cryptographic proof (ZK-SNARK/STARK) to the base layer', wrong: ['A sidechain that copies Bitcoin\'s consensus rules exactly', 'A Lightning Network channel that batches multiple payments', 'A mining pool that combines block templates from multiple miners'] },
        { q: 'How do sovereign rollups differ from validity rollups?', a: 'Users verify the rollup state themselves instead of relying on the base layer to enforce validity', wrong: ['Sovereign rollups are governed by a central authority while validity rollups are decentralized', 'Sovereign rollups use proof-of-stake while validity rollups use proof-of-work', 'Sovereign rollups can only process 7 transactions per second'] },
        { q: 'How could BitVM enable rollups on Bitcoin?', a: 'By allowing fraud proofs to verify rollup state transitions without new opcodes', wrong: ['By adding a Turing-complete virtual machine directly to Bitcoin\'s consensus layer', 'By creating a new sidechain specifically designed for rollup validation', 'By replacing the existing Script language with Solidity'] },
        { q: 'How do Bitcoin rollups differ from sidechains?', a: 'Rollups post proofs or data to Bitcoin\'s base layer, inheriting its security guarantees', wrong: ['Rollups use a separate token for gas fees while sidechains use BTC', 'Rollups require a federation of signers while sidechains are fully trustless', 'Rollups are faster than sidechains but can only handle simple transfers'] },
        { q: 'What is the main trust assumption difference between a validity rollup and an optimistic rollup?', a: 'Validity rollups prove correctness upfront with math; optimistic rollups assume correctness unless someone submits a fraud proof', wrong: ['Validity rollups trust a federation; optimistic rollups trust miners', 'Validity rollups require a soft fork; optimistic rollups work today', 'Validity rollups are centralized; optimistic rollups are fully decentralized'] },
    ],

    'self-custody_pt3': [
        { q: 'What is Sparrow Wallet known for in multisig coordination?', a: 'A desktop wallet that provides deep UTXO management and easy multisig setup with hardware signers', wrong: ['A mobile-only wallet with cloud-based multisig', 'A hardware wallet that stores multiple seed phrases', 'A browser extension for Lightning payments'] },
        { q: 'What is a PSBT (Partially Signed Bitcoin Transaction)?', a: 'A standardized format that allows a transaction to be constructed and signed by multiple parties or devices', wrong: ['A type of Bitcoin transaction that only sends partial amounts', 'A privacy feature that hides the sender of a transaction', 'A special transaction type only used by mining pools'] },
        { q: 'What are output descriptors in Bitcoin?', a: 'A standardized way to describe how a wallet derives addresses, making wallet recovery more reliable', wrong: ['Labels attached to transaction outputs for bookkeeping', 'A list of all outputs ever created on the blockchain', 'Metadata added to transactions by mining pools'] },
        { q: 'Why is multisig generally preferred over Shamir Secret Sharing (SSS) for Bitcoin custody?', a: 'Multisig never requires reconstructing the full private key in one place, while SSS does during signing', wrong: ['Multisig is cheaper in transaction fees than SSS', 'SSS requires more hardware devices than multisig', 'Multisig was invented before SSS and is better documented'] },
        { q: 'What is Nunchuk known for in the Bitcoin wallet space?', a: 'A wallet focused on easy collaborative multisig setup and inheritance planning', wrong: ['The first wallet to support Lightning payments', 'A hardware wallet designed for cold storage only', 'A privacy-focused wallet using CoinJoin by default'] },
    ],

    'self-custody_pt4': [
        { q: 'Why is titanium preferred over stainless steel for seed phrase backup plates?', a: 'Titanium has a higher melting point and better corrosion resistance, surviving more extreme conditions', wrong: ['Titanium is cheaper and more widely available', 'Titanium plates can store more words than steel plates', 'Titanium is the only metal that cannot be detected by metal detectors'] },
        { q: 'What is a passphrase (25th word) in Bitcoin seed phrase security?', a: 'An additional user-chosen word added to the seed phrase that creates an entirely different wallet', wrong: ['The password to unlock a hardware wallet', 'A recovery phrase provided by the wallet manufacturer', 'An encryption key for the blockchain data'] },
        { q: 'Why is geographic distribution important for Bitcoin seed phrase storage?', a: 'It protects against localized disasters like fire, flood, or theft destroying all copies at once', wrong: ['It makes the wallet faster by connecting to nearby nodes', 'It is required by law in most jurisdictions', 'It increases the entropy of the seed phrase'] },
        { q: 'What is a dead man\'s switch in Bitcoin inheritance planning?', a: 'A mechanism that automatically triggers key release or notifications if the owner fails to check in periodically', wrong: ['A physical switch on a hardware wallet that wipes it if tampered with', 'A transaction that sends all Bitcoin to a burn address after a set time', 'A self-destruct feature built into Bitcoin Core'] },
        { q: 'What is the biggest risk of not having a Bitcoin inheritance plan?', a: 'Your Bitcoin could be permanently lost because no one else can access your keys', wrong: ['The government will automatically seize your Bitcoin', 'Your Bitcoin will be redistributed to other network participants', 'Your Bitcoin will lose value over time without an active wallet'] },
    ],
    ...
    'softwar_pt2': [
        ...
        { q: 'According to Softwar, what does proof of work project?', a: 'Physical kinetic power through cyberspace', wrong: ['Financial return on investment', 'Social consensus voting', 'Network speed capabilities'] },
        { q: 'How does Softwar view Bitcoin for nation-states?', a: 'A strategic defense technology', wrong: ['A replacement for fiat currency', 'A consumer payment system', 'An unregulated security'] },
        { q: 'What energy concept is central to the Softwar thesis?', a: 'Using energy to secure cyberspace property', wrong: ['Reducing carbon emissions', 'Solar power adoption', 'Energy arbitrage trading'] },
    ],

    'spv': [
        { q: 'What does SPV (Simplified Payment Verification) allow a lightweight client to do?', a: 'Verify that a transaction is included in a block by checking block headers and Merkle proofs without downloading the full blockchain', wrong: ['Mine Bitcoin blocks using minimal computing power', 'Send transactions without paying any fees', 'Run a full node on a mobile phone with limited storage'] },
        { q: 'What privacy problem did the original SPV bloom filters (BIP-37) have?', a: 'They leaked information about which addresses the wallet was interested in to the serving node', wrong: ['They revealed the user\'s IP address to every node on the network', 'They broadcast the user\'s seed phrase as part of the filter data', 'They required users to share their real name with the node operator'] },
        { q: 'What do BIP-157 and BIP-158 (compact block filters) improve over bloom filters?', a: 'The server sends pre-computed filters for each block, so the client downloads them without revealing which addresses it cares about', wrong: ['They compress the entire blockchain into a single downloadable file', 'They allow nodes to verify transactions without any network connection', 'They replace Merkle proofs with zero-knowledge proofs for faster verification'] },
        { q: 'What is Neutrino in the context of Bitcoin light clients?', a: 'A light client protocol that implements BIP-157/158 compact block filters for private SPV', wrong: ['A privacy coin built as a Bitcoin sidechain', 'A mining algorithm designed for mobile phones', 'A wallet that runs a full node in the cloud on your behalf'] },
        { q: 'What is the main security tradeoff of SPV compared to running a full node?', a: 'SPV clients trust that miners are honest and producing valid blocks, since they don\'t verify all transactions themselves', wrong: ['SPV clients cannot receive Bitcoin, only send it', 'SPV clients must pay higher transaction fees than full nodes', 'SPV clients can only connect to one node at a time'] },
    ],

    'submarine_swap_pt2': [
        { q: 'What is a Loop Out in Lightning?', a: 'Moving funds from Lightning back to on-chain Bitcoin', wrong: ['Opening a new channel', 'Receiving Lightning payments', 'Closing all channels'] },
        { q: 'What is Boltz Exchange known for?', a: 'Trustless Bitcoin on-chain/Lightning swaps', wrong: ['Mining pool services', 'Centralized exchange trading', 'Hardware wallet sales'] },
        { q: 'What makes submarine swaps trustless?', a: 'Atomic execution using hash time-locked contracts', wrong: ['Multi-signature escrow', 'Trusted third-party verification', 'Proof of stake consensus'] },
        { q: 'How do submarine swaps differ from atomic swaps?', a: 'Submarine swaps move between layers of the same chain', wrong: ['Submarine swaps require trust', 'Atomic swaps are faster', 'Submarine swaps use different coins'] },
        { q: 'What enables cross-chain atomic swaps without intermediaries?', a: 'Hash time-locked contracts with hashlock and timelock', wrong: ['Trusted custodians', 'Centralized exchanges', 'Smart contract platforms only'] },
    ],

    'tail_emission': [
        { q: 'What is "tail emission" in the context of cryptocurrency monetary policy?', a: 'A small, perpetual block reward that never drops to zero, ensuring miners always receive some subsidy beyond transaction fees', wrong: ['The final Bitcoin that will ever be mined, expected around 2140', 'The process of burning unsold Bitcoin to reduce supply', 'A mechanism that increases block rewards over time to incentivize mining'] },
        { q: 'Which major cryptocurrency implements tail emission as part of its monetary policy?', a: 'Monero — it has a permanent minimum block reward of 0.6 XMR per block', wrong: ['Bitcoin — it was added in the Taproot upgrade', 'Ethereum — it was introduced in the merge to proof of stake', 'Litecoin — it was activated after its third halving'] },
        { q: 'What concern about Bitcoin\'s long-term security does the tail emission debate address?', a: 'Whether transaction fees alone will be sufficient to incentivize miners to secure the network after block rewards approach zero', wrong: ['Whether Bitcoin\'s encryption will remain secure against quantum computers', 'Whether the 21 million supply cap should be increased', 'Whether nodes will have enough storage for the growing blockchain'] },
        { q: 'What is the main argument AGAINST adding tail emission to Bitcoin?', a: 'It would break the absolute 21 million supply cap — Bitcoin\'s most fundamental property — and destroy its credibility as sound money', wrong: ['Tail emission would make mining too profitable and attract too many miners', 'It would speed up transaction confirmation times too much', 'Tail emission is technically impossible to implement on Bitcoin\'s codebase'] },
        { q: 'What is the "fee market sufficiency" argument against the need for tail emission?', a: 'As Bitcoin adoption grows and block space remains scarce, transaction fees will naturally rise to provide adequate miner revenue', wrong: ['Miners will voluntarily work for free because they believe in Bitcoin\'s mission', 'The government will subsidize mining to keep the network secure', 'New Bitcoin will be discovered on the network, similar to finding new gold deposits'] },
    ],

    'time_preference_pt3': [
        { q: 'According to Saifedean Ammous in "The Bitcoin Standard," what is the relationship between sound money and time preference?', a: 'Sound money lowers time preference, encouraging long-term thinking, saving, and investment in civilization-building projects', wrong: ['Sound money raises time preference because people rush to spend appreciating currency', 'There is no relationship between the type of money and time preference', 'Sound money eliminates time preference entirely, making people indifferent between present and future'] },
        { q: 'How does Ammous argue that low time preference connects to civilization building?', a: 'Civilizations advance when people invest in long-term projects (cathedrals, infrastructure, education) — enabled by money that holds value', wrong: ['Civilizations advance fastest when people spend money as quickly as possible', 'Low time preference leads to stagnation because nobody takes risks', 'Civilization building is unrelated to monetary systems'] },
        { q: 'What behavioral effect does fiat money have on time preference according to the Austrian economics perspective?', a: 'Fiat raises time preference — people spend and borrow more because inflation punishes savers and rewards debtors', wrong: ['Fiat lowers time preference because stable inflation makes long-term planning easier', 'Fiat has no effect on time preference — it is determined purely by culture', 'Fiat encourages saving because interest rates compensate for inflation'] },
        { q: 'How does the concept of delayed gratification relate to Bitcoin and sound money?', a: 'Sound money like Bitcoin rewards delayed gratification — holding purchasing power encourages patience and long-term planning', wrong: ['Delayed gratification is irrelevant to monetary systems', 'Bitcoin punishes delayed gratification because its price is too volatile', 'Sound money discourages delayed gratification by making everything too expensive'] },
        { q: 'What is the key difference between how fiat and Bitcoin influence personal financial behavior?', a: 'Fiat incentivizes consumption and debt (spend now before money loses value); Bitcoin incentivizes saving and prudent investment', wrong: ['Both systems produce identical financial behavior — people always spend the same amount', 'Bitcoin makes people spend more because they feel wealthy from price appreciation', 'Fiat encourages saving while Bitcoin encourages spending due to deflationary pressure'] },
    ],

    'vbyte': [
        { q: 'What is a virtual byte (vbyte) in Bitcoin?', a: 'A unit of measurement where 1 vbyte equals 4 weight units, used to calculate transaction fees after SegWit', wrong: ['A compressed byte format that reduces transaction size by 75%', 'The amount of data a Bitcoin node can process per second', 'A unit measuring the electricity cost of validating a transaction'] },
        { q: 'How did SegWit change the way Bitcoin transaction fees are calculated?', a: 'Fees are now based on weight units (or vbytes) instead of raw bytes, giving witness data a discount', wrong: ['SegWit made all transactions free for the first year after activation', 'SegWit replaced fee-based priority with a first-come-first-served queue', 'SegWit doubled the base fee rate for all transaction types'] },
        { q: 'What is the witness discount ratio in SegWit?', a: 'Witness data costs 1 weight unit per byte while non-witness data costs 4 weight units per byte', wrong: ['Witness data is free and non-witness data costs 2 weight units per byte', 'Both witness and non-witness data cost 2 weight units per byte', 'Witness data costs 3 weight units per byte while non-witness data costs 4'] },
        { q: 'Why is understanding vbytes important for optimizing Bitcoin transaction costs?', a: 'Moving more data into the witness section (e.g., using SegWit addresses) reduces the fee you pay per transaction', wrong: ['Vbytes determine how many confirmations a transaction needs', 'Using fewer vbytes increases the mining reward for the block', 'Transactions with more vbytes get higher priority in the mempool'] },
