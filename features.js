...
const HIDDEN_BADGES = [
    ...
    { id: 'first_post', name: 'Town Crier', emoji: '📣', pts: 100, desc: 'Make your first forum post', hint: 'Post in the Community Forum!', hidden: false, check: function() { return parseInt(localStorage.getItem('btc_forum_post_count') || '0') >= 1 || (typeof currentUser !== 'undefined' && currentUser && currentUser.forumPosts >= 1); } },
...
function showBadgeUnlock(badge) {
    // If Nacho is busy (Q&A, voice input, reading answer), queue for later
...
function _showBadgeUnlock(badge) {
    // Minor hidden badges (< 200 pts): just a toast, no fullscreen overlay
    if (badge.pts && badge.pts < 200) {
        badgeShowing = false;
        if (typeof showToast === 'function') {
            showToast(badge.emoji + ' Hidden Badge: ' + badge.name + ' (+' + badge.pts + ' pts)');
        }
        return;
    }

    // Major hidden badges: full celebration
    badgeShowing = true;
...
setInterval(checkHiddenBadges, 30000);
