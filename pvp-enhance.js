...
!(function () {
      ...
      m.addedNodes.forEach(function (node) {
        if (node.nodeType === 1 && node.textContent) {
          ...
          if (node.textContent.indexOf("Both wrong!") !== -1) {
            var rerollDiv = node.querySelector
              ? node.querySelector('div[style*="font-size:3rem"]')
              : null;
            if (rerollDiv) {
              rerollDiv.textContent = "⚡";
            }
            var textDiv = node.querySelector
              ? node.querySelector('div[style*="font-weight:800"]')
              : null;
            ...
            var subText = node.querySelector
              ? node.querySelector('div[style*="text-faint"]')
              : null;
            if (
              subText &&
              subText.textContent.indexOf("Next question") !== -1
            ) {
  ...
  var _speedObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (m) {
      m.addedNodes.forEach(function (node) {
        ...
        if (
          node.id === "pvpOverlay" ||
          (node.querySelector && node.querySelector("#pvpQuestion"))
        ) {
        ...
        if (
          node.textContent &&
          (node.textContent.indexOf("Correct! Waiting") !== -1 ||
            node.textContent.indexOf("Wrong!") !== -1 ||
            node.textContent.indexOf("Time's up") !== -1)
        ) {
          var elapsed = Date.now() - _questionShownAt;
          if (
            elapsed < 3000 &&
            elapsed > 0 &&
            node.textContent.indexOf("Correct") !== -1
          ) {
            var speedBadge = document.createElement("div");
            speedBadge.style.cssText =
              "text-align:center;margin-top:8px;font-size:0.85rem;font-weight:800;color:#f7931a;animation:pvpSlideIn 0.3s ease;";
            speedBadge.textContent =
              "⚡ LIGHTNING FAST! (" + (elapsed / 1000).toFixed(1) + "s)";
  ...
  function _showTauntBubble(emoji, isMine) {
    ...
    bubble.style.cssText =
      "position:fixed;" +
      (isMine ? "bottom:180px;right:20px;" : "bottom:180px;left:20px;") +
    ...
    if (
      !isMine &&
      typeof window.canPlaySound === "function" &&
      window.canPlaySound()
    ) {
      try {
  ...
  var _tauntObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (m) {
      m.addedNodes.forEach(function (node) {
        ...
        if (
          node.textContent &&
          node.textContent.indexOf("Waiting for opponent") !== -1 &&
          !document.getElementById("pvpTauntBar")
        ) {
          var bar = document.createElement("div");
          bar.id = "pvpTauntBar";
          bar.style.cssText =
            "display:flex;justify-content:center;gap:8px;margin-top:12px;animation:pvpSlideIn 0.3s ease;";
          TAUNTS.forEach(function (t) {
            var btn = document.createElement("button");
            btn.textContent = t;
            btn.style.cssText =
              "font-size:1.5rem;background:var(--card-bg,#1a1a2e);border:1px solid var(--border);border-radius:50%;width:44px;height:44px;cursor:pointer;transition:0.2s;touch-action:manipulation;display:flex;align-items:center;justify-content:center;";
  ...
  window._pvpSaveMatchHistory = function (
    opponentName,
    myScore,
    oppScore,
    won,
  ...
  var _historyObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (m) {
      m.addedNodes.forEach(function (node) {
        ...
        if (
          node.textContent &&
          (node.textContent.indexOf("VICTORY!") !== -1 ||
            node.textContent.indexOf("DEFEAT") !== -1)
        ) {
          var isWin = node.textContent.indexOf("VICTORY!") !== -1;
          // Extract scores from the result screen
          var scoreEls = node.querySelectorAll
          ...
          // Get opponent name
          var oppNameEls = node.querySelectorAll
            ? node.querySelectorAll(
                'div[style*="font-weight:700"][style*="font-size:0.85rem"]',
              )
            : [];
          var oppName =
            oppNameEls.length >= 2 ? oppNameEls[1].textContent : "Unknown";
  ...
  function _injectMatchHistory() {
    ...
    var history;
    try {
      history = JSON.parse(localStorage.getItem("btc_pvp_history") || "[]");
    } catch (e) {
      return;
    }
    ...
    var html =
      '<div id="pvpMatchHistory" style="background:var(--card-bg,#1a1a2e);border:1px solid var(--border);border-radius:16px;padding:14px;margin-bottom:16px;text-align:left;">' +
    ...
    history.slice(0, 5).forEach(function (match) {
      ...
      html +=
        ...
        '<div style="font-size:0.8rem;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">vs ' +
        _escHtml(match.opponent) +
        "</div>" +
        '<div style="font-size:0.65rem;color:var(--text-faint);">' +
        ago +
        "</div>" +
        "</div>" +
        '<div style="font-size:0.85rem;font-weight:800;color:' +
        color +
        ';">' +
        match.myScore +
        "-" +
  ...
  style.textContent =
    "@keyframes pvpTauntFloat { 0% { opacity:1; transform:translateY(0) scale(1); } 50% { opacity:1; transform:translateY(-40px) scale(1.3); } 100% { opacity:0; transform:translateY(-80px) scale(0.8); } }";
      ...
      .onSnapshot(
        function (snap) {
          ...
          if (data.lastTaunt && data.lastTaunt.ts > _lastSeenTauntTs) {
            ...
            if (
              auth.currentUser &&
              data.lastTaunt.uid !== auth.currentUser.uid
            ) {
  ...
  console.log(
    "[PVP] Enhancements loaded — Sudden Death, Speed Bonus, Taunts, Match History",
  );
})();
