    ...
    db.collection('public_profiles').doc(uid).get().then(function(doc) {
        ...
        var avatarHtml = _safePickUrl
            ? '<div style="position:relative;display:inline-block;margin-bottom:8px;">' +
                ...
                '</div>'
            : '<div style="display:inline-flex;align-items:center;justify-content:center;margin-bottom:8px;width:72px;height:72px;border-radius:50%;border:2px solid var(--border);background:var(--card-bg);font-size:1.8rem;' + (_hasProfileFrame ? 'box-shadow:0 0 0 3px #f7931a,0 0 16px rgba(247,147,26,0.4);border-color:#f7931a;' : '') + '">' +
                lvl.emoji +
                '</div>';
