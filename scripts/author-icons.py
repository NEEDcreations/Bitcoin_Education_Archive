#!/usr/bin/env python3
"""Author the Bitcoin Education Archive custom SVG icon set.

Writes assets/icons/<name>.svg. Design language:
  - 24x24 grid, stroke-based, 1.75px, round caps/joins
  - currentColor by default (adapts to light/dark theme)
  - fixed semantic colors for status/brand icons
  - selective duotone fills (opacity ~.12) for depth
Re-run after editing geometry: python3 scripts/author-icons.py
"""
import math, os, re
...
ORANGE = '#f97316'; BTC = '#f7931a'; BLUE = '#38bdf8'; PURPLE = '#a78bfa'


def cog_path(cx=12, cy=12, r_out=9.0, r_in=7.0, teeth=8):
    """Generate a rounded-tooth cog outline."""
    pts = []
    step = 360 / teeth
    half_tooth = step * 0.30
    half_gap = step * 0.24
    for i in range(teeth):
        a = i * step
        for ang, r in ((a - half_tooth, r_out), (a + half_tooth, r_out),
                       (a + half_tooth + (half_gap * 0.6), r_in),
                       (a + step - half_tooth - (half_gap * 0.6), r_in)):
...
# name -> dict(color=stroke color or None for currentColor,
#              inner=svg elements, root=extra root attr overrides)
ICONS = {}

def icon(name, inner, color=None, root=None):
    ICONS[name] = {'inner': inner, 'color': color, 'root': root or {}}
...
icon('deer',
     ...
     '<path d="M14.8 9.1l2-4.6 1.9 1.4"/><path d="M16.8 4.5l-.5-2.3"/>'
     '<path d="M8 11.2C6.9 10.2 5.4 10 4.3 10.6c.6 1.1 2 1.6 3.5 1.4"/>'
     '<path d="M16 11.2c1.1-1 2.6-1.2 3.7-.6-.6 1.1-2 1.6-3.5 1.4"/>'
     '<path d="M12 9c2.6 0 4.4 1.9 4.4 4.5 0 2.9-1.8 5.1-4.4 5.1s-4.4-2.2-4.4-5.1C7.6 10.9 9.4 9 12 9Z"/>'
     '<circle cx="10.2" cy="13.5" r=".85" fill="currentColor" stroke="none"/>'
     '<circle cx="13.8" cy="13.5" r=".85" fill="currentColor" stroke="none"/>'
     '<ellipse cx="12" cy="16.3" rx="1.25" ry=".9" fill="currentColor" stroke="none"/>')
...
icon('arrow-left', '<path d="M19.75 12h-15"/><path d="M10.5 6.25 4.75 12l5.75 5.75"/>')
icon('arrow-up-right', '<path d="M7 17 17 7"/><path d="M9 7h8v8"/>')
icon('refresh',
     '<path d="M20.75 12A8.75 8.75 0 1 1 18 5.6"/>'
     '<path d="M20.75 3.5v4.4h-4.4"/>')

# ---------- Status / feedback ----------
icon('check', '<path d="m4.5 12.75 5 5L20 6.75"/>')
icon('check-circle',
     f'<circle cx="12" cy="12" r="9" fill="{GREEN}" fill-opacity=".12"/>'
     '<path d="m7.9 12.4 2.9 2.9 5.6-6"/>',
     color=GREEN)
icon('x', '<path d="M6 6l12 12"/><path d="M18 6 6 18"/>')
icon('x-circle',
     f'<circle cx="12" cy="12" r="9" fill="{RED}" fill-opacity=".1"/>'
     '<path d="m9.2 9.2 5.6 5.6"/><path d="m14.8 9.2-5.6 5.6"/>',
     color=RED)
icon('ban', f'<circle cx="12" cy="12" r="8.75"/><path d="M5.9 5.9l12.2 12.2"/>', color=RED)
icon('siren',
     '<path d="M7.25 16.5v-4.25a4.75 4.75 0 0 1 9.5 0v4.25"/>'
     f'<rect x="4.5" y="16.5" width="15" height="3.5" rx="1.5" fill="{RED}" fill-opacity=".12"/>'
     '<path d="M12 2.5v1.9"/><path d="m4.9 5.4 1.5 1.2"/><path d="m19.1 5.4-1.5 1.2"/>',
     color=RED)
icon('alert',
     f'<path d="M10.5 4.1 2.9 17.3a1.72 1.72 0 0 0 1.5 2.6h15.2a1.72 1.72 0 0 0 1.5-2.6L13.5 4.1a1.73 1.73 0 0 0-3 0Z" fill="{AMBER}" fill-opacity=".12"/>'
     '<path d="M12 9.4v4.4"/><circle cx="12" cy="16.9" r=".4" fill="currentColor"/>',
     color=AMBER)
icon('help',
...
icon('hourglass',
     ...
     '<path d="M15.75 3.5v3c0 2.4-3.75 3.6-3.75 5.5s3.75 3.1 3.75 5.5v3"/>')

for dname, dcol in (('dot-red', '#ef4444'), ('dot-green', '#22c55e'),
                    ('dot-orange', '#f97316'), ('dot-purple', '#a855f7'),
                    ('dot-blue', '#3b82f6'), ('dot-yellow', '#eab308')):
    icon(dname,
         f'<circle cx="12" cy="12" r="8" fill="{dcol}" fill-opacity=".18" stroke="none"/>'
         f'<circle cx="12" cy="12" r="4.9" fill="{dcol}" stroke="none"/>',
         root={'stroke': 'none'})
...
# ---------- Communication ----------
icon('chat',
     '<path d="M20.9 11.5a8.9 8.9 0 0 1-8.9 8.9c-1.6 0-3.1-.42-4.4-1.15L3.1 20.3l1.05-4.4A8.9 8.9 0 1 1 20.9 11.5Z" fill="currentColor" fill-opacity=".07"/>')
icon('megaphone',
     '<path d="M4 10.2 16.6 4.4v15.2L4 13.8Z" fill="currentColor" fill-opacity=".08"/>'
     '<path d="M8.1 15v3a1.65 1.65 0 0 0 3.3 0v-1.5"/>'
     '<path d="M19.6 9.2a4.5 4.5 0 0 1 0 5.6"/>')
icon('mail',
     '<rect x="3.25" y="5.25" width="17.5" height="13.5" rx="2.4" fill="currentColor" fill-opacity=".06"/>'
     '<path d="m4.75 7.75 7.25 5.5 7.25-5.5"/>')
icon('bell',
     '<path d="M6.3 9.9a5.7 5.7 0 0 1 11.4 0c0 3.3.85 4.65 1.8 5.7H4.5c.95-1.05 1.8-2.4 1.8-5.7Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="M10.2 18.9a1.9 1.9 0 0 0 3.6 0"/>')
icon('upload',
     '<path d="M4 15.5v2.75A1.75 1.75 0 0 0 5.75 20h12.5A1.75 1.75 0 0 0 20 18.25V15.5"/>'
     '<path d="M12 14.75v-10"/><path d="M7.8 8.4 12 4.2l4.2 4.2"/>')
icon('smartphone',
     '<rect x="7" y="3.25" width="10" height="17.5" rx="2.4" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M10.8 17.6h2.4"/>')
icon('radio',
     '<circle cx="12" cy="12" r="1.7" fill="currentColor" stroke="none"/>'
     '<path d="M7.9 16.1a5.9 5.9 0 0 1 0-8.2"/><path d="M16.1 7.9a5.9 5.9 0 0 1 0 8.2"/>'
     '<path d="M5 19a10 10 0 0 1 0-14"/><path d="M19 5a10 10 0 0 1 0 14"/>')

# ---------- Docs / knowledge ----------
icon('clipboard',
     '<rect x="5.25" y="4.6" width="13.5" height="16.4" rx="2"/>'
     '<rect x="9" y="2.8" width="6" height="3.6" rx="1.2" fill="currentColor" fill-opacity=".12"/>'
     '<path d="M8.9 11.2h6.2"/><path d="M8.9 14.7h6.2"/><path d="M8.9 18.2h3.6"/>')
icon('book',
     '<path d="M12 6.9c-1.6-1.9-4-2.3-7.1-2.3v13.6c3.1 0 5.5.4 7.1 2.3 1.6-1.9 4-2.3 7.1-2.3V4.6c-3.1 0-5.5.4-7.1 2.3Z" fill="currentColor" fill-opacity=".06"/>'
     '<path d="M12 6.9v13.6"/>')
icon('library',
     '<rect x="4" y="4.4" width="3.7" height="15.2" rx="1"/>'
     '<rect x="9.4" y="4.4" width="3.7" height="15.2" rx="1"/>'
     '<path d="m15.9 5.5 3.5-.9 3.2 14.1-3.5.9Z" fill="currentColor" fill-opacity=".08"/>')
icon('scroll',
     '<path d="M8.4 3.6h9.1a2 2 0 0 1 2 2v12.8a2 2 0 0 1-2 2H8.6a2.2 2.2 0 0 1-2.2-2.2V5.8"/>'
     '<path d="M8.4 3.6a2.2 2.2 0 0 0-2 2.2V8h3.4"/>'
     '<path d="M10 9.9h6.4"/><path d="M10 13.1h6.4"/><path d="M10 16.3h4"/>')
icon('pencil',
     '<path d="M17.2 3.7a2.16 2.16 0 0 1 3.1 3.1L8.6 18.5 4 20l1.5-4.6Z" fill="currentColor" fill-opacity=".08"/>'
     '<path d="m14.7 6.2 3.1 3.1"/>')
icon('file',
     '<path d="M6.25 3.75h7l4.5 4.5v12.25a1.5 1.5 0 0 1-1.5 1.5h-10a1.5 1.5 0 0 1-1.5-1.5V5.25a1.5 1.5 0 0 1 1.5-1.5Z"/>'
     '<path d="M13.25 3.75v4.5h4.5"/>')
icon('news',
     '<rect x="3.75" y="5.1" width="16.5" height="13.8" rx="2"/>'
     '<path d="M7.1 8.7h9.8"/>'
     '<rect x="7.1" y="11.6" width="4.1" height="4" rx=".6" fill="currentColor" fill-opacity=".12"/>'
     '<path d="M14 12.1h2.9"/><path d="M14 15.2h2.9"/>')
icon('bookmark',
     '<path d="M7.25 4.25h9.5v16.1L12 16.9l-4.75 3.45Z" fill="currentColor" fill-opacity=".1"/>')
icon('pin',
     '<path d="M12 3.4a3.6 3.6 0 0 1 3.6 3.6c0 1.2-.6 2.25-1.5 2.9l1.8 3.8H8.1l1.8-3.8a3.6 3.6 0 0 1 1.6-6.5H12Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="M12 13.7v6.7"/>')
icon('map-pin',
     '<path d="M12 21.1s-7.1-6.3-7.1-11.1a7.1 7.1 0 0 1 14.2 0c0 4.8-7.1 11.1-7.1 11.1Z" fill="currentColor" fill-opacity=".08"/>'
     '<circle cx="12" cy="9.9" r="2.5"/>')
icon('grad-cap',
     '<path d="M2.75 9.3 12 4.5l9.25 4.8L12 14.1Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="M6.5 11.6v4.5c0 1.25 2.45 2.5 5.5 2.5s5.5-1.25 5.5-2.5v-4.5"/>'
     '<path d="M21.25 9.3v4.7"/>')
icon('brain',
     '<path d="M12 4.2c-1-1.2-2.9-1.4-4.1-.4-1 .8-1.4 2-1 3.1-1.3.4-2.2 1.6-2.2 3 0 .9.4 1.8 1 2.4-.8.7-1.2 1.8-1 2.9.3 1.3 1.4 2.3 2.7 2.4.1 1.4 1.3 2.5 2.7 2.5.7 0 1.4-.3 1.9-.8.5.5 1.2.8 1.9.8 1.4 0 2.6-1.1 2.7-2.5 1.3-.1 2.4-1.1 2.7-2.4.2-1.1-.2-2.2-1-2.9.6-.6 1-1.5 1-2.4 0-1.4-.9-2.6-2.2-3 .4-1.1 0-2.3-1-3.1-1.2-1-3.1-.8-4.1.4Z" fill="currentColor" fill-opacity=".07"/>'
     '<path d="M12 4.6v15.7"/>'
     '<path d="M12 9.2c-.9.3-1.9 0-2.4-.8"/><path d="M12 13.6c.9.3 1.9 0 2.4-.8"/>')
icon('calendar',
     '<rect x="4" y="5.4" width="16" height="14.6" rx="2" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M8.4 3.4v3.6"/><path d="M15.6 3.4v3.6"/><path d="M4 10h16"/>')
icon('clock',
     '<circle cx="12" cy="13" r="7.2"/>'
     '<path d="M12 9.6V13l2.5 1.8"/>'
     '<path d="m5 6.3 2.7-2.2"/><path d="m19 6.3-2.7-2.2"/>')
icon('stopwatch',
     '<circle cx="12" cy="13.4" r="6.9"/>'
     '<path d="M12 6.5V4.2"/><path d="M9.9 4.2h4.2"/>'
     '<path d="M12 13.4l2.5-2.5"/><path d="m18.3 7.5 1.2-1.2"/>')

# ---------- Security ----------
icon('lock',
     '<rect x="5.25" y="10.4" width="13.5" height="9.9" rx="2.2" fill="currentColor" fill-opacity=".08"/>'
     '<path d="M8.25 10.4V7.75a3.75 3.75 0 0 1 7.5 0v2.65"/>'
     '<circle cx="12" cy="15.3" r="1.4" fill="currentColor" stroke="none"/>')
icon('key',
     '<circle cx="8" cy="15.6" r="3.9"/>'
     '<path d="M10.9 12.8 19.6 4.1"/><path d="m16.9 6.7 2.1 2.1"/><path d="m14.3 9.3 1.8 1.8"/>')
icon('shield',
     '<path d="M12 3.2 5 5.95v5.3c0 4.6 3 7.7 7 9.55 4-1.85 7-4.95 7-9.55v-5.3Z" fill="currentColor" fill-opacity=".08"/>'
     '<path d="m9.2 11.9 1.95 1.95 3.65-3.8"/>')
icon('spy',
     '<path d="M4 12.75h16"/>'
     '<path d="M7.25 12.75 8.4 6.5c2.4-.85 4.8-.85 7.2 0l1.15 6.25"/>'
     '<circle cx="8.4" cy="16.4" r="2.35"/><circle cx="15.6" cy="16.4" r="2.35"/>'
     '<path d="M10.75 16.4h2.5"/>')
icon('eye',
     '<path d="M2.75 12S6.25 5.75 12 5.75 21.25 12 21.25 12 17.75 18.25 12 18.25 2.75 12 2.75 12Z" fill="currentColor" fill-opacity=".06"/>'
     '<circle cx="12" cy="12" r="2.9"/>')

# ---------- Tools / building ----------
icon('pickaxe',
     '<path d="M3.9 9.9C5.9 5.8 8.7 3.7 12 3.7s6.1 2.1 8.1 6.2c-.7.3-1.4.5-2.1.6C16.6 7.6 14.4 6.4 12 6.4s-4.6 1.2-6 4.1c-.7-.1-1.4-.3-2.1-.6Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="M12 6.6v14.3"/>')
icon('swords',
     '<path d="M4.2 4.2 15.5 15.5"/><path d="M19.8 4.2 8.5 15.5"/>'
     '<path d="m13.6 17.4 3.8-3.8"/><path d="m6.6 13.6 3.8 3.8"/>'
     '<path d="M16.2 20 14.5 18.3"/><path d="m7.8 20 1.7-1.7"/>')
# 🛠️ maps to wrench (no separate tools icon)
icon('wrench',
     '<path d="M20.4 6.9a5.1 5.1 0 0 1-6.9 6L7 19.4a1.9 1.9 0 0 1-2.7-2.7l6.5-6.5a5.1 5.1 0 0 1 6-6.9L13.9 6.2l.5 3.1 3.1.5Z" fill="currentColor" fill-opacity=".08"/>')
icon('hammer',
     '<path d="M4.1 8.1 8.3 4l5 1.4 2.3 2.3-4.3 4.3-2.3-2.3-1.2 1.1Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="m12.4 11 6.9 6.9a1.5 1.5 0 0 1-2.1 2.1l-6.9-6.9"/>')
icon('gear',
     f'<path d="{cog_path()}" fill="currentColor" fill-opacity=".07"/>'
     '<circle cx="12" cy="12" r="3.1"/>')
icon('toolbox',
     '<rect x="3.75" y="9.4" width="16.5" height="9.9" rx="2" fill="currentColor" fill-opacity=".06"/>'
     '<path d="M9.25 9.4V7.7A1.7 1.7 0 0 1 10.95 6h2.1a1.7 1.7 0 0 1 1.7 1.7v1.7"/>'
     '<path d="M3.75 13.6h6.9m2.7 0h6.9"/>'
     '<rect x="10.65" y="12.3" width="2.7" height="2.7" rx=".6"/>')
icon('chains',
     '<rect x="8.75" y="3.4" width="6.5" height="8.1" rx="3.25"/>'
     '<rect x="8.75" y="12.5" width="6.5" height="8.1" rx="3.25"/>')
icon('cube',
     '<path d="M12 3.1 20 7.55v8.9L12 20.9 4 16.45v-8.9Z" fill="currentColor" fill-opacity=".06"/>'
     '<path d="m4 7.55 8 4.45 8-4.45"/><path d="M12 12v8.9"/>')
icon('bricks',
     '<rect x="3.5" y="5.5" width="17" height="13" rx="1.5" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M3.5 9.85h17"/><path d="M3.5 14.2h17"/>'
     '<path d="M9.2 5.5v4.35"/><path d="M14.8 5.5v4.35"/>'
     '<path d="M12 9.85v4.35"/><path d="M9.2 14.2v4.3"/><path d="M14.8 14.2v4.3"/>')
icon('scales',
     '<path d="M12 4.4v15.1"/><path d="M4.75 7.2h14.5"/><path d="M9.1 19.5h5.8"/>'
     '<path d="M7.4 7.4 4.8 12.8h5.2Z"/><path d="M4.8 12.8a2.6 2.6 0 0 0 5.2 0"/>'
...
icon('globe',
     ...
     '<path d="M12 3.25c-3.1 2.3-3.1 15.2 0 17.5"/>'
     '<path d="M3.7 9.4h16.6"/><path d="M3.7 14.6h16.6"/>')
icon('map',
     '<path d="M3.6 6.4 9 4.5l6 2 5.4-1.9v13l-5.4 1.9-6-2-5.4 1.9Z" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M9 4.5v13"/><path d="M15 6.5v13"/>')
icon('mountain',
     '<path d="M2.9 19.25 9.1 8.1l3.2 5.7 2.3-4.1 6.5 9.55Z" fill="currentColor" fill-opacity=".07"/>'
     '<path d="m7.5 11 1.6 1.7 1.5-1.6"/>')
icon('landmark',
     '<path d="M4.4 9.9 12 4.4l7.6 5.5Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="M6.4 12.4v4.9"/><path d="M10.2 12.4v4.9"/><path d="M13.8 12.4v4.9"/><path d="M17.6 12.4v4.9"/>'
     '<path d="M4.6 19.9h14.8"/>')
icon('home',
...
# ---------- Charts / money ----------
icon('chart',
     '<path d="M3.75 19.6h16.5"/>'
     '<path d="M6.6 19.6v-7" stroke-width="2.6"/><path d="M12 19.6V6.2" stroke-width="2.6"/><path d="M17.4 19.6v-9.5" stroke-width="2.6"/>')
icon('trend-up',
     '<path d="m3.75 17.25 5.75-5.75 3.5 3.5 7.25-7.5"/>'
     '<path d="M15.75 7.5h4.5V12"/>')
icon('trend-down',
...
icon('banknote',
     ...
     '<path d="M6.4 12h.01"/><path d="M17.6 12h.01"/>')
icon('card',
...
# ---------- Media ----------
icon('tv',
     '<rect x="3.5" y="7" width="17" height="12.4" rx="2" fill="currentColor" fill-opacity=".05"/>'
     '<path d="m8.5 3.5 3.5 3.5 3.5-3.5"/>')
icon('music',
     '<path d="M9 18.3V6.4l9-1.9v11.8"/>'
     '<circle cx="6.6" cy="18.3" r="2.4" fill="currentColor" fill-opacity=".15"/>'
     '<circle cx="15.6" cy="16.3" r="2.4" fill="currentColor" fill-opacity=".15"/>')
icon('headphones',
     '<path d="M4.5 14.4v-2a7.5 7.5 0 0 1 15 0v2"/>'
     '<rect x="4.5" y="13.6" width="3.5" height="6.2" rx="1.6" fill="currentColor" fill-opacity=".12"/>'
     '<rect x="16" y="13.6" width="3.5" height="6.2" rx="1.6" fill="currentColor" fill-opacity=".12"/>')
icon('guitar',
     '<circle cx="8.3" cy="15.7" r="4.7" fill="currentColor" fill-opacity=".06"/>'
     '<circle cx="11.6" cy="12" r="2.7" fill="currentColor" fill-opacity=".06"/>'
     '<circle cx="9.2" cy="14.8" r="1.35"/>'
     '<path d="m13.5 10.1 6-6"/><path d="m18.2 2.8 3 3"/>')
icon('mic',
     '<rect x="9.25" y="3.25" width="5.5" height="10" rx="2.75" fill="currentColor" fill-opacity=".1"/>'
     '<path d="M6 11.4a6 6 0 0 0 12 0"/><path d="M12 17.4v3.1"/><path d="M9.25 20.5h5.5"/>')
icon('volume',
     '<path d="M4.5 9.25h2.9l4.6-4v13.5l-4.6-4H4.5Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="M15.5 9a4.3 4.3 0 0 1 0 6"/><path d="M18.2 6.4a8.2 8.2 0 0 1 0 11.2"/>')
icon('volume-off',
     '<path d="M4.5 9.25h2.9l4.6-4v13.5l-4.6-4H4.5Z" fill="currentColor" fill-opacity=".1"/>'
     '<path d="m16.2 9.7 4.6 4.6"/><path d="m20.8 9.7-4.6 4.6"/>')
icon('disc',
     '<circle cx="12" cy="12" r="8.75" fill="currentColor" fill-opacity=".05"/>'
     '<circle cx="12" cy="12" r="2.75"/>'
     '<path d="M12 6.1a5.9 5.9 0 0 1 5.9 5.9"/>')
icon('pause',
     '<rect x="7.75" y="5.5" width="3" height="13" rx="1.4" fill="currentColor" fill-opacity=".12"/>'
     '<rect x="13.25" y="5.5" width="3" height="13" rx="1.4" fill="currentColor" fill-opacity=".12"/>')
icon('camera',
     '<rect x="3.5" y="7.25" width="17" height="12" rx="2.2" fill="currentColor" fill-opacity=".05"/>'
     '<circle cx="12" cy="13.1" r="3.4"/>'
     '<path d="m8.6 7.25 1.2-2.5h4.4l1.2 2.5"/>')
icon('clapper',
     '<rect x="3.75" y="10.2" width="16.5" height="9.2" rx="1.75" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M3.6 9.9 19.9 5.5l-.7-2.6L2.9 7.3Z"/>'
     '<path d="m7.7 8.85.9-2.9"/><path d="m12.2 7.6.9-2.9"/><path d="m16.6 6.4.9-2.9"/>')
icon('gamepad',
     '<path d="M7.1 8.4h9.8a4.8 4.8 0 0 1 4.75 5.55l-.55 3.1a2.45 2.45 0 0 1-4.25 1.15L15 16.4H9l-1.85 1.8A2.45 2.45 0 0 1 2.9 17.05l-.55-3.1A4.8 4.8 0 0 1 7.1 8.4Z" fill="currentColor" fill-opacity=".06"/>'
     '<path d="M8.4 10.9v3.4"/><path d="M6.7 12.6h3.4"/>'
...
icon('slot',
     '<rect x="3.9" y="5.9" width="16.2" height="12.6" rx="2" fill="currentColor" fill-opacity=".05"/>'
     '<rect x="6.3" y="8.9" width="3.2" height="6.1" rx=".8"/>'
     '<rect x="10.4" y="8.9" width="3.2" height="6.1" rx=".8"/>'
     '<rect x="14.5" y="8.9" width="3.2" height="6.1" rx=".8"/>'
     '<circle cx="7.9" cy="12" r=".5" fill="currentColor"/><circle cx="12" cy="12" r=".5" fill="currentColor"/><circle cx="16.1" cy="12" r=".5" fill="currentColor"/>')
icon('wheel',
...
# ---------- Commerce / misc ----------
icon('cart',
     '<path d="M3.9 4.9h2l2.25 10.6a1.75 1.75 0 0 0 1.7 1.4h6.9a1.75 1.75 0 0 0 1.7-1.35l1.65-6.55H7.1"/>'
     '<circle cx="10.1" cy="20" r="1.35"/><circle cx="16.5" cy="20" r="1.35"/>')
icon('search',
     '<circle cx="11" cy="11" r="7.25"/><path d="m16.4 16.4 4.35 4.35"/>')
icon('link',
     '<path d="M10.6 13.4a3.6 3.6 0 0 0 5.1 0l3.15-3.15a3.6 3.6 0 0 0-5.1-5.1L12 6.9"/>'
     '<path d="M13.4 10.6a3.6 3.6 0 0 0-5.1 0l-3.15 3.15a3.6 3.6 0 0 0 5.1 5.1L12 17.1"/>')
icon('trash',
     '<path d="M4.75 6.6h14.5"/>'
     '<path d="M9.5 6.6V5.1a1.5 1.5 0 0 1 1.5-1.5h2a1.5 1.5 0 0 1 1.5 1.5v1.5"/>'
     '<path d="m6.25 6.6.9 12.5a1.75 1.75 0 0 0 1.75 1.65h6.2a1.75 1.75 0 0 0 1.75-1.65l.9-12.5"/>'
     '<path d="m10.1 10.4.25 6.4"/><path d="m13.9 10.4-.25 6.4"/>')
icon('plus', '<path d="M12 5.25v13.5"/><path d="M5.25 12h13.5"/>')
icon('package',
     '<path d="M12 3.25 20 7.5v9L12 20.75 4 16.5v-9Z" fill="currentColor" fill-opacity=".05"/>'
     '<path d="m4 7.5 8 4.25L20 7.5"/><path d="M12 11.75v9"/><path d="m8 5.4 8.1 4.3"/>')
icon('save',
     '<path d="M4.75 6.5A1.75 1.75 0 0 1 6.5 4.75h9.3l3.45 3.45v9.3a1.75 1.75 0 0 1-1.75 1.75H6.5a1.75 1.75 0 0 1-1.75-1.75Z" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M8.25 4.75V9h7V4.75"/><circle cx="12" cy="14.6" r="2.1"/>')
icon('laptop',
     '<rect x="5" y="5.25" width="14" height="9" rx="1.5" fill="currentColor" fill-opacity=".06"/>'
     '<path d="M3.25 17.25 5 14.25h14l1.75 3a1 1 0 0 1-.9 1.5H4.15a1 1 0 0 1-.9-1.5Z"/>')
icon('monitor',
     '<rect x="3.5" y="4.75" width="17" height="11.5" rx="1.75" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M12 16.25v3"/><path d="M8.75 19.25h6.5"/>')
icon('robot',
     '<rect x="5.5" y="8" width="13" height="10" rx="2.4" fill="currentColor" fill-opacity=".05"/>'
     '<path d="M12 8V5.4"/><circle cx="12" cy="4.1" r="1.2"/>'
     '<circle cx="9.4" cy="12.4" r="1.15" fill="currentColor" stroke="none"/>'
     '<circle cx="14.6" cy="12.4" r="1.15" fill="currentColor" stroke="none"/>'
     '<path d="M9.75 15.4h4.5"/><path d="M5.5 11.9H3.75"/><path d="M18.5 11.9h1.75"/>')
icon('users',
     '<circle cx="9" cy="8.4" r="3.4"/>'
     '<path d="M3.5 19.6a5.5 5.5 0 0 1 11 0"/>'
     '<path d="M15.9 5.4a3.4 3.4 0 0 1 0 6.05"/>'
     '<path d="M17.4 14.3a5.5 5.5 0 0 1 3.1 5.3"/>')
icon('user',
     '<circle cx="12" cy="7.75" r="3.9"/>'
     '<path d="M4.75 20.25a7.25 7.25 0 0 1 14.5 0"/>')
icon('flag',
     f'<path d="M5.5 4.6h11.6l-2.7 3.5 2.7 3.5H5.5Z" fill="{RED}" fill-opacity=".12"/>'
     '<path d="M5.5 3.5v17"/>',
     color=RED)
icon('skull',
     '<path d="M12 3.6a7.3 7.3 0 0 0-7.3 7.3c0 2.6 1.4 4.4 3 5.5v2.3A1.3 1.3 0 0 0 9 20h6a1.3 1.3 0 0 0 1.3-1.3v-2.3c1.6-1.1 3-2.9 3-5.5A7.3 7.3 0 0 0 12 3.6Z" fill="currentColor" fill-opacity=".06"/>'
     '<circle cx="9.3" cy="11.4" r="1.5" fill="currentColor" stroke="none"/>'
     '<circle cx="14.7" cy="11.4" r="1.5" fill="currentColor" stroke="none"/>'
     '<path d="M10.5 20v-1.6"/><path d="M13.5 20v-1.6"/>')
icon('hash',
...
icon('handshake',
     '<path d="M2.75 12.4 7.4 8.3l4.8 3.1"/>'
     '<path d="M21.25 12.4 16.6 8.3l-3 2.5"/>'
     '<path d="m12.2 11.4 1.35 1.35a1.25 1.25 0 0 1-1.77 1.77l-1.35-1.35"/>'
     '<path d="m10.43 13.17 1.1 1.1a1.25 1.25 0 0 1-1.77 1.77l-1.1-1.1"/>'
     '<path d="m8.66 14.94.85.85a1.25 1.25 0 0 1-1.77 1.77l-.85-.85"/>'
     '<path d="m16.2 13.9 1.9 1.65"/>')
icon('bee',
...
icon('hundred',
     ...
     '<ellipse cx="16.6" cy="12" rx="1.9" ry="3.7"/>'
     '<path d="M4.6 18.9c4.9 1 9.9 1 14.8 0" stroke-opacity=".55"/>')


def main():
    os.makedirs(OUT, exist_ok=True)
    for name, spec in ICONS.items():
        stroke = spec['color'] or 'currentColor'
        attrs = {
            'xmlns': 'http://www.w3.org/2000/svg', 'viewBox': '0 0 24 24',
            'fill': 'none', 'stroke': stroke, 'stroke-width': '1.75',
            'stroke-linecap': 'round', 'stroke-linejoin': 'round',
        }
        attrs.update(spec['root'])
        attr_str = ' '.join(f'{k}="{v}"' for k, v in attrs.items() if v != 'none' or k == 'fill')
        svg = f'<svg {attr_str}>{spec["inner"]}</svg>\n'
...
def write_preview():
    """Contact sheet for eyeballing the set in light + dark: assets/icons/preview.html"""
    cells = []
    for name in sorted(ICONS):
        svg = open(os.path.join(OUT, f'{name}.svg')).read()
        cells.append(f'<div class="cell"><div class="ic">{svg}</div><div class="nm">{name}</div></div>')
    html = '''<!doctype html><meta charset="utf-8"><title>BEA icon set preview</title><style>
body{font-family:system-ui;margin:0;display:flex}
.pane{flex:1;padding:20px}
.light{background:#f8fafc;color:#1e293b}
.dark{background:#020617;color:#e2e8f0}
.grid{display:grid;grid-template-columns:repeat(8,1fr);gap:6px}
.cell{display:flex;flex-direction:column;align-items:center;padding:8px 2px;border-radius:8px}
.light .cell{background:#fff;border:1px solid #e2e8f0}
.dark .cell{background:rgba(255,255,255,.03);border:1px solid #1e293b}
.ic svg{width:30px;height:30px;display:block}
.nm{font-size:8.5px;margin-top:5px;opacity:.65}
</style>
<div class="pane light"><div class="grid">CELLS</div></div>
<div class="pane dark"><div class="grid">CELLS</div></div>
'''.replace('CELLS', ''.join(cells))
    with open(os.path.join(OUT, 'preview.html'), 'w') as f:
        f.write(html)
    print('preview.html regenerated')
