...
// Presence-based viewer tracking — each viewer writes a heartbeat doc
// Stale docs (>60s) are ignored in the count
var _viewerId = 'v_' + Math.random().toString(36).substring(2, 10) + Date.now().toString(36);
...
function joinStation(stationId) {
    ...
    if (typeof firebase !== 'undefined' && firebase.firestore) {
        ...
        // Write presence doc
        db.collection('tctv_presence').doc(_viewerId).set({
            station: stationId,
        ...
        // Listen for all presence docs and count by station
        if (!_viewerUnsub) {
            _viewerUnsub = db.collection('tctv_presence').onSnapshot(function(snap) {
                var counts = {};
                var cutoff = Date.now() - 60000; // 60s ago
                snap.forEach(function(doc) {
                    var d = doc.data();
                    if (d.ts && d.ts.toMillis && d.ts.toMillis() > cutoff && d.station) {
...
function leaveStation() {
    ...
    if (typeof firebase !== 'undefined' && firebase.firestore) {
        firebase.firestore().collection('tctv_presence').doc(_viewerId).delete().catch(function() {});
    }
...
window.renderTimechainTV = function() {
    ...
    STATIONS.forEach(function(s, idx) {
        ...
        html += '<div data-station-id="' + s.id + '" onclick="document.getElementById(\'main\').scrollTop=0;window.scrollTo(0,0);switchStation(\'' + s.id + '\')" style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:10px;cursor:pointer;transition:0.2s;background:' + (isActive ? 'rgba(247,147,26,0.1)' : 'transparent') + ';border:1px solid ' + (isActive ? 'rgba(247,147,26,0.3)' : 'transparent') + ';">';
