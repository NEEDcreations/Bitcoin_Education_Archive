...
// Simple iframe-based player — no YouTube API needed
...
// createPlayer removed — using simple iframe
...
window.renderTimechainTV = function() {
    ...
    // Overlay removed — iframe handles interaction
    html += '</div>';
    ...
    var _startPlayback = function() {
        ...
        function initPlayer() {
            if (true) {
                ...
                // Play enforcer removed — iframe handles playback
            } else {
...
window.switchStation = function(stationId) {
    ...
    // Load new video — just set iframe src
    if (state.video) {
        loadVideo(state.video.id, state.offset);
...
window.cleanupTimechainTV = function() {
    ...
    // Player cleanup — iframe will be replaced on next loadVideo
    // playerReady removed
...
// YouTube IFrame API not used — simple iframe embeds instead
